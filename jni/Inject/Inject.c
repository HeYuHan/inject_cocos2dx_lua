...
int main(int argc, char *argv[])
{
	...
	printf("begin attach pid %d\n", pid);
	// Attach
	if (0 > ptrace(PTRACE_ATTACH, pid, 0, 0)) {
		printf("cannot attach to %d, error!\n", pid);
		exit(1);
	}
	printf("begin wait pid %d\n", pid);
	waitpid(pid, NULL, 0);
	if (appname) {
...
//int main(int argc, char** argv) {
//    printf("inject start...\n");
//    pid_t target_pid=find_pid_of("com.mahjong.sichuang");
//    printf("find pid:%d\n",target_pid);
//    char* args="i am arg sb";
//    char* sodir="/data/data/com.mahjong.sichuang/lib/libcocos2dlua.so";
//    char* func_name="hook_entry";
//    int ret = inject_remote_process(target_pid, sodir, func_name,  args, strlen(args));
//    printf("inject ret:%d\n",ret);
//    return 0;
//}
