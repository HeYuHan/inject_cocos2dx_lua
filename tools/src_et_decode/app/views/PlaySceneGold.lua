...
local PlaySceneGold = class("PlaySceneGold", function()
	return cc.Scene:create()
end)

PlaySceneGold.DecisionType = {
	-- 接炮胡
	TAKE_CANNON_WIN				= 1,
	-- 自摸胡
	SELF_DRAWN_WIN				= 2,
...
PlaySceneGold.StartDecisionType = {
	-- 缺一色
	TYPE_QUEYISE				= 1,
	-- 板板胡
	TYPE_BANBANHU				= 2,
...
function PlaySceneGold:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	local csbNode, animation = gt.createCSAnimation("PlaySceneGold.csb")
	...
	self.rootNode:reorderChild(self.decisionBtnNode, PlaySceneGold.ZOrder.DECISION_BTN)
	self.decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(selfDrawnDcsNode, PlaySceneGold.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneGold.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		if gt.debugMode then
			...
			self:addChild(settingPanel, PlaySceneGold.ZOrder.SETTING,102)
		else
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneGold.ZOrder.CHAT , 101)
	end)

	-- 麻将层
	local playMjLayer = cc.Layer:create()
	self.rootNode:addChild(playMjLayer, PlaySceneGold.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneGold.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneGold.ZOrder.DISMISS_ROOM)
...
function PlaySceneGold:onRcvGoonNext(msgTbl)
	gt.dump(msgTbl)
...
--玩家领取金币成功
function PlaySceneGold:onRcvGetGold(msgTbl)
	gt.dump(msgTbl)
...
--广播玩家金币信息
function PlaySceneGold:onRcvplayerGold(msgTbl)
	gt.dump(msgTbl)
	if msgTbl.m_pos then
		...
		-- scoreLabel:setString(tostring(msgTbl.m_coins))
		-- roomPlayer.scoreLabel = scoreLabel
	end
end

function PlaySceneGold:playYuYinAnimation()
	local action = nil
...
function PlaySceneGold:cancelYuYinAnimation()
...
function PlaySceneGold:getLuaBridge()
...
function PlaySceneGold:unregisterAllMsgListener()
...
-- end --
function PlaySceneGold:onRcvLoginServer(msgTbl)
	self.startGame = true
...
function PlaySceneGold:onRecUserRemoveBarCard( msgTbl )
...
-- 断线重连,走一次登录流程
function PlaySceneGold:reLogin()
	-- print("========重连登录1")
...
function PlaySceneGold:onRcvLogin(msgTbl)
	-- print("========重连登录4")
	-- 去掉转圈
	-- gt.removeLoadingTips()
...
--服务器返回gate登录
function PlaySceneGold:onRcvLoginGate( msgTbl )
...
function PlaySceneGold:onNodeEvent(eventName)
...
--金币场踢人
function PlaySceneGold:onRcvReturnMainScene(msgTbl)
	-- gt.dump(msgTbl)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
...
--玩家自动出牌
function PlaySceneGold:onRcvUserPlayWork(msgTbl)
	gt.log("客户端摸牌后的操作")
	gt.dump(msgTbl)
	...
	if  msgTbl.m_type == 1 then
		...
		if gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision"):isVisible() or gt.seekNodeByName(self.rootNode, "Node_decisionBtn") then
			gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision"):setVisible(false)
			gt.seekNodeByName(self.rootNode, "Node_decisionBtn"):setVisible(false)
...
-- @return
-- end --
function PlaySceneGold:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneGold:playTimeCDUpdate(delta)
...
-- end --
function PlaySceneGold:readyBtnClickEvt()
...
function PlaySceneGold:TimeCDCallback()
	local msgToSend = {}
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneGold:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvGiveGold(msgTbl)
	gt.log("赠送金币")
	gt.dump(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneGold:onEnterBackground()
	if gt.isIOSPlatform() or gt.isAndroidPlatform then
...
function PlaySceneGold:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneGold:onTouchBegan(touch, event)
	...
	if self.isPlayThreeCardState == false then
		...
		if self.replaceThreeCardType then
			...
			gt.log("onTouchBegan888888888888888")
			if self.isPlayerShow then
				gt.log("self.isPlayerShow = true")
			else
				gt.log("self.isPlayerShow = false")
			end
			if self.isPlayerDecision then
				gt.log("self.isPlayerDecision = true")
			else
				gt.log("self.isPlayerDecision = false")
			end
			...
			gt.log("onTouchBegan666666666666666666666")
			local touchMjTile, mjTileIdx = self:touchPlayerMjTiles(touch)
			if not touchMjTile then
				return false
			end
			...
			gt.log("onTouchBegan77777777777777")
			-- 判断选中麻将牌的花色 如果是定缺的花色 
			local roomPlayer = self.roomPlayers[self.playerSeatIdx]
			if touchMjTile.mjColor ~= roomPlayer.dingQueColor and #roomPlayer.dingQueTable>0 then
				return false
			...
			gt.log("onTouchBegan6666666666666666666")
			-- 表示自己发送了换三张消息
			if self.ownerSendReplaceCardMsg then
				return false
			end
			gt.log("onTouchBeganself.ownerReplaceCardType")
			---eeee示别人没换完  但自己换完了   
			if self.ownerReplaceCardType then
				return false
			end
			gt.log("onTouchBegantouchPlayerMjTiles(touch)")
			-- 表示没换完
			local touchMjTile, mjTileIdx = self:touchPlayerMjTiles(touch)
			if not touchMjTile then
				return false
...
function PlaySceneGold:onTouchMoved(touch, event)
...
function PlaySceneGold:onTouchEnded(touch, event)
	if self.AIoutcard then 
		self.chooseMjTile = nil
		self.isTouchBegan = false
		self.isTouchMoved = false
...
function PlaySceneGold:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneGold:update(delta)
...
function PlaySceneGold:checkVersion()
...
function PlaySceneGold:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneGold:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneGold:initWifi()
...
function PlaySceneGold:updateWifi()
...
function PlaySceneGold:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneGold:checkRightCradNum()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvAddPlayer(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvRemovePlayer(msgTbl)
	gt.dump(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
...
-- @description 断线重连
-- end --
function PlaySceneGold:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneGold:CheckScore()
...
-- 校验换三张
function PlaySceneGold:onRcvSyncReplaceCardType( )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvReady(msgTbl)
	gt.dump(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvStartGame(msgTbl)
	gt.dump(msgTbl)
	self.startGame = true
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:onRcvTurnShowMjTile(msgTbl)
	dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	self.roundStateNode:setVisible(true)
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneGold.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneGold.FLIMTYPE.FLIMLAYER_BU)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneGold.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneGold.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneGold.ZOrder.FLIMLAYER,PlaySceneGold.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneGold.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneGold.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneGold.ZOrder.FLIMLAYER,PlaySceneGold.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
-- @description 广播玩家出牌
-- end --
function PlaySceneGold:onRcvSyncShowMjTile(msgTbl)
	dump(msgTbl)
	...
	if msgTbl.m_type == 2 then
		...
		self:showDecisionAnimation(seatIdx, PlaySceneGold.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)

		self:showHuPaiType(seatIdx,msgTbl.m_color, msgTbl.m_number,true)

	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneGold.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == -1 then
		...
		-- roomPlayer.holdMjTiles[roomPlayer.mjTilesRemainCount].mjTileSpr:setVisible(false)
		-- roomPlayer.mjTilesRemainCount = roomPlayer.mjTilesRemainCount - 1--此处金币场有问题 12.19
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneGold.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneGold.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneGold.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneGold.DecisionType.BRIGHT_BU)
		end
	end
...
-- end --
function PlaySceneGold:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneGold.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneGold.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneGold.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneGold.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneGold:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneGold:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneGold.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneGold:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneGold:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneGold.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneGold.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneGold.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneGold.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneGold.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
-- @description 通知玩家决策
-- end --
function PlaySceneGold:onRcvMakeDecision(msgTbl)
...
--msgTbl.m_think = {{6, {{3,4},{3,5}}}, {6, {{3,7}, {3,8}}}, {6, {{3,5},{3,7}}}   }
	...
	if msgTbl.m_flag == 1 then
		...
		self:playTimeCDStart(gt.WaitTime)
		self.TimeCDControl = true
		-- self:CreateReadyTimeCDStart("20",cc.p(gt.winSize.width * 0.5,self.readyBtn:getPositionY()-50),true)
...
-- @description 广播决策结果
-- end --
function PlaySceneGold:onRcvSyncMakeDecision(msgTbl)
	gt.log("广播决策结果")
	dump(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneGold.DecisionType.QIANG_GANG, msgTbl.m_hu)
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneGold.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneGold.DecisionType.EAT)
...
function PlaySceneGold:onRcvChatMsg(msgTbl)
...
function PlaySceneGold:onRcvRoundXueliu(msgTbl)
...
function PlaySceneGold:onRcvRoundReport(msgTbl)
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		-- 弹出局结算界面
		local roundReport = require("app/views/RoundReport_Gold"):create(curRoomPlayers, self.playerSeatIdx, msgTbl,self.playerScore)
		self:addChild(roundReport, PlaySceneGold.ZOrder.ROUND_REPORT,103)
...
function PlaySceneGold:copyTab(st)
    local tab = {}
...
function PlaySceneGold:onRcvFinalReport(msgTbl)
	gt.log("进入总结算")
	self.lastRound = true
	local curRoomPlayers = {}
	curRoomPlayers = self:copyTab(self.roomPlayers)
	...
	self:addChild(self.finalReport, PlaySceneGold.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneGold:updateCurrentTime()
...
function PlaySceneGold:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneGold:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGold:playerEnterRoom(msgTbl)
	gt.log("玩家自己进入房间")
	gt.dump(msgTbl)
	-- 房间中的玩家
	self.roomPlayers = {}
	...
	for i=1,#msgTbl.m_playtype do
		...
		elseif msgTbl.m_playtype[i] == 46 then
...
-- @param seatIdx 座次
-- end --
function PlaySceneGold:playerGetReady(seatIdx)
	self.playTimeCDLabel:setString("0")
	--初始化控件
	self.readyPlayNode:setVisible(true)
	self.roundStateNode:setVisible(false)
...
-- end --
function PlaySceneGold:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneGold:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneGold.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneGold:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneGold:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
...
-- @param replaceType
-- end --
function PlaySceneGold:addMjTileToPlayer(mjColor, mjNumber, replaceType)
...
-- end --
function PlaySceneGold:sortPlayerMjTiles( isthreeCard )
...
-- @return 选中的麻将牌
-- end --
function PlaySceneGold:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneGold:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneGold:addAlreadyHuMjTiles(seatIdx, mjColor, mjNumber, index)
...
function PlaySceneGold:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneGold:checkMjTile()
...
function PlaySceneGold:checkPengAndGang( isGang, color, number )
...
-- end --
function PlaySceneGold:removePreRoomPlayerOutMjTile(color, number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneGold:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneGold:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneGold:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneGold:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneGold:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneGold:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneGold:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneGold:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
function PlaySceneGold:removeHuMj()
	for i = 1, 4 do
		local Node_HuGroup = gt.seekNodeByName(self.rootNode, "Node_HuGroup_" .. i)
		for m = 1, 3 do
			local Img_kuang = gt.seekNodeByName(Node_HuGroup, "Img_kuang_" .. m)
...
-- @param decisionType 决策类型
-- end --
function PlaySceneGold:showDecisionAnimation(seatIdx, decisionType, huCard)
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneGold.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneGold.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneGold.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneGold.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneGold.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneGold.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneGold.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneGold.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneGold.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneGold.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneGold.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneGold:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneGold:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneGold:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneGold:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneGold.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneGold.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneGold.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneGold.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneGold.TAG.FLIMLAYER_BU)
	else
...
-- @param isShow 显示标志
-- end --
function PlaySceneGold:LaoYueNodeVisible(isShow)
	-- body
	local laoYueNode = gt.seekNodeByName(self.rootNode, "Node_Laoyue")
	if(isShow)then
		self.rootNode:reorderChild(laoYueNode, PlaySceneGold.ZOrder.HAIDILAOYUE)
...
function PlaySceneGold:startAudio()
...
function PlaySceneGold:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneGold:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
-- @param msgTbl
-- end --
function PlaySceneGold:onRecUserDingQue( msgTbl )
	gt.dump(msgTbl)
	for i=1,4 do
		local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. i)
		local Image_que = gt.seekNodeByName(playerInfoNode,"Image_dingque")
...
-- 定缺完成
function PlaySceneGold:onRecUserDingQueComplate( msgTbl )
	gt.dump(msgTbl)
	if self.dingqueInfo then
		self.dingqueInfo:setVisible(false)
	end
...
function PlaySceneGold:showDingQueType( seatIdx, color )
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneGold:dingquePrompt()
...
function PlaySceneGold:jianAnimate( node )
	node:stopAllActions()
...
function PlaySceneGold:setHuTag(seatIdx)
	local hutypeNode = gt.seekNodeByName(self.rootNode,"Node_hutype")
...
function PlaySceneGold:showHuPaiType(seatIdx ,color, number , mjhuPaiType)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 确定可以换的花色
function PlaySceneGold:promptReplaceThreeCard()
...
function PlaySceneGold:exhReplaceThreeCard()
...
function PlaySceneGold:replaceThreeCardAction()
...
function PlaySceneGold:ThreeCardInsertMjType( mjTile )
...
function PlaySceneGold:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneGold:onRecUserReplaceCard( msgTbl )
	dump(msgTbl)
	gt.log("开始换三张 。。。。。。。。。。 ")
...
function PlaySceneGold:checkReplaceOkBtn()
...
function PlaySceneGold:onRecUserReplaceCardComplate( msgTbl )
...
function PlaySceneGold:removeThreeCardFormPlayer()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
	for i=#roomPlayer.holdMjTiles, 1, -1 do 
		local card = roomPlayer.holdMjTiles[i]
...
function PlaySceneGold:IsSameIp()
	local tmp = {}
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneGold:showLastFourCard()
	local Image_ReplaceNotice = gt.seekNodeByName(self.rootNode,"Image_ReplaceNotice")
	Image_ReplaceNotice:setVisible(true)
	Image_ReplaceNotice:setPosition(cc.p(640,200))
	self.Image_ReplaceNotice = Image_ReplaceNotice
