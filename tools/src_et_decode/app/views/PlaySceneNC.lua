...
local PlaySceneNC = class("PlaySceneNC", function()	
	return cc.Scene:create()
end)

PlaySceneNC.DecisionType = {
	-- 接炮胡
	TAKE_CANNON_WIN				= 1,
	-- 自摸胡
	SELF_DRAWN_WIN				= 2,
...
function PlaySceneNC:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	local csbNode, animation = gt.createCSAnimation("PlaySceneNC.csb")
	...
	for i = 1, 4 do
		...
		local Spr_piao = gt.seekNodeByName(playerNode,"Spr_piao")
		if Spr_piao then
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneNC.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	self.selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(self.selfDrawnDcsNode, PlaySceneNC.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneNC.ZOrder.CHAT)
	chatBgNode:setVisible(false)
	--飘按钮
	self.piaoNode = gt.seekNodeByName(self.rootNode, "Node_Piao")
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneNC.ZOrder.SETTING,102)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneNC.ZOrder.CHAT, 101)
	...
	self.rootNode:addChild(playMjLayer, PlaySceneNC.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneNC.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneNC.ZOrder.DISMISS_ROOM)
	...
	self.langBg = gt.seekNodeByName(self.Node_Tang, "Img_bg")
	self.back = gt.seekNodeByName(self.langBg, "Btn_return")
	self.back:setTag(50)
	self.confirm = gt.seekNodeByName(self.langBg, "Btn_sure")
	self.confirm:setTag(51)

	--点完躺决策之后出现的返回按钮
	self.btn_backTang = gt.seekNodeByName(self.rootNode, "Btn_decision_return")
	self.btn_backTang:setVisible(false)
	gt.addBtnPressedListener(self.btn_backTang, function()
		    self.btn_backTang:setVisible(false)
			...
			self:setMaskLayer(self.roomPlayers[self.playerSeatIdx], maskCardTbl, true)
...
function PlaySceneNC:playYuYinAnimation()
	local action = nil
...
function PlaySceneNC:cancelYuYinAnimation()
...
function PlaySceneNC:getLuaBridge()
...
function PlaySceneNC:unregisterAllMsgListener()
...
-- end --
function PlaySceneNC:onRcvLoginServer(msgTbl)
...
function PlaySceneNC:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneNC:onRcvLoginGate(msgTbl)
...
function PlaySceneNC:onNodeEvent(eventName)
...
function PlaySceneNC:onRecUserRemoveBarCard( msgTbl )
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneNC:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneNC:onEnterBackground()
...
function PlaySceneNC:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneNC:onTouchBegan(touch, event)
	...
	if self.isPlayThreeCardState == false then
		gt.log("self.isPlayThreeCardState == false2")
		-- 不需要换三张 
		-- 胡牌后不可以出牌了
		...
		gt.log("self.isPlayThreeCardState == true")
		-- 选择了换三张玩法
...
function PlaySceneNC:onTouchMoved(touch, event)
	...
	if self.isPlayThreeCardState then
		...
		gt.log("------------------onTouchMoved99999999999999999----------------")
		local touchPoint = self.playMjLayer:convertTouchToNodeSpace(touch)
		if not self.isTangDecision then
			self.chooseMjTile.mjTileSpr:setPosition(touchPoint)
		else
			self.isTouchBegan = false
		end
	...
	gt.log("------------------onTouchMoved2222222222222222----------------")
end

function PlaySceneNC:onTouchEnded(touch, event)
	gt.log("------------------onTouchEnded----------------")
	if self.isPlayThreeCardState == false then
		-- 不需要换三张 
		gt.log("------------------onTouchEnded1----------------")
		local isShowMjTile = false
		-- 拖拽出牌
		local touchPoint = self.playMjLayer:convertTouchToNodeSpace(touch)
		if cc.pDistanceSQ(self.preTouchPoint, touchPoint) > 400 then
			gt.log("------------------onTouchEnde888888----------------")
			-- 拖拽距离大于20判断为拖动
			local roomPlayer = self.roomPlayers[self.playerSeatIdx]
			...
			if self.chooseMjTile ~= self.preClickMjTile then
				...
				gt.log("------------------onTouchEnded11111111111111----------------")
				--显示躺选择牌
		...
		gt.log("------------------onTouchEnded4----------------")
		if isShowMjTile then
			gt.log("------------------onTouchEnded5----------------")
			if self.isTangDecision then
				gt.log("------------------onTouchEnded6----------------")
				return
			end
			-- 发送出牌消息
			local msgToSend = {}
...
function PlaySceneNC:outPaiBigAnimate(seatIdx, color, number ,isShow)
	...
	if isShow == 1 then
		...
		SpritePai:setSpriteFrame(string.format("p4s%d_%d.png", color, number))
...
function PlaySceneNC:update(delta)
...
function PlaySceneNC:checkVersion()
...
function PlaySceneNC:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneNC:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneNC:updateWifi()
...
function PlaySceneNC:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneNC:checkRightCradNum()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvAddPlayer(msgTbl)
	dump(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneNC:onRcvSyncRoomState(msgTbl)
	...
	if self.Node_Tang:isVisible() then
		for i=1,15 do
			if self.langBg:getChildByTag(i) then
				self.langBg:removeChildByTag(i)
			end
		end
		self.Node_Tang:setVisible(false)
		gt.popLayer()
	...
	if msgTbl.m_dCount == 12 then
		self:showLastEightCard()
	end
	...
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		...
		--听牌不能出
		if msgTbl["m_tingCards" .. turnPos] then
			roomPlayer.tingCards = msgTbl["m_tingCards" .. turnPos]
		end
	...
	if self.roomPlayers[self.playerSeatIdx].langMjTiles and #self.roomPlayers[self.playerSeatIdx].langMjTiles > 0 then
	else
		local maskCardTbl = self:getPlayerTings()
		self:setMaskLayer(self.roomPlayers[self.playerSeatIdx], maskCardTbl, true)
	end
...
--检测总分和是否为0
function PlaySceneNC:CheckScore()
...
function PlaySceneNC:onRcvWinCard(msgTbl)
	gt.log("onRcvWinCard:================")
	dump(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
...
-- 校验换三张
function PlaySceneNC:onRcvSyncReplaceCardType( )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:onRcvTurnShowMjTile(msgTbl)
	dump(msgTbl)
	gt.log("通知玩家出牌")
	self.Node_Tang:setVisible(false)
	-- 牌局状态,剩余牌
	...
	if seatIdx == self.playerSeatIdx then
		...
		if msgTbl.m_flag == 0 then
			...
			if not roomPlayer.isLangSelectedState then
				local maskCardTbl = self:getPlayerTings()
				gt.log("无躺的时候发定缺牌 isLangSelectedState ＝ false")
				for i = 1, #maskCardTbl do
					if maskCardTbl[i][1] == msgTbl.m_color and maskCardTbl[i][2] == msgTbl.m_number then
						mjTile.mjTileSpr:setColor(cc.c3b(100,100,100))
						mjTile.isMark = true
					end
				end
			end
			gt.log("#roomPlayer.holdMjTiles = "..#roomPlayer.holdMjTiles)
			if not roomPlayer.isLangSelectedState then
				local maskCardTbl = self:getPlayerTings()
				self:setMaskLayer(roomPlayer, maskCardTbl, true)
			end
		...
		if #decisionTypes > 0 then
			...
			for _, decisionBtn in ipairs(self.selfDrawnDcsNode:getChildren()) do
				...
				if nodeName == "Btn_decisionPass" then
					-- 设置不存在的索引值
					self.decisionBtn_guo = decisionBtn
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							self.selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneNC.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneNC.FLIMTYPE.FLIMLAYER_BU)
			...
			if msgTbl.m_bOnlyHu == true then
				-- self.selfDrawnDcsNode:removeChildByTag(0)
			...
			for idx, decisionData in ipairs(decisionTypes) do
				...
				elseif decisionData.flag == 11 then
					-- 躺
					gt.log("把躺决策加入决策层")
					local btn_lang_name = "Btn_decision_bai"
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneNC.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneNC.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneNC.ZOrder.FLIMLAYER,PlaySceneNC.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneNC.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneNC.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneNC.ZOrder.FLIMLAYER,PlaySceneNC.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
				...
				elseif decisionData.flag == 11 then  -- tang
					gt.addBtnPressedListener(decisionBtn, function(sender)
						...
						--隐藏决策按钮
						-- for i, btn_spr in ipairs(btn_presentList) do
						-- 	btn_presentList[i][2]:setVisible(false)
						-- end
						-- --隐藏过按钮
						-- self.decisionBtn_guo:setVisible(false)
						--隐藏选择背景
						self.btn_backTang:setVisible(true)
						--屏蔽非停牌
...
function PlaySceneNC:checkOtherTingCards(roomPlayer, m_tingCards)
	local maskCardTbl = {}
...
--isOther：是否是自己
function PlaySceneNC:setMySelfMaskLayer(roomPlayer, tingInfo)
	local num = #tingInfo
	if num < 1 then
		return
	end
...
--tingInfo：要屏蔽的听的牌：
--isOther：是否是自己
function PlaySceneNC:setMaskLayer(roomPlayer, tingInfo, isOther)
	...
	gt.log("tingNum = "..tingNum)
	if tingNum == #roomPlayer.holdMjTiles then
		self:setAllMaskLayer(false)
		gt.log("手牌都不置灰")
		return 
	end
...
function PlaySceneNC:setAllMaskLayer(isMark)
...
function PlaySceneNC:createMaskLayer(opacity)
	if not opacity then
		-- 用默认透明度
...
--显示选择狼起的牌
function PlaySceneNC:showSelectMj()
	gt.log("进入躺决策按钮回调")
	--添加不能触摸层
	...
	local mjTileName = string.format("p4b1_1.png") --获取图片
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)  --创建精灵
	self.Node_Tang:setZOrder(112)
	self.Node_Tang:setVisible(true)
	self.langBg:setContentSize(cc.size(((#playerOwner.holdMjTiles - 1) *  mjTileSpr:getContentSize().width) + mjTileSpr:getContentSize().width * 1.3, mjTileSpr:getContentSize().height * 1.3))
	local langTbl = {}
	for i, mjTile in pairs(playerOwner.holdMjTiles) do
		if self.chooseMjTile.mjTileSpr ~= mjTile.mjTileSpr then
			table.insert(langTbl, mjTile)
		end
	end
	gt.dump(langTbl)
	self.spr_num = #langTbl
	for i, mjTile in pairs(langTbl) do
		mjTile.selectedState = false
	
		local mjTileName = string.format(gt.SelfMJSprFrame, mjTile.mjColor, mjTile.mjNumber) --获取图片

		local checkSpr = cc.Sprite:create("images/otherImages/btn_checkbox1.png")  --创建精灵
		local checkGou = cc.Sprite:create("images/otherImages/btn_check1.png")  --创建精灵
		checkGou:setVisible(false)

		local button = ccui.Button:create()
		button:loadTextures(mjTileName,mjTileName,"",ccui.TextureResType.plistType)
		button:setTouchEnabled(true)
		button:setTag(i)
		button:setZOrder(100-i)
			
		local function touchEvent(sender, type)
   			if type == ccui.TouchEventType.ended then
   				if sender:getChildByTag(10):isVisible() then
   					sender:getChildByTag(10):setVisible(false)
   					mjTile.selectedState = false
   				else
   					sender:getChildByTag(10):setVisible(true)
   					mjTile.selectedState = true
   				end
   		 	end
	  	end
	 	button:addTouchEventListener(touchEvent)
		button:addChild(checkGou, 20, 10)
		button:addChild(checkSpr)
		checkGou:setPosition(mjTileSpr:getContentSize().width - checkSpr:getContentSize().width / 2, checkSpr:getContentSize().height / 2)
		checkSpr:setPosition(mjTileSpr:getContentSize().width - checkSpr:getContentSize().width / 2, checkSpr:getContentSize().height / 2)
		self.langBg:addChild(button)
		button:setPosition(cc.p(10 + mjTileSpr:getContentSize().width * i, self.langBg:getContentSize().height / 2))
	end
	...
	local function backCallBack(sender, eventType)
        ...
        if eventType == ccui.TouchEventType.began then
			...
			if self.selfDrawnDcsNode:isVisible() then
				self.selfDrawnDcsNode:setVisible(false)
			end
			self.isPlayerDecision = false
        	--self:showBackBg()
		    -- self.btn_backLang:setVisible(true)
		    -- self.selfDrawnDcsNode:setVisible(true)
		    self.btn_backTang:setVisible(true)
	...
	--返回
	-- local back = gt.seekNodeByName(self.Node_Tang, "Btn_return")
	self.back:setTouchEnabled(true)
	self.back:addTouchEventListener(backCallBack)
	self.back:setPosition(cc.p(0, self.langBg:getContentSize().height / 2))


	local function confirmCallBack(sender, eventType)
		gt.popLayer()
        if eventType == ccui.TouchEventType.began then
        ...
        elseif eventType == ccui.TouchEventType.ended then
		    for i=1,self.spr_num do
		   	...
		   	-- self.btn_backLang:setVisible(false)
		   	self.selfDrawnDcsNode:setVisible(false)
		    local selectedTable = {}
		    for i, mjTile in pairs(playerOwner.holdMjTiles) do
		    	if mjTile.selectedState == true then
		    		table.insert(selectedTable, {mjTile.mjColor, mjTile.mjNumber})
		    	end
		    end

		   if #selectedTable % 3 ~= 0 then
		        local msgToSend = {}
				msgToSend.m_msgId = gt.CG_TANGCARD
				msgToSend.m_pos = self.playerSeatIdx - 1
				msgToSend.m_cardValue = selectedTable
	...
	--确定按钮
	-- local confirm = gt.seekNodeByName(self.Node_Tang, "Btn_sure")
	self.confirm:setTouchEnabled(true)
	self.confirm:addTouchEventListener(confirmCallBack)
	self.confirm:setPosition(cc.p(self.langBg:getContentSize().width, self.langBg:getContentSize().height / 2))

	self.langBg:setPosition(gt.winSize.width / 2,self.langBg:getContentSize().height * 1.5)
	if display.autoscale == "FIXED_HEIGHT" then
		self.rootNode:setScale(0.75)
		self.langBg:setScale(0.9)
		self.langBg:setPositionX(650)
	end
end
--躺成功
function PlaySceneNC:onRecErrorInfo(msgTbl)
	...
	if msgTbl.m_result == 0 then
		...
		self.selfDrawnDcsNode:setVisible(false)
		self.Node_Tang:setVisible(false)
		--table.insert(roomPlayer.langQReorderMjTiles, eatGroup)
		if roomPlayer.seatIdx == self.playerSeatIdx then
			gt.log("自己躺牌")
			self.isPlayerDecision = false
		...
		roomPlayer.langMjTiles = msgTbl.m_cardValue
		...
		require("app/views/NoticeTips"):create("提示", "选择错误", nil, nil, true)
		self:showSelectMj()
		gt.seekNodeByName(self.Node_Tang, "Btn_sure"):setTouchEnabled(true)
	end
end
function PlaySceneNC:setOtherMark()
...
function PlaySceneNC:getPlayerTings()
	local maskCardTbl = {}
...
--移除手中狼起的牌
function PlaySceneNC:sendSelectedMj(seatIdx)
	-- 发送出牌消息
	local roomPlayer = self.roomPlayers[seatIdx]
	if roomPlayer.seatIdx == self.playerSeatIdx then
		self.isPlayerShow = false
...
-- @return
-- end --
function PlaySceneNC:langQReorderMjTiles(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	...
	for i, mjTile in ipairs(roomPlayer.langMjTiles) do
		local mjTileName = string.format(gt.MJSprFrameOut, roomPlayer.displaySeatIdx, roomPlayer.langMjTiles[i][1], roomPlayer.langMjTiles[i][2])
		local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
...
function  PlaySceneNC:updatePlayerInfo()
	for i = 1, 4 do
		local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. i)
...
-- @description 广播玩家出牌
-- end --
function PlaySceneNC:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		-- 自摸胡
		self:showDecisionAnimation(seatIdx, PlaySceneNC.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneNC.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneNC.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneNC.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneNC.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneNC.DecisionType.BRIGHT_BU)
		end
...
-- end --
function PlaySceneNC:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneNC.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneNC.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneNC.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneNC.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneNC:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneNC:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneNC.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneNC:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneNC:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneNC.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneNC.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneNC.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneNC.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneNC.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
function PlaySceneNC:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneNC:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
	-- local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
	-- decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneNC:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneNC:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneNC.DecisionType.QIANG_GANG, msgTbl.m_hu)
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneNC.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneNC.DecisionType.EAT)
...
function PlaySceneNC:onRcvChatMsg(msgTbl)
...
function PlaySceneNC:showFangjianYuAnimation(msgTbl, seatIdx)
...
function PlaySceneNC:showPaiType(roundReportMsg, seatIdx)
	if not self.paiTypeActivity then
		...
		self:addChild(self.paiTypeActivity, PlaySceneNC.ZOrder.ROUND_REPORT_Activity)
	end
end


--单局结束
function PlaySceneNC:onRcvRoundReport(msgTbl)
	self.roundReportMsg = nil
	gt.log("00000---")
	gt.dump(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
--显示单局结算
function PlaySceneNC:showRoundReport(msgTbl)
	--删除聊天框
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			local roundReport = require("app/views/RoundReport_MY"):create(self.roomPlayers, self.playerSeatIdx, msgTbl, msgTbl.m_end)
			self:addChild(roundReport, PlaySceneNC.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
			local roundReport = require("app/views/RoundReport_MY"):create(self.curRoomPlayers, self.playerSeatIdx, msgTbl, msgTbl.m_end)
			self:addChild(roundReport, PlaySceneNC.ZOrder.ROUND_REPORT)
		end
...
function PlaySceneNC:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneNC:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneNC:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneNC.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneNC:updateCurrentTime()
...
function PlaySceneNC:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneNC:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNC:playerEnterRoom(msgTbl)
...
-- end --
function PlaySceneNC:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneNC:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneNC:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneNC:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneNC.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneNC:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneNC:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
	-- 显示当先座位标识
...
-- @return
-- end --
function PlaySceneNC:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneNC:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneNC:addMjTileToPlayer(mjColor, mjNumber, replaceType)
...
-- end --
function PlaySceneNC:sortPlayerMjTiles( isthreeCard )
	...
	if num > 0 then
		local newTile = roomPlayer.holdMjTiles[num].mjTileSpr
		local newTileX, newTileY = newTile:getPosition()
...
-- @return 选中的麻将牌
-- end --
function PlaySceneNC:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneNC:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneNC:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneNC:checkMjTile()
...
function PlaySceneNC:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneNC:removePreRoomPlayerOutMjTile(color,number)
	-- 移除上家打出的牌
	gt.log("function is removePreRoomPlayerOutMjTile  color is "..color.." number is "..number)
	if self.preShowSeatIdx then
		gt.log("self.preShowSeatIdx is true")
		...
		gt.log("self.preShowSeatIdx is false")
		gt.socketClient:reloginServer()
	end
end
...
-- @param seatIdx 座次
-- end --
function PlaySceneNC:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneNC:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneNC:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneNC:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneNC:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneNC:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneNC:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneNC:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneNC:showDecisionAnimation(seatIdx, decisionType, huCard)
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneNC.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneNC.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneNC.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneNC.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneNC.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneNC.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneNC.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneNC.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneNC.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneNC.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneNC.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneNC:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneNC:backMainSceneEvt(eventType, isRoomCreater, roomID)
	gt.log("backMainSceneEvt")
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneNC:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format("p4s%d_%d.png", 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneNC:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneNC.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneNC.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneNC.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneNC.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneNC.TAG.FLIMLAYER_BU)
	else
...
function PlaySceneNC:startAudio()
...
function PlaySceneNC:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneNC:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
-- @param msgTbl
-- end --
function PlaySceneNC:onRecUserDingQue( msgTbl )
...
-- 定缺完成
function PlaySceneNC:onRecUserDingQueComplate( msgTbl )
	gt.dump(msgTbl)
...
function PlaySceneNC:showDingQueType( seatIdx, color )
	local roomPlayer = self.roomPlayers[seatIdx]

	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	local Spr_piao = gt.seekNodeByName(playerInfoNode,"Spr_piao")
	local Text_PiaoNum = gt.seekNodeByName(playerInfoNode,"Text_PiaoNum")
	if color > 0 then
		roomPlayer.piaoNum = color
		Spr_piao:setVisible(true)
		Text_PiaoNum:setString("x"..color)
	else
		Spr_piao:setVisible(false)
	end
end

function PlaySceneNC:dingquePrompt()
...
function PlaySceneNC:jianAnimate( node )
...
-- 显示飘X
function PlaySceneNC:ShowChoosPiao()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
	local function touchcallback1(sender)
		local msgToSend = {}
		msgToSend.m_msgId = gt.GC_USER_DING_QUE
		msgToSend.m_color = sender:getTag()
		msgToSend.m_pos = 0
		gt.socketClient:sendMessage(msgToSend)
		gt.dump(msgToSend)
		gt.soundManager:PlaySpeakSound(roomPlayer.sex, "piao", roomPlayer)
	end
	local function touchcallback2(sender)
		local msgToSend = {}
		msgToSend.m_msgId = gt.GC_USER_DING_QUE
		msgToSend.m_color = -1
		msgToSend.m_pos = 0
		gt.socketClient:sendMessage(msgToSend)
		gt.dump(msgToSend)
		gt.soundManager:PlaySpeakSound(roomPlayer.sex, "bupiao", roomPlayer)
	end
	--显示定飘
	self.piaoNode:setVisible(true)
	local Btn_piao1 = gt.seekNodeByName(self.piaoNode,"Btn_piao1")
	local Btn_piao2 = gt.seekNodeByName(self.piaoNode,"Btn_piao2")
	local Btn_piao3 = gt.seekNodeByName(self.piaoNode,"Btn_piao3")
	local Btn_piao4 = gt.seekNodeByName(self.piaoNode,"Btn_piao4")
	local Btn_piao5 = gt.seekNodeByName(self.piaoNode,"Btn_piao5")
	local Btn_bupiao = gt.seekNodeByName(self.piaoNode,"Btn_bupiao")
	Btn_piao1:setTag(1)
	Btn_piao2:setTag(2)
	Btn_piao3:setTag(3)
	Btn_piao4:setTag(4)
	Btn_piao5:setTag(5)
	gt.addBtnPressedListener(Btn_piao1, function(sender)
		touchcallback1(sender)
	end)
	gt.addBtnPressedListener(Btn_piao2, function(sender)
		touchcallback1(sender)
	end)
	gt.addBtnPressedListener(Btn_piao3, function(sender)
		touchcallback1(sender)
	end)
	gt.addBtnPressedListener(Btn_piao4, function(sender)
		touchcallback1(sender)
	end)
	gt.addBtnPressedListener(Btn_piao5, function(sender)
		touchcallback1(sender)
	end)
	gt.addBtnPressedListener(Btn_bupiao, function()
		touchcallback2()
	end)
	-- Btn_piao1:addTouchEventListener(touchcallback1)
	-- Btn_piao2:addTouchEventListener(touchcallback1)
	-- Btn_piao3:addTouchEventListener(touchcallback1)
	-- Btn_piao4:addTouchEventListener(touchcallback1)
	-- Btn_piao5:addTouchEventListener(touchcallback1)
	-- Btn_bupiao:addTouchEventListener(touchcallback2)

	if self.IsPiaoNum == 0 then
		self.piaoNode:setVisible(false)
	elseif self.IsPiaoNum == 3 then
		Btn_piao4:setVisible(false)
		Btn_piao5:setVisible(false)
		Btn_bupiao:setPosition(Btn_piao4:getPosition())
	elseif self.IsPiaoNum == 4 then
		Btn_piao5:setVisible(false)
		Btn_bupiao:setPosition(Btn_piao5:getPosition())
	elseif self.IsPiaoNum == 5 then
	end
end

function PlaySceneNC:showHuPaiType( seatIdx ,color, number , mjhuPaiType, isboolBreak, rcvWinCard )
	...
	if color ~= 0 and number ~= 0 then
		if mjhuPaiType then
			...
			if seatIdx ~= self.playerSeatIdx then
				-- 别人胡牌  把胡的牌亮出来
				if roomPlayer.mjTilesRemainCount ~= 0 then
					local vv = roomPlayer.holdMjTiles[roomPlayer.mjTilesRemainCount].mjTileSpr
					vv:setVisible(true)
					vv:setSpriteFrame(string.format(gt.MJSprFrameOut,roomPlayer.displaySeatIdx, color, number))
				else
					local vv = roomPlayer.holdMjTiles[roomPlayer.mjTilesRemainCount+1].mjTileSpr
					vv:setVisible(true)
					vv:setSpriteFrame(string.format(gt.MJSprFrameOut,roomPlayer.displaySeatIdx, color, number))
				end
			else
...
-- 确定可以换的花色
function PlaySceneNC:promptReplaceThreeCard()
...
function PlaySceneNC:exhReplaceThreeCard()
...
function PlaySceneNC:replaceThreeCardAction()
...
function PlaySceneNC:ThreeCardInsertMjType( mjTile )
...
function PlaySceneNC:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneNC:onRecUserReplaceCard( msgTbl )
	-- dump(msgTbl)
	gt.log("开始换三张 。。。。。。。。。。 ")
...
function PlaySceneNC:checkReplaceOkBtn()
	...
	if chooseType == true then
		gt.log("the checkReplaceOkBtn 1111111111 .......... ")
		-- 同花色  
		...
		gt.log("the checkReplaceOkBtn 222222222222 .......... ")
		Btn_Replace_ThreeCard:setEnabled(false)
	end
	
end

function PlaySceneNC:onRecUserReplaceCardComplate( msgTbl )
...
function PlaySceneNC:removeThreeCardFormPlayer()
...
function PlaySceneNC:showLastFourCard()
...
function PlaySceneNC:showLastEightCard()
	...
	self.Image_ReplaceNotice = Image_ReplaceNotice
	local showLabel = gt.seekNodeByName(Image_ReplaceNotice,"label_replace_show")
	showLabel:setString("最后12张,不允许摆")
