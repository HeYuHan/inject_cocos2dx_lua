...
local PlaySceneNeiJiang = class("PlaySceneNeiJiang", PlaySceneBase)

PlaySceneNeiJiang.DecisionType = {
    -- 接炮胡
    TAKE_CANNON_WIN                = 1,
    -- 自摸胡
    SELF_DRAWN_WIN                = 2,
...
PlaySceneNeiJiang.FLIMTYPE = {
    FLIMLAYER_BAR                = 1,
    FLIMLAYER_BU                = 2,
}

PlaySceneNeiJiang.TAG = {
    FLIMLAYER_BAR                = 50,
    FLIMLAYER_BU                = 51,
}

function PlaySceneNeiJiang:ctor(enterRoomMsgTbl)
    ...
    self.rootNode:reorderChild(_rootNodeList.Node_decisionBtn, PlaySceneNeiJiang.ZOrder.DECISION_BTN)
    self.rootNode:reorderChild(_rootNodeList.Node_selfDrawnDecision, PlaySceneNeiJiang.ZOrder.DECISION_BTN)
    self.rootNode:reorderChild(_rootNodeList.Node_chatBg, PlaySceneNeiJiang.ZOrder.CHAT)
    ...
    gt.addBtnPressedListener(settingBtn, function()
        ...
        self:addChild(settingPanel, PlaySceneNeiJiang.ZOrder.SETTING,102)
    end)
    ...
    gt.addBtnPressedListener(messageBtn, function()
        ...
        self:addChild(chatPanel, PlaySceneNeiJiang.ZOrder.CHAT, 101)
    end)

    -- 麻将层
    local playMjLayer = cc.Layer:create()
    self.rootNode:addChild(playMjLayer, PlaySceneNeiJiang.ZOrder.MJTILES)
    ...
    self.rootNode:addChild(outMjtileSignNode, PlaySceneNeiJiang.ZOrder.OUTMJTILE_SIGN)
    ...
    self:addChild(self.applyDimissRoom, PlaySceneNeiJiang.ZOrder.DISMISS_ROOM)
...
function PlaySceneNeiJiang:adjustUIforFixHeight()
...
function PlaySceneNeiJiang:adjustUIfor7Pai()
...
-- 初始化时隐藏节点
function PlaySceneNeiJiang:initHideNodes()
    self.nod_Piao:setVisible(false)
end

function PlaySceneNeiJiang:initYuyin()
    --语音提示
...
function PlaySceneNeiJiang:playYuYinAnimation()
    local action = nil
...
function PlaySceneNeiJiang:cancelYuYinAnimation()
...
function PlaySceneNeiJiang:registerAllMsgListener()
    -- 接收消息分发函数
...
function PlaySceneNeiJiang:unregisterAllMsgListener()
    ...
    gt.socketClient:unregisterMsgListener(self.CG_DING_PIAO)
    gt.socketClient:unregisterMsgListener(self.CG_DING_PIAO_COMPLATE)
end

function PlaySceneNeiJiang:initShowRemainTiles(msgTbl)
...
-- end --
function PlaySceneNeiJiang:onRcvLoginServer(msgTbl)
...
-- 断线重连,走一次登录流程
function PlaySceneNeiJiang:reLogin()
    print("========重连登录1")
...
function PlaySceneNeiJiang:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneNeiJiang:onRcvLoginGate(msgTbl)
...
function PlaySceneNeiJiang:onNodeEvent(eventName)
...
function PlaySceneNeiJiang:onRecUserRemoveBarCard( msgTbl )
...
function PlaySceneNeiJiang:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneNeiJiang:onEnterBackground()
...
function PlaySceneNeiJiang:onTouchCancelled(touch, event)
    self.isTouchBegan = false
end

function PlaySceneNeiJiang:onTouchBegan(touch, event)
...
function PlaySceneNeiJiang:onTouchMoved(touch, event)
...
function PlaySceneNeiJiang:onTouchEnded(touch, event)
    if self.isPlayThreeCardState == false then
        -- 不需要换三张
...
function PlaySceneNeiJiang:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneNeiJiang:update(delta)
...
function PlaySceneNeiJiang:checkVersion()
...
function PlaySceneNeiJiang:initPaoMaDeng()
    -- 跑马灯
...
function PlaySceneNeiJiang:onRcvMarquee(msgTbl)
    if gt.isIOSPlatform() and gt.isInReview then
        -- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneNeiJiang:initWifi()
...
function PlaySceneNeiJiang:updateWifi()
...
-- 检查手牌是否低于13张牌
function PlaySceneNeiJiang:checkRightCradNum()
    local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvEnterRoom(msgTbl)
    gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvAddPlayer(msgTbl)
    dump(msgTbl)
    -- 封装消息数据放入到房间玩家表中
    local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvRemovePlayer(msgTbl)
    gt.dump(msgTbl)
    local seatIdx = msgTbl.m_pos + 1
    local roomPlayer = self.roomPlayers[seatIdx]
    -- 隐藏玩家信息
...
-- @description 断线重连
-- end --
function PlaySceneNeiJiang:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneNeiJiang:CheckScore()
...
function PlaySceneNeiJiang:onRcvWinCard(msgTbl)
    gt.log("onRcvWinCard:================")
    dump(msgTbl)
    -- 同步断线后胡牌的状态
    self.ownerWin = false
...
-- 校验换三张
function PlaySceneNeiJiang:onRcvSyncReplaceCardType( )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvReady(msgTbl)
    gt.dump(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvOffLineState(msgTbl)
    local seatIdx = msgTbl.m_pos + 1
    local roomPlayer = self.roomPlayers[seatIdx]
    local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
    -- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvRoundState(msgTbl)
    -- 牌局状态,剩余牌
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:onRcvTurnShowMjTile(msgTbl)
    dump(msgTbl)
    gt.log("通知玩家出牌")
    -- 牌局状态,剩余牌
    self.lbl_remainTiles:setString(tostring(msgTbl.m_dCount))
                ...
                if nodeName == "Btn_decisionPass" then
                    ...
                    gt.addBtnPressedListener(decisionBtn, function()
                        local function passDecision()
                            ...
                            -- 删除弹出框（杠）
                            self:removeFlimLayer(PlaySceneNeiJiang.FLIMTYPE.FLIMLAYER_BAR)
                            -- 删除弹出框（补）
                            self:removeFlimLayer(PlaySceneNeiJiang.FLIMTYPE.FLIMLAYER_BU)
                ...
                if decisionData.flag == 3 or decisionData.flag == 4 then
                    if #cardList_bar == 1 then
                        ...
                        gt.addBtnPressedListener(decisionBtn, function(sender)
                            ...
                            self:removeFlimLayer(PlaySceneNeiJiang.FLIMTYPE.FLIMLAYER_BU)
                            -- add new
                            local flimLayer = self:createFlimLayer(PlaySceneNeiJiang.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
                            self:addChild(flimLayer,PlaySceneNeiJiang.ZOrder.FLIMLAYER,PlaySceneNeiJiang.TAG.FLIMLAYER_BAR)
                ...
                elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
                    if #cardList_bu == 1 then
                        ...
                        gt.addBtnPressedListener(decisionBtn, function(sender)
                            ...
                            self:removeFlimLayer(PlaySceneNeiJiang.FLIMTYPE.FLIMLAYER_BU)
                            -- add new
                            local flimLayer = self:createFlimLayer(PlaySceneNeiJiang.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
                            self:addChild(flimLayer,PlaySceneNeiJiang.ZOrder.FLIMLAYER,PlaySceneNeiJiang.TAG.FLIMLAYER_BU)
                            flimLayer:ignoreAnchorPointForPosition(false)
                            flimLayer:setAnchorPoint(0.5,0)
                            local pos_x = 0
                            if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
function PlaySceneNeiJiang:createMaskLayer(opacity)
    if not opacity then
        -- 用默认透明度
...
function PlaySceneNeiJiang:BtnTangCallBack(m_mjdata)
...
function  PlaySceneNeiJiang:updatePlayerInfo()
    for i = 1, 4 do
...
-- @description 广播玩家出牌
-- end --
function PlaySceneNeiJiang:onRcvSyncShowMjTile(msgTbl)
    ...
    if msgTbl.m_type == 2 then
        -- 自摸胡
        self:showDecisionAnimation(seatIdx, PlaySceneNeiJiang.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
    ...
    elseif msgTbl.m_type == 9 then
        -- 胡
        self:showDecisionAnimation(seatIdx, PlaySceneNeiJiang.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
    ...
    elseif msgTbl.m_type == 3 then
        ...
        if (next(msgTbl.m_think) ~= nil) then
            ...
            self:showDecisionAnimation(seatIdx, PlaySceneNeiJiang.DecisionType.DARK_BAR)
        end
    elseif msgTbl.m_type == 7 then
        ...
        if (next(msgTbl.m_think) ~= nil) then
            ...
            self:addMjTileBar(seatIdx, mj_color, mj_number, false)
            self:hideOtherPlayerMjTiles(seatIdx, true, false)
            self:showDecisionAnimation(seatIdx, PlaySceneNeiJiang.DecisionType.DARK_BU)
        end
    elseif msgTbl.m_type == 4 then
        ...
        if (next(msgTbl.m_think) ~= nil) then
            ...
            self:showDecisionAnimation(seatIdx, PlaySceneNeiJiang.DecisionType.BRIGHT_BAR)
        end
    elseif msgTbl.m_type == 8 then
        ...
        if (next(msgTbl.m_think) ~= nil) then
            ...
            local  mj_number = msgTbl.m_think[1][2]
            self:changePungToBrightBar(seatIdx, mj_color, mj_number)
            self:showDecisionAnimation(seatIdx, PlaySceneNeiJiang.DecisionType.BRIGHT_BU)
        end
    end
...
-- end --
function PlaySceneNeiJiang:onRcvSyncStartDecision(msgTbl)
    local seatIdx = msgTbl.m_pos + 1
...
-- end --
function PlaySceneNeiJiang:onRcvSyncBarTwoCard(msgTbl)
    -- set test data
    -- auto yang
    -- msgTbl = {}
    -- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneNeiJiang:showBarTwoCardAnimation(seatIdx,cardList)
    local roomPlayer = self.roomPlayers[seatIdx]
    ...
    self:addChild(image_bg,PlaySceneNeiJiang.ZOrder.HAIDILAOYUE)
    image_bg:setScale(0)
    -- 设置坐标位置
    local  m_curPos_x = 1
    local  m_curPos_y = 1
...
function PlaySceneNeiJiang:discardsOneCard(seatIdx,mjColor,mjNumber)
    local roomPlayer = self.roomPlayers[seatIdx]
    local mjTilesReferPos = roomPlayer.mjTilesReferPos
    local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneNeiJiang:showStartDecisionAnimation(seatIdx, decisionType, showCard)
    -- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
    ...
    self.rootNode:addChild(decisionSignSpr, PlaySceneNeiJiang.ZOrder.DECISION_SHOW)
    -- 标识显示动画
...
function PlaySceneNeiJiang:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneNeiJiang:onSendMSg66( cartType, cardarray )
    self.isPlayerDecision = false
    -- 隐藏决策按键
...
-- @description 通知玩家决策
-- end --
function PlaySceneNeiJiang:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneNeiJiang:onRcvSyncMakeDecision(msgTbl)
    ...
    if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
        ...
        if msgTbl.m_think[1] == 2 then
            ...
            self:showDecisionAnimation(seatIdx, PlaySceneNeiJiang.DecisionType.QIANG_GANG, msgTbl.m_hu)
            if #self.mjMarkTable ~= 0 then
                for _, mjTile in ipairs(self.mjMarkTable) do
                    if mjTile.mjTileSpr then
                        mjTile.mjTileSpr:setColor(cc.c3b(255,255,255))
    ...
    elseif msgTbl.m_think[1] == 5 then
        ...
        -- 碰牌动画
        self:showDecisionAnimation(seatIdx, PlaySceneNeiJiang.DecisionType.PUNG)
    ...
    elseif msgTbl.m_think[1] == 6 then
        ...
        -- 碰牌动画
        self:showDecisionAnimation(seatIdx, PlaySceneNeiJiang.DecisionType.EAT)
...
function PlaySceneNeiJiang:onRcvChatMsg(msgTbl)
...
--单局结束
function PlaySceneNeiJiang:onRcvRoundReport(msgTbl)
    gt.log("00000---")
    gt.dump(msgTbl)
    self.roundReportMsg = nil
...
--显示单局结算
function PlaySceneNeiJiang:showRoundReport(msgTbl)
	self.isBaoJiaoSelected = false
	self.isBaoJiaoState = false
    ...
    local callFunc = cc.CallFunc:create(function(sender)
        ...
        if self.hasPiao or self.hasBaoJiao then
            if msgTbl.m_end == 0 then -- 不是最后一局
                ...
                self:addChild(roundReport, PlaySceneNeiJiang.ZOrder.ROUND_REPORT)
            else
                gt.isZan = true
...
function PlaySceneNeiJiang:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneNeiJiang:onRcvFinalReport(msgTbl)
    self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneNeiJiang:showFinalReport(msgTbl)
    ...
    self:addChild(self.finalReport, PlaySceneNeiJiang.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneNeiJiang:updateCurrentTime()
...
function PlaySceneNeiJiang:checkPlayName( str )
    local retStr = ""
    local num = 0
    local lenInByte = #str
    local x = 1
...
-- end --
function PlaySceneNeiJiang:roomAddPlayer(roomPlayer)
    local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNeiJiang:playerEnterRoom(msgTbl)
...
-- end --
function PlaySceneNeiJiang:readyBtnClickEvt()
    local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
    readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneNeiJiang:playerGetReady(seatIdx)
    local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneNeiJiang:hidePlayersReadySign()
    for i = 1, 4 do
        local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneNeiJiang:showPlayerInfo(sender)
    ...
    self:addChild(playerInfoTips, PlaySceneNeiJiang.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneNeiJiang:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- end --
function PlaySceneNeiJiang:setTurnSeatSign(displaySeatIdx)
    -- 显示轮到的玩家座位标识
    local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
    -- 显示当先座位标识
...
-- @return
-- end --
function PlaySceneNeiJiang:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneNeiJiang:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneNeiJiang:addMjTileToPlayer(mjColor, mjNumber, replaceType)
...
-- end --
function PlaySceneNeiJiang:sortPlayerMjTiles( isthreeCard )
...
-- @return 选中的麻将牌
-- end --
function PlaySceneNeiJiang:touchPlayerMjTiles(touch)
    local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneNeiJiang:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
    -- 添加到已出牌列表
    local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneNeiJiang:checkOutMjTile( color, number )
    for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneNeiJiang:checkMjTile()
...
function PlaySceneNeiJiang:checkPengAndGang( isGang, color, number )
    local mjNumber = 0
    for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
        for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
            --检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneNeiJiang:removePreRoomPlayerOutMjTile(color,number)
    -- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneNeiJiang:showOutMjtileSign(seatIdx)
    local roomPlayer = self.roomPlayers[seatIdx]
    local endIdx = #roomPlayer.outMjTiles
    local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneNeiJiang:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
    if seatIdx == self.playerSeatIdx then
        return
    end
...
-- end --
function PlaySceneNeiJiang:addMjTilePung(seatIdx, mjColor, mjNumber)
    local roomPlayer = self.roomPlayers[seatIdx]
    local pungData = {}
...
-- end --
function PlaySceneNeiJiang:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
    local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneNeiJiang:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
    local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneNeiJiang:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
    -- dump(m_cardValue)
    local roomPlayer = self.roomPlayers[seatIdx]
    local mjTilesReferPos = roomPlayer.mjTilesReferPos
    -- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneNeiJiang:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
    local roomPlayer = self.roomPlayers[seatIdx]
    local mjTilesReferPos = roomPlayer.mjTilesReferPos
    -- 显示碰杠牌
    local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneNeiJiang:changePungToBrightBar(seatIdx, mjColor, mjNumber)
    local roomPlayer = self.roomPlayers[seatIdx]
    if seatIdx == self.playerSeatIdx then
        for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneNeiJiang:showDecisionAnimation(seatIdx, decisionType, huCard)
    local roomPlayer = self.roomPlayers[seatIdx]
    -- 四川麻将  杠就是刮风下雨
    if decisionType == PlaySceneNeiJiang.DecisionType.BRIGHT_BAR or
       decisionType == PlaySceneNeiJiang.DecisionType.BRIGHT_BU then
           -- 刮风
        ...
        self.rootNode:addChild(brightBarAnimateNode, PlaySceneNeiJiang.ZOrder.MJBAR_ANIMATION)
    ...
    elseif decisionType == PlaySceneNeiJiang.DecisionType.DARK_BAR or
           decisionType == PlaySceneNeiJiang.DecisionType.DARK_BU then
           -- 下雨
        local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
        local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
    ...
    elseif decisionType == PlaySceneNeiJiang.DecisionType.TAKE_CANNON_WIN or
           decisionType == PlaySceneNeiJiang.DecisionType.SELF_DRAWN_WIN or
           decisionType == PlaySceneNeiJiang.DecisionType.QIANG_GANG then
           -- 胡牌动画 现在只有一个胡的标志
           local decisionSignSpr = nil
           if decisionType == PlaySceneNeiJiang.DecisionType.QIANG_GANG then
        ...
        if decisionType == PlaySceneNeiJiang.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneNeiJiang.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneNeiJiang:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneNeiJiang:reset()
    -- 玩家手势隐藏
    self:hidePlayersReadySign()
...
function PlaySceneNeiJiang:backMainSceneEvt(eventType, isRoomCreater, roomID)
    -- 事件回调
    gt.removeTargetAllEventListener(self)
    -- 消息回调
    self:unregisterAllMsgListener()
...
function PlaySceneNeiJiang:createFlimLayer(flimLayerType,cardList)
    -- 一个麻将
    local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
    local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
    local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneNeiJiang:removeFlimLayer(flimLayerType)
    local child = self:getChildByTag(PlaySceneNeiJiang.TAG.FLIMLAYER_BAR)

    if flimLayerType == PlaySceneNeiJiang.FLIMTYPE.FLIMLAYER_BAR then
        child = self:getChildByTag(PlaySceneNeiJiang.TAG.FLIMLAYER_BAR)
    elseif flimLayerType == PlaySceneNeiJiang.FLIMTYPE.FLIMLAYER_BU then
        child = self:getChildByTag(PlaySceneNeiJiang.TAG.FLIMLAYER_BU)
    else
...
function PlaySceneNeiJiang:startAudio()
...
function PlaySceneNeiJiang:stopAudio()
    if gt.isUseNewMusic() == false then
        return
    end
...
function PlaySceneNeiJiang:cancelAudio()
    bridge.cancelVoice()
end

function PlaySceneNeiJiang:onRcvDingPiao(msgTbl)
    dump(msgTbl)
...
function PlaySceneNeiJiang:showDingPiao()
    local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
function PlaySceneNeiJiang:onRcvDingPiaoComplate(msgTbl)
	dump(msgTbl)
	self.nod_Piao:setVisible(false)
...
-- @param msgTbl
-- end --
function PlaySceneNeiJiang:onRecUserDingQue( msgTbl )
...
function PlaySceneNeiJiang:showDingQue()
  -- 提示玩家定缺
  -- 显示定缺
  self.dingqueColorState = false
...
-- 定缺完成
function PlaySceneNeiJiang:onRecUserDingQueComplate( msgTbl )
...
function PlaySceneNeiJiang:showDingQueType( seatIdx, color )
    local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneNeiJiang:dingquePrompt()
    local _result = {}
...
function PlaySceneNeiJiang:jianAnimate( node )
...
function PlaySceneNeiJiang:showHuPaiType( seatIdx ,color, number , mjhuPaiType, isboolBreak, rcvWinCard )
...
-- 确定可以换的花色
function PlaySceneNeiJiang:promptReplaceThreeCard()
...
function PlaySceneNeiJiang:exhReplaceThreeCard()
...
function PlaySceneNeiJiang:replaceThreeCardAction()
...
function PlaySceneNeiJiang:showFangjianYuAnimation(msgTbl, seatIdx)
    self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneNeiJiang:showPaiType(roundReportMsg, seatIdx)
    self.paiTypeActivity = require("app/views/PaiTypeActivity"):create(roundReportMsg, self.playerSeatIdx, self.curRoomPlayers)
    self:addChild(self.paiTypeActivity, PlaySceneNeiJiang.ZOrder.ROUND_REPORT_Activity)
end


function PlaySceneNeiJiang:ThreeCardInsertMjType( mjTile )
...
function PlaySceneNeiJiang:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneNeiJiang:onRecUserReplaceCard( msgTbl )
    -- dump(msgTbl)
    gt.log("开始换三张 。。。。。。。。。。 ")
...
function PlaySceneNeiJiang:checkReplaceOkBtn()
...
function PlaySceneNeiJiang:onRecUserReplaceCardComplate( msgTbl )
...
function PlaySceneNeiJiang:removeThreeCardFormPlayer()
...
function PlaySceneNeiJiang:onRecUserDBaoJiaoComplate( msgTbl )
...
-- 具体注释见自贡同名函数
function PlaySceneNeiJiang:onRecBaoJiao( msgTbl )
	gt.dump(msgTbl)
...
function PlaySceneNeiJiang:setAllMaskLayer(isMark, isAllMask)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
--全部屏蔽遮罩层true:遮挡false:不遮挡
function PlaySceneNeiJiang:setAllMaskLayer2(isMark, isAllMask)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
	for j, mjTile in ipairs(roomPlayer.holdMjTiles) do
		if isAllMask == 1 then
			mjTile.mjTileSpr:setColor(self.colorGray)
...
function PlaySceneNeiJiang:showBCDecisionAnimation(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneNeiJiang:checkOtherTingCards(roomPlayer, m_tingCards)
	local maskCardTbl = {}
...
function PlaySceneNeiJiang:setMySelfMaskLayer(roomPlayer, tingInfo)
	local num = #tingInfo
	gt.dump(tingInfo)
	if num < 1 then
		return
...
-- 是否是报叫开局的时候
function PlaySceneNeiJiang:isBaoJiaoFirstStart(m_dCount)
    local _cardCount = 36*self.fangCount - self.playerCount*self.mjCount - 1

    return m_dCount == _cardCount
end

function PlaySceneNeiJiang:getDisplaySeatIdxByMPos(m_pos)
...
function PlaySceneNeiJiang:showBaoziAnimation(msgTbl)
