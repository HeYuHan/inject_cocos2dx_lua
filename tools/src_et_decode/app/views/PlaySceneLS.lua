...
local PlaySceneLS = class("PlaySceneLS", function()	
	return cc.Scene:create()
end)

PlaySceneLS.DecisionType = {
	-- 接炮胡
	TAKE_CANNON_WIN				= 1,
	-- 自摸胡
	SELF_DRAWN_WIN				= 2,
...
function PlaySceneLS:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	local csbNode, animation = gt.createCSAnimation("PlaySceneLS.csb")
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneLS.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	self.selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(self.selfDrawnDcsNode, PlaySceneLS.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneLS.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneLS.ZOrder.SETTING,102)
	end)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneLS.ZOrder.CHAT, 101)
	end)

	-- 麻将层
	local playMjLayer = cc.Layer:create()
	self.rootNode:addChild(playMjLayer, PlaySceneLS.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneLS.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneLS.ZOrder.DISMISS_ROOM)
...
function PlaySceneLS:playYuYinAnimation()
	local action = nil
...
function PlaySceneLS:cancelYuYinAnimation()
...
function PlaySceneLS:getLuaBridge()
...
function PlaySceneLS:unregisterAllMsgListener()
...
-- end --
function PlaySceneLS:onRcvLoginServer(msgTbl)
...
function PlaySceneLS:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneLS:onRcvLoginGate(msgTbl)
...
function PlaySceneLS:onNodeEvent(eventName)
...
function PlaySceneLS:onRecUserRemoveBarCard( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneLS:onEnterBackground()
...
function PlaySceneLS:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneLS:onTouchBegan(touch, event)
	...
	if touchMjTile.mjColor == 3 and touchMjTile.mjNumber == 1 and self.IsYaojiState then
		return false
	end
	-- 判断选中麻将牌的花色 如果是定缺的花色 
...
function PlaySceneLS:onTouchMoved(touch, event)
...
function PlaySceneLS:onTouchEnded(touch, event)
...
function PlaySceneLS:outPaiBigAnimate(seatIdx, color, number ,isShow)
	--播放出牌音效
...
function PlaySceneLS:update(delta)
...
function PlaySceneLS:checkVersion()
...
function PlaySceneLS:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneLS:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneLS:updateWifi()
...
function PlaySceneLS:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneLS:checkRightCradNum()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvAddPlayer(msgTbl)
	dump(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneLS:onRcvSyncRoomState(msgTbl)
	...
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		...
		if roomPlayer.seatIdx == self.playerSeatIdx then
			...
			if msgTbl.m_myCard then
				...
				for _, mjTile in ipairs(roomPlayer.holdMjTiles) do
					...
					elseif mjTile.mjColor == 3 and mjTile.mjNumber == 1 and self.IsYaojiState then
		...
		local hdarkBarArray = msgTbl["m_ahCard" .. turnPos]
		if hdarkBarArray then
			local barTable = {}
			local group1 = {}
			local group2 = {}
			local group3 = {}
			...
			for i, v in ipairs(hdarkBarArray) do
				if i <= 4 then
					table.insert(group1,{v[1],v[2]})
					if i == 4 then
						table.insert(barTable,group1)
					end
				elseif i > 4 and i <= 8 then
					table.insert(group2,{v[1],v[2]})
					if i == 8 then
						table.insert(barTable,group2)
					end
				elseif i > 8 and i <= 12  then
					table.insert(group3,{v[1],v[2]})
					if i == 12 then
						table.insert(barTable,group3)
					end
				elseif i > 12 and i <= 16  then
					table.insert(group4,{v[1],v[2]})
					if i == 16 then
						table.insert(barTable,group4)
					end
				end
			end

			for j, barTile in pairs(barTable) do
				self:addMjTileBar(seatIdx, barTile[1][1], barTile[1][2], false, barTile)
			end
		end
		-- 明杠
		...
		local hbrightBarArray = msgTbl["m_mhCard" .. turnPos]
		if hbrightBarArray then
			local barTable = {}
			local group1 = {}
			local group2 = {}
			local group3 = {}
			...
			for i, v in ipairs(hbrightBarArray) do
				if i <= 4 then
					table.insert(group1,{v[1],v[2]})
					if i == 4 then
						table.insert(barTable,group1)
			...
			for j, barTile in pairs(barTable) do
				self:addMjTileBar(seatIdx, barTile[1][1], barTile[1][2], true, barTile)
			end
		...
		local hpungArray = msgTbl["m_phCard" .. turnPos]
		if hpungArray then
			local barTable = {}
			local group1 = {}
			local group2 = {}
			...
			for i, v in ipairs(hpungArray) do
				if i <= 3 then
					table.insert(group1,{v[1],v[2]})
					if i == 3 then
						table.insert(barTable,group1)
			...
			for j, barTile in pairs(barTable) do
				self:addMjTilePung(seatIdx, barTile[1][1], barTile[1][2], barTile)
			end
		end
		-- 躺的牌
...
--检测总分和是否为0
function PlaySceneLS:CheckScore()
...
--检验玩法
function PlaySceneLS:CheckPlayType(msgTbl)
...
function PlaySceneLS:onRcvWinCard(msgTbl)
	gt.log("onRcvWinCard:================")
	dump(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:onRcvTurnShowMjTile(msgTbl)
	dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
	...
	if seatIdx == self.playerSeatIdx then
		...
		gt.dump(roomPlayer.dingQueTable)
		local mjMarkTable = {}
		...
		if msgTbl.m_think then
			for _,value in ipairs(msgTbl.m_think) do
				...
				if think_m_type == 13 then
					--挑
					local decisionData = {}
					decisionData.flag = 13
					decisionData.cardList = {}
					for i,v in ipairs(think_m_cardList) do
						local card = {}
						card.mjColor = v[1]
						card.mjNumber = v[2]
						table.insert(decisionData.cardList,card)
		...
		if #decisionTypes > 0 or #msgTbl.m_ting > 0 then
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							self.selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneLS.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneLS.FLIMTYPE.FLIMLAYER_BU)
			...
			for idx, decisionData in ipairs(decisionTypes) do
				if decisionData.flag == 3 or decisionData.flag == 4 then
					...
					-- end
					table.insert(cardList_bar,decisionData)
				elseif decisionData.flag == 7 or decisionData.flag == 8 then
					...
					-- end
					table.insert(cardList_bu,decisionData)
				end
			end
			gt.log("杠的次数")
			gt.log(#cardList_bar)
			gt.dump(cardList_bar)
			gt.log("补的次数")
			gt.log(#cardList_bu)
			gt.dump(cardList_bu)
			...
			for idx, decisionData in ipairs(decisionTypes) do
				...
				elseif decisionData.flag == 3 or decisionData.flag == 4 then
					...
					if mjTileSpr then
						if #cardList_bar == 1 then
							mjTileSpr:setSpriteFrame(string.format(gt.SelfMJSprFrameOut, cardList_bar[1].cardList[1].mjColor, cardList_bar[1].cardList[1].mjNumber))
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then
					...
					if mjTileSpr then
						if #cardList_bu == 1 then
							mjTileSpr:setSpriteFrame(string.format(gt.SelfMJSprFrameOut, cardList_bu[1].cardList[1].mjColor, cardList_bu[1].cardList[1].mjNumber))
							mjTileSpr:setVisible(true)
						else
							mjTileSpr:setVisible(false)
						end
				...
				elseif decisionData.flag == 13 then
					--挑
					decisionBtn = gt.seekNodeByName(self.selfDrawnDcsNode, "Btn_decision_tiao")
					table.insert(btn_presentList,{3,decisionBtn})
					-- 显示杠胡牌
					local mjTileSpr = gt.seekNodeByName(decisionBtn, "Spr_mjTile")
					if mjTileSpr then
						mjTileSpr:setSpriteFrame(string.format(gt.SelfMJSprFrameOut, decisionData.cardList[1].mjColor, decisionData.cardList[1].mjNumber))
						mjTileSpr:setVisible(true)
					end
				end
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneLS.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneLS.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneLS.ZOrder.FLIMLAYER,PlaySceneLS.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneLS.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneLS.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneLS.ZOrder.FLIMLAYER,PlaySceneLS.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
				...
				elseif decisionData.flag == 13 then
					...
					gt.addBtnPressedListener(decisionBtn, function(sender)
						...
						msgToSend.m_msgId = gt.CG_SHOW_MJTILE
						msgToSend.m_type = decisionData.flag
						msgToSend.m_think = {{decisionData.cardList[1].mjColor,decisionData.cardList[1].mjNumber},{decisionData.cardList[2].mjColor,decisionData.cardList[2].mjNumber}}
						dump( msgToSend )
						gt.socketClient:sendMessage(msgToSend)
					end)
				else
...
function PlaySceneLS:checkOtherTingCards(roomPlayer, m_tingCards)
	local maskCardTbl = {}
...
--isOther：是否是自己
function PlaySceneLS:setMySelfMaskLayer(roomPlayer, tingInfo)
	local num = #tingInfo
	if num < 1 then
		return
	end
...
--tingInfo：要屏蔽的听的牌：
--isOther：是否是自己
function PlaySceneLS:setMaskLayer(roomPlayer, tingInfo, isOther)
...
function PlaySceneLS:setAllMaskLayer(isMark)
...
function PlaySceneLS:createMaskLayer(opacity)
	if not opacity then
		-- 用默认透明度
...
--躺成功
function PlaySceneLS:onRecErrorInfo(msgTbl)
	...
	if msgTbl.m_result == 0 then
		...
		self.rootNode:addChild(decisionSignSpr, PlaySceneLS.ZOrder.DECISION_SHOW)
...
function PlaySceneLS:setOtherMark()
...
function PlaySceneLS:getPlayerTings()
	local maskCardTbl = {}
...
--移除手中狼起的牌
function PlaySceneLS:sendSelectedMj(seatIdx)
	-- 发送出牌消息
	local roomPlayer = self.roomPlayers[seatIdx]
	if roomPlayer.seatIdx == self.playerSeatIdx then
		self.isPlayerShow = false
...
-- @return
-- end --
function PlaySceneLS:langQReorderMjTiles(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
function  PlaySceneLS:updatePlayerInfo()
	for i = 1, 4 do
		local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. i)
...
-- @description 广播玩家出牌
-- end --
function PlaySceneLS:onRcvSyncShowMjTile(msgTbl)
	...
	if #self.mjMarkTable ~= 0 and not self.roomPlayers[self.playerSeatIdx].isLangSelectedState then
		...
		for _, mjTile in ipairs(self.mjMarkTable) do
			...
			if self.IsYaojiState and mjTile.mjColor == 3 and mjTile.mjNumber == 1 then
			else
	...
	if msgTbl.m_type == 2 then
		-- 自摸胡
		self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			local  mj_color = msgTbl.m_think[1][1]
			local  mj_number = msgTbl.m_think[1][2]
			self:addMjTileBar(seatIdx, mj_color, mj_number, false,msgTbl.m_think)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			if msgTbl.m_errorCode == -1 then
				gt.log("===m_errorCode==-1=")
			else
				self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.DARK_BAR)
			end
			if #msgTbl.m_think > 1 then
				for idx=1,#msgTbl.m_think do
					local num = #roomPlayer.holdMjTiles
					for i=num,1,-1 do
						if roomPlayer.holdMjTiles[i].mjColor == msgTbl.m_think[idx][1] and roomPlayer.holdMjTiles[i].mjNumber == msgTbl.m_think[idx][2] then
	...
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			if msgTbl.m_errorCode == -1 then
				...
				self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.DARK_BU)
			end
			-- gt.dump(roomPlayer.holdMjTiles)
			-- if #msgTbl.m_think > 1 then
			-- 	for idx=1,#msgTbl.m_think do
	...
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
				...
				local mj_number1 = 0
				if #msgTbl.m_think > 1 then
					mj_color1 = msgTbl.m_think[2][1]
					mj_number1 = msgTbl.m_think[2][2]
				end

				self:changePungToBrightBar(seatIdx, mj_color, mj_number, mj_color1, mj_number1)
				if msgTbl.m_errorCode == -1 then
					gt.log("===m_errorCode==-1=")
				else
					self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.BRIGHT_BAR)
				end
				if #msgTbl.m_think > 1 then
					for idx=1,#msgTbl.m_think do
						local num = #roomPlayer.holdMjTiles
	...
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			if msgTbl.m_errorCode == -1 then
				...
				self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.BRIGHT_BU)
			end
	...
	elseif msgTbl.m_type == 13 then
		--挑
		gt.log("	挑 	 ")
		if (next(msgTbl.m_think) ~= nil) then
			--todo
			self:changeTiao(seatIdx,msgTbl.m_think)
			self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.TIAO)
		end
	end
...
-- 挑
function PlaySceneLS:changeTiao( seatIdx, mThink )
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	for i,BarData in ipairs(roomPlayer.mjTileBrightBars) do
		if tonumber(BarData.mjColor) == tonumber(mjColor) and tonumber(BarData.mjNumber) == tonumber(mjNumber) then
			if #BarData.hmjList > 1 then
				local title = BarData.groupNode:getChildByTag(4)
				local tag = 4
                -- if not title then -- 带混的碰转杠后没有Tag=4的节点，
                --     for j, child in ipairs(BarData.groupNode:getChildren()) do
                --         -- 查找幺鸡牌
                --         if child.mjTileData and child.mjTileData[1] == 3 and child.mjTileData[2] == 1 then
                --             title = child
                --             tag = j
                --             break
                --         end
                --     end
                -- end
                for j,v in ipairs(BarData.hmjList) do
                	if v.mjColor == mThink[1][1] and v.mjNumber == mThink[1][2] then
                		title = BarData.groupNode:getChildByTag(j)
                		tag = j
                		break
                	end
                end

				local mjTileName = string.format(gt.MJSprFrameOut, roomPlayer.displaySeatIdx, tonumber(mjColor), tonumber(mjNumber))
				gt.log(mjTileName)
				title:setSpriteFrame(mjTileName)
				title:setColor(cc.c3b(255,255,255))
				BarData.hmjList[tag].mjColor = mjColor
				BarData.hmjList[tag].mjNumber = mjNumber
			end
		end
	end
	...
	-- 	end
	-- end
	--mjTilesReferPos.groupStartPos = cc.pSub(mjTilesReferPos.groupStartPos, mjTilesReferPos.groupSpace)
	--mjTilesReferPos.holdStart = cc.pSub(mjTilesReferPos.holdStart, mjTilesReferPos.groupSpace)
...
-- end --
function PlaySceneLS:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneLS.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneLS.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneLS.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneLS.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneLS:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneLS:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneLS.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneLS:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneLS:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneLS.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneLS.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneLS.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneLS.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
function PlaySceneLS:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneLS:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
...
-- @description 通知玩家决策
-- end --
function PlaySceneLS:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneLS:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.QIANG_GANG, msgTbl.m_hu)
	...
	elseif msgTbl.m_think[1] == 3 or  msgTbl.m_think[1] == 4 then
		local mThink = nil
		if #msgTbl.m_think > 1 then
			if #msgTbl.m_think[2] > 1 then
				mThink = msgTbl.m_think[2]
			end
		end
		-- 明杠
		self:addMjTileBar(seatIdx, msgTbl.m_color, msgTbl.m_number, true,mThink)
		-- 杠牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.BRIGHT_BAR)
		...
		-- 移除上家打出的牌
		self:removePreRoomPlayerOutMjTile(msgTbl.m_color, msgTbl.m_number)
		-- if #msgTbl.m_think > 1 and mThink ~= nil then
		-- 	local roomPlayer = self.roomPlayers[seatIdx]
		-- 	for idx=1,#mThink do
		-- 		local num = #roomPlayer.holdMjTiles
		-- 		for i=num,1,-1 do
		-- 			if roomPlayer.holdMjTiles[i].mjColor == mThink[idx][1] and roomPlayer.holdMjTiles[i].mjNumber == mThink[idx][2] then
		-- 				gt.log("mjTile.mjColor:"..roomPlayer.holdMjTiles[i].mjColor)
		-- 				gt.log("mjTile.mjNumber:"..roomPlayer.holdMjTiles[i].mjNumber)
		-- 				roomPlayer.holdMjTiles[i].mjTileSpr:removeFromParent()
		-- 				table.remove(roomPlayer.holdMjTiles,i)
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌
		self:addMjTilePung(seatIdx, msgTbl.m_color, msgTbl.m_number, mThink)
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneLS.DecisionType.EAT)
...
function PlaySceneLS:onRcvChatMsg(msgTbl)
...
--单局结束
function PlaySceneLS:onRcvRoundReport(msgTbl)
	self.roundReportMsg = nil
	gt.log("00000---")
	gt.dump(msgTbl)
...
function PlaySceneLS:showRoundReport(msgTbl)
	--删除聊天框
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			...
			self:addChild(roundReport, PlaySceneLS.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
...
function PlaySceneLS:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneLS:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneLS:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneLS.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneLS:updateCurrentTime()
...
function PlaySceneLS:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneLS:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLS:playerEnterRoom(msgTbl)
...
-- end --
function PlaySceneLS:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneLS:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneLS:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneLS:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneLS.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneLS:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneLS:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
	-- 显示当先座位标识
...
-- @return
-- end --
function PlaySceneLS:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneLS:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneLS:addMjTileToPlayer(mjColor, mjNumber, replaceType)
...
-- end --
function PlaySceneLS:sortPlayerMjTiles( isthreeCard )
  ...
  if self.IsYaojiState then
    for i=#dingqueTable, 1, -1 do
      if dingqueTable[i].mjColor == 3 and dingqueTable[i].mjNumber == 1 then
        table.remove(dingqueTable, i)
      end
    end
  end

	roomPlayer.dingQueTable = dingqueTable
	gt.dump(dingqueTable)
...
-- @return 选中的麻将牌
-- end --
function PlaySceneLS:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneLS:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
...
function PlaySceneLS:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneLS:checkMjTile()
...
function PlaySceneLS:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneLS:removePreRoomPlayerOutMjTile(color,number)
	-- 移除上家打出的牌
	if self.preShowSeatIdx then
		gt.log("self.preShowSeatIdx = true")
		...
		gt.log("self.preShowSeatIdx = false")
		gt.socketClient:reloginServer()
	end
end
...
-- @param seatIdx 座次
-- end --
function PlaySceneLS:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneLS:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneLS:addMjTilePung(seatIdx, mjColor, mjNumber, mthink)
	gt.log("function is addMjTilePung")
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
	if mthink and #mthink > 1 then
		-- 加入到列表中
		...
		for i=1,3 do
			pungData.hmjList[i] = {}
			pungData.hmjList[i].mjColor = mthink[i][1]
			pungData.hmjList[i].mjNumber = mthink[i][2]
		end
		table.insert(roomPlayer.mjTilePungs, pungData)
	else
		pungData.mjColor = mjColor
	...
	pungData.groupNode = self:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, false, false, mthink)
end
...
-- end --
function PlaySceneLS:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar, mthink)
	local roomPlayer = self.roomPlayers[seatIdx]
	local barData = {}
	if mthink and #mthink > 1 then
		-- 加入到列表中
		...
		for i=1,4 do
			barData.hmjList[i] = {}
			barData.hmjList[i].mjColor = mthink[i][1]
			barData.hmjList[i].mjNumber = mthink[i][2]
		end
		if isBrightBar then
			-- 明杠
	...
	barData.groupNode = self:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, true, isBrightBar, mthink)
end
...
-- end --
function PlaySceneLS:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneLS:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneLS:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar, mthink )
	--gt.log("-------bbbb----")
	gt.dump( mthink )
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	...
	for i = 1, mjTilesCount do
		local mjTileName = nil
        local mjTileData = nil
		if isBar and not isBrightBar and i <= 3 then
			-- 暗杠前三张牌扣着
			if mthink and #mthink > 1 and i == 1 then
				mjTileName = string.format(gt.MJSprFrameOut, roomPlayer.displaySeatIdx, mthink[i][1], mthink[i][2])
			else
		...
		mjTileSpr:setTag(i)
        mjTileSpr.mjTileData = mjTileData
		groupNode:addChild(mjTileSpr)
	end
	...
	if seatIdx == self.playerSeatIdx then
		...
		if type(mjNumber) == "number" then
			if not self.pung then
				if mthink and #mthink > 1 then
					for j,v in ipairs(mthink) do
						for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
							if mjTile.mjNumber == mthink[j][2] and  mjTile.mjColor == mthink[j][1] then
								mjTile.mjTileSpr:removeFromParent()
								table.remove(roomPlayer.holdMjTiles, i)
								break
							end
...
-- @param mjNumber
-- end --
function PlaySceneLS:changePungToBrightBar(seatIdx, mjColor, mjNumber, mjColor1, mjNumber1)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
			if mjTile.mjColor == mjColor and mjTile.mjNumber == mjNumber then
	...
	if mjColor1 ~= 0 and mjNumber1 ~= 0 then
		for i, pungData in ipairs(roomPlayer.mjTilePungs) do
			if pungData.mjColor == mjColor1 and pungData.mjNumber == mjNumber1 then

				gt.dump(pungData)
				gt.dump(pungData.hmjList)
				if pungData.hmjList then
					hmjList = pungData.hmjList
				end
				gt.log("====f====" .. pungData.mjColor .. pungData.mjNumber)
				-- 从碰牌列表中删除
		...
		for i, pungData in ipairs(roomPlayer.mjTilePungs) do
			if pungData.mjColor == mjColor and pungData.mjNumber == mjNumber then
				...
				gt.dump(pungData)
				gt.log("====f=fdff===" .. pungData.mjColor)
				brightBarData = pungData
				table.remove(roomPlayer.mjTilePungs, i)
				break
			end
	...
	if brightBarData then
		...
		brightBarData.hmjList = {}
		for i=1,4 do
			brightBarData.hmjList[i] = {}
			if i == 4 then
				brightBarData.hmjList[i].mjColor = mjColor
				brightBarData.hmjList[i].mjNumber = mjNumber
			else
				if hmjList and #hmjList > 0 then
					brightBarData.hmjList[i].mjColor = hmjList[i].mjColor
					brightBarData.hmjList[i].mjNumber = hmjList[i].mjNumber
				else
					if mjColor1 ~= 0 and mjNumber1 ~= 0 then
						brightBarData.hmjList[i].mjColor = mjColor1
						brightBarData.hmjList[i].mjNumber = mjNumber1
					else
						brightBarData.hmjList[i].mjColor = mjColor
						brightBarData.hmjList[i].mjNumber = mjNumber
					end
...
-- @param decisionType 决策类型
-- end --
function PlaySceneLS:showDecisionAnimation(seatIdx, decisionType, huCard)
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneLS.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneLS.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneLS.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneLS.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneLS.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneLS.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneLS.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneLS.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneLS.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneLS.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneLS.DecisionType.QIANG_GANG then
			gt.soundManager:PlaySpeakSound(roomPlayer.sex, "hu", roomPlayer)
		else
			gt.soundManager:PlaySpeakSound(roomPlayer.sex, "zimo", roomPlayer)
		end
	elseif decisionType == PlaySceneLS.DecisionType.TIAO then
	   	-- 挑
...
-- @param seatIdx 座次
-- end --
function PlaySceneLS:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneLS:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneLS:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
	...
	local width
	if #cardList < 5 then
		width = 30+mjTileSpr:getContentSize().width*4*(#cardList)+space_gang*(#cardList-1)
	else
		width = 30+mjTileSpr:getContentSize().width*16+space_gang*3
	end

	
	local height = 24+mjTileSpr:getContentSize().height + ((#cardList)%4) * mjTileSpr:getContentSize().height
	...
	for idx,value in ipairs(cardList) do
		...
		local cardTab = value.cardList
		if #cardTab == 1 then
			local mjColor = cardTab[1].mjColor
			local mjNumber = cardTab[1].mjNumber
			...
			for i=1,4 do
	    		...
	    		if idx > 8 then
	    			index = idx - 8
	    			button:setPosition(cc.p(15+space_gang*(index-1)+width_oneMJ*(i-1)+width_oneMJ*4*(index-1), 180))
	    		elseif idx > 4 then
	    			index = idx - 4
	    			button:setPosition(cc.p(15+space_gang*(index-1)+width_oneMJ*(i-1)+width_oneMJ*4*(index-1), 95))
	    		else
	    		...
	    		local function touchEvent(ref, type)
	       			if type == ccui.TouchEventType.ended then
						...
						msgToSend.m_think = {}
						local think_temp = {cardData.cardList[1].mjColor,cardData.cardList[1].mjNumber}
						table.insert(msgToSend.m_think,think_temp)
						dump(msgToSend)
						gt.socketClient:sendMessage(msgToSend)
			...
			for i=1,#cardTab do
				local mjSprName = string.format(gt.SelfMJSprFrameOut, cardTab[i].mjColor, cardTab[i].mjNumber)
				local button = ccui.Button:create()
				button:loadTextures(mjSprName,mjSprName,"",ccui.TextureResType.plistType)
				button:setTouchEnabled(true)
	    		button:setAnchorPoint(cc.p(0,0))
...
function PlaySceneLS:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneLS.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneLS.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneLS.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneLS.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneLS.TAG.FLIMLAYER_BU)
	else
...
function PlaySceneLS:startAudio()
...
function PlaySceneLS:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneLS:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
-- @param msgTbl
-- end --
function PlaySceneLS:onRecUserDingQue( msgTbl )
...
-- 定缺完成
function PlaySceneLS:onRecUserDingQueComplate( msgTbl )
	gt.dump(msgTbl)
	if self.dingqueInfo then
		self.dingqueInfo:setVisible(false)
	end
...
function PlaySceneLS:showDingQueType( seatIdx, color )
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneLS:dingquePrompt()
	...
	for k,v in pairs(roomPlayer.holdMjTiles) do
		...
		elseif v.mjColor == 3 then
			if self.IsYaojiState == true and v.mjNumber == 1 then
			else
			    -- 条	
...
function PlaySceneLS:jianAnimate( node )
...
function PlaySceneLS:showHuPaiType( seatIdx ,color, number , mjhuPaiType, isboolBreak, rcvWinCard )
		...
		if mjhuPaiType then
			...
			if seatIdx ~= self.playerSeatIdx then
				...
				if isboolBreak == true then--(断线重连)
					if roomPlayer.holdMjTiles[roomPlayer.mjTilesRemainCount+1].mjTileSpr then
						...
						if roomPlayer.displaySeatIdx == 1 then
					 		vv:setPosition(vv:getPositionX(),vv:getPositionY() + 30)
						end
					end
				else
				...
				if isboolBreak == true then--(断线重连)
					...
					self.ownerWin = false
					if roomPlayer.holdMjTiles[#roomPlayer.holdMjTiles].mjColor == color and roomPlayer.holdMjTiles[#roomPlayer.holdMjTiles].mjNumber == number then
					else
						-- 把胡的牌亮出来
						for i,mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- 确定可以换的花色
function PlaySceneLS:promptReplaceThreeCard()
...
function PlaySceneLS:exhReplaceThreeCard()
...
function PlaySceneLS:replaceThreeCardAction()
...
function PlaySceneLS:ThreeCardInsertMjType( mjTile )
...
function PlaySceneLS:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneLS:checkReplaceOkBtn()
...
function PlaySceneLS:removeThreeCardFormPlayer()
...
function PlaySceneLS:showFangjianYuAnimation(msgTbl, seatIdx)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneLS:showPaiType(roundReportMsg, seatIdx)
	if not self.paiTypeActivity then
		...
		self:addChild(self.paiTypeActivity, PlaySceneLS.ZOrder.ROUND_REPORT_Activity)
	end
end


function PlaySceneLS:showLastFourCard()
