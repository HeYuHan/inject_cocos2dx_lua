...
local PlaySceneZG = class("PlaySceneZG", function()
...
function PlaySceneZG:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	local csbNode, animation = gt.createCSAnimation("PlaySceneZG.csb")
	...
	--报叫
	self.baojiao = gt.seekNodeByName(csbNode,"Node_baojiao")
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneZG.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(selfDrawnDcsNode, PlaySceneZG.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneZG.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneZG.ZOrder.SETTING,102)
	end)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneZG.ZOrder.CHAT , 101)
	end)

	-- 麻将层
	local playMjLayer = cc.Layer:create()
	self.rootNode:addChild(playMjLayer, PlaySceneZG.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneZG.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneZG.ZOrder.DISMISS_ROOM)
...
function PlaySceneZG:playYuYinAnimation()
	local action = nil
...
function PlaySceneZG:cancelYuYinAnimation()
...
function PlaySceneZG:getLuaBridge()
...
function PlaySceneZG:unregisterAllMsgListener()
...
-- end --
function PlaySceneZG:onRcvLoginServer(msgTbl)
...
function PlaySceneZG:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneZG:onRcvLoginGate( msgTbl )
...
function PlaySceneZG:onNodeEvent(eventName)
...
function PlaySceneZG:onRecUserRemoveBarCard( msgTbl )
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneZG:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneZG:onEnterBackground()
...
function PlaySceneZG:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneZG:onTouchBegan(touch, event)
...
function PlaySceneZG:onTouchMoved(touch, event)
...
function PlaySceneZG:onTouchEnded(touch, event)
...
function PlaySceneZG:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneZG:update(delta)
...
function PlaySceneZG:checkVersion()
...
function PlaySceneZG:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneZG:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneZG:updateWifi()
...
function PlaySceneZG:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneZG:checkRightCradNum()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvAddPlayer(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	dump(msgTbl)
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneZG:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneZG:CheckScore()
...
function PlaySceneZG:onRcvWinCard(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:onRcvTurnShowMjTile(msgTbl)
	-- dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
	...
	if seatIdx == self.playerSeatIdx then
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							if (self.m_dCount == 32 and self.mjCount == 13) or (self.m_dCount == 50 and self.mjCount == 7) then
							...
							selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneZG.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneZG.FLIMTYPE.FLIMLAYER_BU)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneZG.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneZG.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneZG.ZOrder.FLIMLAYER,PlaySceneZG.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneZG.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneZG.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneZG.ZOrder.FLIMLAYER,PlaySceneZG.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
		...
		if self.isBaoJiaoState and ((self.m_dCount == 32 and self.mjCount == 13) or (self.m_dCount == 50 and self.mjCount == 7)) then
			
			if not self.isBaojiaoHu then
				local roomPlayer = self.roomPlayers[self.playerSeatIdx]
				self:checkOtherTingCards(roomPlayer, msgTbl.m_ting)
			end
		elseif self.isBaoJiaoState and ((self.m_dCount ~= 32 and self.mjCount == 13) or (self.m_dCount ~= 50 and self.mjCount == 7)) then
...
function PlaySceneZG:setAllMaskLayer(isMark, isAllMask)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
--全部屏蔽遮罩层true:遮挡false:不遮挡
function PlaySceneZG:setAllMaskLayer2(isMark, isAllMask)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
	for j, mjTile in ipairs(roomPlayer.holdMjTiles) do
		if isAllMask == 1 then
			mjTile.mjTileSpr:setColor(cc.c3b(100,100,100))
...
function PlaySceneZG:checkOtherTingCards(roomPlayer, m_tingCards)
	local maskCardTbl = {}
...
--isOther：是否是自己
function PlaySceneZG:setMySelfMaskLayer(roomPlayer, tingInfo)
	local num = #tingInfo
	gt.dump(tingInfo)
	if num < 1 then
		return
...
--报叫完成
function PlaySceneZG:onRecUserDBaoJiaoComplate( msgTbl )
	gt.log("==========bb==a=====")
	--报叫结束
...
-- @description 广播玩家出牌
-- end --
function PlaySceneZG:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		...
		self:showDecisionAnimation(seatIdx, PlaySceneZG.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneZG.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneZG.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneZG.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneZG.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneZG.DecisionType.BRIGHT_BU)
		end
	end
...
-- end --
function PlaySceneZG:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneZG.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneZG.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneZG.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneZG.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneZG:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneZG:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneZG.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneZG:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneZG:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneZG.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneZG.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneZG.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneZG.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneZG.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
-- end --
function PlaySceneZG:onRcvStartDecision(msgTbl)
...
-- 起手胡,过之外的按钮
function PlaySceneZG:createStartHuDecisionMenu()
...
function PlaySceneZG:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneZG:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
	local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
	decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneZG:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneZG:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneZG.DecisionType.QIANG_GANG, msgTbl.m_hu)
		end
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneZG.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneZG.DecisionType.EAT)
...
function PlaySceneZG:onRcvChatMsg(msgTbl)
...
function PlaySceneZG:showFangjianYuAnimation(msgTbl, seatIdx)
...
function PlaySceneZG:showPaiType(roundReportMsg, seatIdx)
	self.paiTypeActivity = require("app/views/PaiTypeActivity"):create(roundReportMsg, self.playerSeatIdx, self.curRoomPlayers)
	self:addChild(self.paiTypeActivity, PlaySceneZG.ZOrder.ROUND_REPORT_Activity)
end


--单局结束
function PlaySceneZG:onRcvRoundReport(msgTbl)
	gt.log("00000---")
	self.roundReportMsg = nil
	gt.dump(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
--显示单局结算
function PlaySceneZG:showRoundReport(msgTbl)
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			...
			self:addChild(roundReport, PlaySceneZG.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
...
function PlaySceneZG:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneZG:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneZG:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneZG.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneZG:updateCurrentTime()
...
function PlaySceneZG:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneZG:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZG:playerEnterRoom(msgTbl)
...
-- end --
function PlaySceneZG:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneZG:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneZG:hidePlayersReadySign()
	for i = 1, 3 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneZG:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneZG.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneZG:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneZG:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
...
-- @return
-- end --
function PlaySceneZG:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneZG:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneZG:addMjTileToPlayer(mjColor, mjNumber)
...
-- end --
function PlaySceneZG:sortPlayerMjTiles()
...
-- @return 选中的麻将牌
-- end --
function PlaySceneZG:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneZG:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneZG:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneZG:checkMjTile()
...
function PlaySceneZG:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneZG:removePreRoomPlayerOutMjTile(color, number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneZG:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneZG:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneZG:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneZG:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneZG:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneZG:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneZG:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneZG:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneZG:showDecisionAnimation(seatIdx, decisionType, huCard)

	
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneZG.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneZG.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneZG.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneZG.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneZG.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneZG.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneZG.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneZG.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneZG.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneZG.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneZG.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneZG:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneZG:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneZG:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneZG:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneZG.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneZG.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneZG.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneZG.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneZG.TAG.FLIMLAYER_BU)
	else
...
-- @param isShow 显示标志
-- end --
function PlaySceneZG:LaoYueNodeVisible(isShow)
	-- body
	local laoYueNode = gt.seekNodeByName(self.rootNode, "Node_Laoyue")
	if(isShow)then
		self.rootNode:reorderChild(laoYueNode, PlaySceneZG.ZOrder.HAIDILAOYUE)
...
function PlaySceneZG:startAudio()
...
function PlaySceneZG:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneZG:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
function  PlaySceneZG:updatePlayerInfo()
	for i = 1, 3 do
		local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. i)
...
function PlaySceneZG:showHuPaiType( seatIdx ,color, number , mjhuPaiType,isboolBreak, rcvWinCard)
...
function PlaySceneZG:onRecBaoJiao( msgTbl )
...
-- @param decisionType 决策类型
-- end --
function PlaySceneZG:showBCDecisionAnimation(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
   	-- 胡牌动画 
...
function PlaySceneZG:showLastFourCard()
