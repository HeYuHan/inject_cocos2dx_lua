...
local PlaySceneGA = class("PlaySceneGA", PlaySceneBase)

PlaySceneGA.DecisionType = {
    -- 接炮胡
    TAKE_CANNON_WIN                = 1,
    -- 自摸胡
    SELF_DRAWN_WIN                = 2,
...
function PlaySceneGA:ctor(enterRoomMsgTbl)
    gt.log("______________________")
    -- 注册节点事件
    self:registerScriptHandler(handler(self, self.onNodeEvent))

    -- 加载界面资源
    self.rootNode = gt.loadCSB("PlaySceneGA.csb", self)
    self.rootNode:runAction(cc.CSLoader:createTimeline("PlaySceneGA.csb"))

    -- gt.seekNodeByName(self.rootNode, "Text_TableType"):setPositionX(710)
    gt.seekNodeByName(self.rootNode, "Text_TableType"):setContentSize(cc.size(900,30))
    ...
    if gt.isInReview == false then
        ...
        gt.addBtnPressedListener(roomIDLabelButton, function()
            if gt.isCopyText() then
                local TypeStr,tableStr = gt.PalyTypeText(enterRoomMsgTbl.m_state,enterRoomMsgTbl.m_playtype, enterRoomMsgTbl.m_baseScore)
    ...
    -- 倒计时
    self.playTimeCDLabel = gt.seekNodeByName(_rootNodeList.Node_roundState, "Label_playTimeCD")
    self.playTimeCDLabel:setString("0")

    self.rootNode:reorderChild(_rootNodeList.Node_decisionBtn, PlaySceneGA.ZOrder.DECISION_BTN)
    self.rootNode:reorderChild(_rootNodeList.Node_selfDrawnDecision, PlaySceneGA.ZOrder.DECISION_BTN)
    self.rootNode:reorderChild(_rootNodeList.Node_chatBg, PlaySceneGA.ZOrder.CHAT)
    ...
    gt.addBtnPressedListener(settingBtn, function()
        ...
        self:addChild(settingPanel, PlaySceneGA.ZOrder.SETTING,102)
    end)
    ...
    gt.addBtnPressedListener(messageBtn, function()
        ...
        self:addChild(chatPanel, PlaySceneGA.ZOrder.CHAT, 101)
    end)

    -- 麻将层
    local playMjLayer = cc.Layer:create()
    self.rootNode:addChild(playMjLayer, PlaySceneGA.ZOrder.MJTILES)
    ...
    self.rootNode:addChild(outMjtileSignNode, PlaySceneGA.ZOrder.OUTMJTILE_SIGN)
    ...
    self:addChild(self.applyDimissRoom, PlaySceneGA.ZOrder.DISMISS_ROOM)
...
function PlaySceneGA:adjustUIforFixHeight()
    if display.autoscale == "FIXED_HEIGHT" then
        ...
        gt.seekNodeByName(self.rootNode, "Label_time"):setPositionY(802)
        gt.seekNodeByName(self.rootNode, "Label_roomtext"):setPositionY(802)
        gt.seekNodeByName(self.rootNode, "Label_roomID"):setPositionY(802)
        gt.seekNodeByName(self.rootNode, "Text_TableType_BG"):setPositionY(808)
        gt.seekNodeByName(self.rootNode, "Text_TableType"):setPositionY(807)
...
function PlaySceneGA:initYuyin()
    --语音提示
...
function PlaySceneGA:playYuYinAnimation()
    local action = nil
...
function PlaySceneGA:cancelYuYinAnimation()
...
function PlaySceneGA:getLuaBridge()
...
function PlaySceneGA:registerAllMsgListener()
    -- 接收消息分发函数
...
function PlaySceneGA:unregisterAllMsgListener()
...
-- end --
function PlaySceneGA:onRcvLoginServer(msgTbl)
...
function PlaySceneGA:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneGA:onRcvLoginGate(msgTbl)
...
function PlaySceneGA:onNodeEvent(eventName)
...
function PlaySceneGA:onRecUserRemoveBarCard( msgTbl )
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneGA:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneGA:onEnterBackground()
...
function PlaySceneGA:onTouchCancelled(touch, event)
    self.isTouchBegan = false
end

function PlaySceneGA:onTouchBegan(touch, event)
...
function PlaySceneGA:onTouchMoved(touch, event)
...
function PlaySceneGA:onTouchEnded(touch, event)
    if self.isPlayThreeCardState == false then
        -- 不需要换三张
...
function PlaySceneGA:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneGA:update(delta)
...
function PlaySceneGA:checkVersion()
...
function PlaySceneGA:initPaoMaDeng()
    -- 跑马灯
...
function PlaySceneGA:onRcvMarquee(msgTbl)
    if gt.isIOSPlatform() and gt.isInReview then
        -- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneGA:updateWifi()
...
function PlaySceneGA:getStaticMethod(methodName)
    local ok = ""
    local result = ""
    if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneGA:checkRightCradNum()
    local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvEnterRoom(msgTbl)
    gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvAddPlayer(msgTbl)
    dump(msgTbl)
    -- 封装消息数据放入到房间玩家表中
    local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvRemovePlayer(msgTbl)
    gt.dump(msgTbl)
    local seatIdx = msgTbl.m_pos + 1
    local roomPlayer = self.roomPlayers[seatIdx]
    -- 隐藏玩家信息
...
-- @description 断线重连
-- end --
function PlaySceneGA:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneGA:CheckScore()
...
function PlaySceneGA:onRcvWinCard(msgTbl)
    gt.log("onRcvWinCard:================")
    dump(msgTbl)
    -- 同步断线后胡牌的状态
    self.ownerWin = false
...
-- 校验换三张
function PlaySceneGA:onRcvSyncReplaceCardType( )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvReady(msgTbl)
    gt.dump(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvOffLineState(msgTbl)
    local seatIdx = msgTbl.m_pos + 1
    local roomPlayer = self.roomPlayers[seatIdx]
    local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
    -- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvRoundState(msgTbl)
    -- 牌局状态,剩余牌
    local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:onRcvTurnShowMjTile(msgTbl)
    dump(msgTbl)
    gt.log("通知玩家出牌")
    -- 牌局状态,剩余牌
    local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
                ...
                if nodeName == "Btn_decisionPass" then
                    ...
                    gt.addBtnPressedListener(decisionBtn, function()
                        local function passDecision()
                            ...
                            -- 删除弹出框（杠）
                            self:removeFlimLayer(PlaySceneGA.FLIMTYPE.FLIMLAYER_BAR)
                            -- 删除弹出框（补）
                            self:removeFlimLayer(PlaySceneGA.FLIMTYPE.FLIMLAYER_BU)
                ...
                if decisionData.flag == 3 or decisionData.flag == 4 then
                    if #cardList_bar == 1 then
                        ...
                        gt.addBtnPressedListener(decisionBtn, function(sender)
                            ...
                            self:removeFlimLayer(PlaySceneGA.FLIMTYPE.FLIMLAYER_BU)
                            -- add new
                            local flimLayer = self:createFlimLayer(PlaySceneGA.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
                            self:addChild(flimLayer,PlaySceneGA.ZOrder.FLIMLAYER,PlaySceneGA.TAG.FLIMLAYER_BAR)
                ...
                elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
                    if #cardList_bu == 1 then
                        ...
                        gt.addBtnPressedListener(decisionBtn, function(sender)
                            ...
                            self:removeFlimLayer(PlaySceneGA.FLIMTYPE.FLIMLAYER_BU)
                            -- add new
                            local flimLayer = self:createFlimLayer(PlaySceneGA.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
                            self:addChild(flimLayer,PlaySceneGA.ZOrder.FLIMLAYER,PlaySceneGA.TAG.FLIMLAYER_BU)
                            flimLayer:ignoreAnchorPointForPosition(false)
                            flimLayer:setAnchorPoint(0.5,0)
                            local pos_x = 0
                            if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
function PlaySceneGA:createMaskLayer(opacity)
    if not opacity then
        -- 用默认透明度
...
function PlaySceneGA:BtnTangCallBack(m_mjdata)
...
function  PlaySceneGA:updatePlayerInfo()
    for i = 1, 4 do
...
-- @description 广播玩家出牌
-- end --
function PlaySceneGA:onRcvSyncShowMjTile(msgTbl)
    ...
    if msgTbl.m_type == 2 then
        -- 自摸胡
        self:showDecisionAnimation(seatIdx, PlaySceneGA.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
    ...
    elseif msgTbl.m_type == 9 then
        -- 胡
        self:showDecisionAnimation(seatIdx, PlaySceneGA.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
    ...
    elseif msgTbl.m_type == 3 then
        ...
        if (next(msgTbl.m_think) ~= nil) then
            ...
            self:showDecisionAnimation(seatIdx, PlaySceneGA.DecisionType.DARK_BAR)
        end
    elseif msgTbl.m_type == 7 then
        ...
        if (next(msgTbl.m_think) ~= nil) then
            ...
            self:addMjTileBar(seatIdx, mj_color, mj_number, false)
            self:hideOtherPlayerMjTiles(seatIdx, true, false)
            self:showDecisionAnimation(seatIdx, PlaySceneGA.DecisionType.DARK_BU)
        end
    elseif msgTbl.m_type == 4 then
        ...
        if (next(msgTbl.m_think) ~= nil) then
            ...
            self:showDecisionAnimation(seatIdx, PlaySceneGA.DecisionType.BRIGHT_BAR)
        end
    elseif msgTbl.m_type == 8 then
        ...
        if (next(msgTbl.m_think) ~= nil) then
            ...
            local  mj_number = msgTbl.m_think[1][2]
            self:changePungToBrightBar(seatIdx, mj_color, mj_number)
            self:showDecisionAnimation(seatIdx, PlaySceneGA.DecisionType.BRIGHT_BU)
        end
    end
...
-- end --
function PlaySceneGA:onRcvSyncStartDecision(msgTbl)
    local seatIdx = msgTbl.m_pos + 1
...
-- end --
function PlaySceneGA:onRcvSyncBarTwoCard(msgTbl)
    -- set test data
    -- auto yang
    -- msgTbl = {}
    -- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneGA:showBarTwoCardAnimation(seatIdx,cardList)
    local roomPlayer = self.roomPlayers[seatIdx]
    ...
    self:addChild(image_bg,PlaySceneGA.ZOrder.HAIDILAOYUE)
    image_bg:setScale(0)
    -- 设置坐标位置
    local  m_curPos_x = 1
    local  m_curPos_y = 1
...
function PlaySceneGA:discardsOneCard(seatIdx,mjColor,mjNumber)
    local roomPlayer = self.roomPlayers[seatIdx]
    local mjTilesReferPos = roomPlayer.mjTilesReferPos
    local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneGA:showStartDecisionAnimation(seatIdx, decisionType, showCard)
    -- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
    ...
    self.rootNode:addChild(decisionSignSpr, PlaySceneGA.ZOrder.DECISION_SHOW)
    -- 标识显示动画
...
function PlaySceneGA:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneGA:onSendMSg66( cartType, cardarray )
    self.isPlayerDecision = false
    -- 隐藏决策按键
...
-- @description 通知玩家决策
-- end --
function PlaySceneGA:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneGA:onRcvSyncMakeDecision(msgTbl)
    ...
    if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
        ...
        if msgTbl.m_think[1] == 2 then
            ...
            self:showDecisionAnimation(seatIdx, PlaySceneGA.DecisionType.QIANG_GANG, msgTbl.m_hu)
            if #self.mjMarkTable ~= 0 then
                for _, mjTile in ipairs(self.mjMarkTable) do
                    if mjTile.mjTileSpr then
                        mjTile.mjTileSpr:setColor(cc.c3b(255,255,255))
    ...
    elseif msgTbl.m_think[1] == 5 then
        ...
        -- 碰牌动画
        self:showDecisionAnimation(seatIdx, PlaySceneGA.DecisionType.PUNG)
    ...
    elseif msgTbl.m_think[1] == 6 then
        ...
        -- 碰牌动画
        self:showDecisionAnimation(seatIdx, PlaySceneGA.DecisionType.EAT)
...
function PlaySceneGA:onRcvChatMsg(msgTbl)
...
--单局结束
function PlaySceneGA:onRcvRoundReport(msgTbl)
    gt.log("00000---")
    gt.dump(msgTbl)
    self.roundReportMsg = nil
...
--显示单局结算
function PlaySceneGA:showRoundReport(msgTbl)
    --删除聊天框
    ...
    local callFunc = cc.CallFunc:create(function(sender)
        ...
        if msgTbl.m_end == 0 then -- 不是最后一局
            ...
            self:addChild(roundReport, PlaySceneGA.ZOrder.ROUND_REPORT)
        else
            gt.isZan = true
...
function PlaySceneGA:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneGA:onRcvFinalReport(msgTbl)
    self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneGA:showFinalReport(msgTbl)
    ...
    self:addChild(self.finalReport, PlaySceneGA.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneGA:updateCurrentTime()
...
function PlaySceneGA:checkPlayName( str )
    local retStr = ""
    local num = 0
    local lenInByte = #str
    local x = 1
...
-- end --
function PlaySceneGA:roomAddPlayer(roomPlayer)
    local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneGA:playerEnterRoom(msgTbl)
    ...
    local TypeStr,tableStr = gt.PalyTypeText(msgTbl.m_state,msgTbl.m_playtype, msgTbl.m_baseScore)
...
-- end --
function PlaySceneGA:readyBtnClickEvt()
    local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
    readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneGA:playerGetReady(seatIdx)
    local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneGA:hidePlayersReadySign()
    for i = 1, 4 do
        local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneGA:showPlayerInfo(sender)
    ...
    self:addChild(playerInfoTips, PlaySceneGA.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneGA:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneGA:setTurnSeatSign(seatIdx)
    -- 显示轮到的玩家座位标识
    local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
    -- 显示当先座位标识
...
-- @return
-- end --
function PlaySceneGA:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneGA:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneGA:addMjTileToPlayer(mjColor, mjNumber, replaceType)
...
-- end --
function PlaySceneGA:sortPlayerMjTiles( isthreeCard )
...
-- @return 选中的麻将牌
-- end --
function PlaySceneGA:touchPlayerMjTiles(touch)
    local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneGA:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
    -- 添加到已出牌列表
    local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneGA:checkOutMjTile( color, number )
    for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneGA:checkMjTile()
...
function PlaySceneGA:checkPengAndGang( isGang, color, number )
    local mjNumber = 0
    for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
        for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
            --检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneGA:removePreRoomPlayerOutMjTile(color,number)
    -- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneGA:showOutMjtileSign(seatIdx)
    local roomPlayer = self.roomPlayers[seatIdx]
    local endIdx = #roomPlayer.outMjTiles
    local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneGA:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
    if seatIdx == self.playerSeatIdx then
        return
    end
...
-- end --
function PlaySceneGA:addMjTilePung(seatIdx, mjColor, mjNumber)
    local roomPlayer = self.roomPlayers[seatIdx]
    local pungData = {}
...
-- end --
function PlaySceneGA:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
    local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneGA:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
    local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneGA:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
    -- dump(m_cardValue)
    local roomPlayer = self.roomPlayers[seatIdx]
    local mjTilesReferPos = roomPlayer.mjTilesReferPos
    -- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneGA:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
    local roomPlayer = self.roomPlayers[seatIdx]
    local mjTilesReferPos = roomPlayer.mjTilesReferPos
    -- 显示碰杠牌
    local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
    ...
    for i = 1, mjTilesCount do
        ...
        if isBar and not isBrightBar and ((i <= 3) or (seatIdx ~= self.playerSeatIdx)) then
            -- 暗杠前三张牌扣着
...
-- @param mjNumber
-- end --
function PlaySceneGA:changePungToBrightBar(seatIdx, mjColor, mjNumber)
    local roomPlayer = self.roomPlayers[seatIdx]
    if seatIdx == self.playerSeatIdx then
        for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneGA:showDecisionAnimation(seatIdx, decisionType, huCard)
    local roomPlayer = self.roomPlayers[seatIdx]
    -- 四川麻将  杠就是刮风下雨
    if decisionType == PlaySceneGA.DecisionType.BRIGHT_BAR or
       decisionType == PlaySceneGA.DecisionType.BRIGHT_BU then
           -- 刮风
        ...
        self.rootNode:addChild(brightBarAnimateNode, PlaySceneGA.ZOrder.MJBAR_ANIMATION)
    ...
    elseif decisionType == PlaySceneGA.DecisionType.DARK_BAR or
           decisionType == PlaySceneGA.DecisionType.DARK_BU then
           -- 下雨
        local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
        local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
    ...
    elseif decisionType == PlaySceneGA.DecisionType.TAKE_CANNON_WIN or
           decisionType == PlaySceneGA.DecisionType.SELF_DRAWN_WIN or
           decisionType == PlaySceneGA.DecisionType.QIANG_GANG then
           -- 胡牌动画 现在只有一个胡的标志
           local decisionSignSpr = nil
           if decisionType == PlaySceneGA.DecisionType.QIANG_GANG then
        ...
        if decisionType == PlaySceneGA.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneGA.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneGA:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneGA:backMainSceneEvt(eventType, isRoomCreater, roomID)
    -- 事件回调
    gt.removeTargetAllEventListener(self)
    -- 消息回调
    self:unregisterAllMsgListener()
...
function PlaySceneGA:createFlimLayer(flimLayerType,cardList)
    -- 一个麻将
    local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
    local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
    local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneGA:removeFlimLayer(flimLayerType)
    local child = self:getChildByTag(PlaySceneGA.TAG.FLIMLAYER_BAR)

    if flimLayerType == PlaySceneGA.FLIMTYPE.FLIMLAYER_BAR then
        child = self:getChildByTag(PlaySceneGA.TAG.FLIMLAYER_BAR)
    elseif flimLayerType == PlaySceneGA.FLIMTYPE.FLIMLAYER_BU then
        child = self:getChildByTag(PlaySceneGA.TAG.FLIMLAYER_BU)
    else
...
function PlaySceneGA:startAudio()
...
function PlaySceneGA:stopAudio()
    if gt.isUseNewMusic() == false then
        return
    end
    --停止录音
...
function PlaySceneGA:cancelAudio()
    if gt.isIOSPlatform() then
...
-- @param msgTbl
-- end --
function PlaySceneGA:onRecUserDingQue( msgTbl )
    ...
    if not _currentPlayerMustChooseQue then
        for k,v in pairs(msgTbl.m_state) do
            if k == self.playerSeatIdx then
            else
                local roomPlayer = self.roomPlayers[k]
    ...
    if not _currentPlayerMustChooseQue and #dingqueInfoTab>0 then
...
function PlaySceneGA:showDingQue()
  -- 提示玩家定缺
  -- 显示定缺
  self.dingqueColorState = false
...
-- 定缺完成
function PlaySceneGA:onRecUserDingQueComplate( msgTbl )
...
function PlaySceneGA:showDingQueType( seatIdx, color )
    local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneGA:dingquePrompt()
    local _result = {}
...
function PlaySceneGA:jianAnimate( node )
...
function PlaySceneGA:showHuPaiType( seatIdx ,color, number , mjhuPaiType, isboolBreak, rcvWinCard )
...
-- 确定可以换的花色
function PlaySceneGA:promptReplaceThreeCard()
...
function PlaySceneGA:exhReplaceThreeCard()
...
function PlaySceneGA:replaceThreeCardAction()
...
function PlaySceneGA:showFangjianYuAnimation(msgTbl, seatIdx)
    self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneGA:showPaiType(roundReportMsg, seatIdx)
    self.paiTypeActivity = require("app/views/PaiTypeActivity"):create(roundReportMsg, self.playerSeatIdx, self.curRoomPlayers)
    self:addChild(self.paiTypeActivity, PlaySceneGA.ZOrder.ROUND_REPORT_Activity)
end


function PlaySceneGA:ThreeCardInsertMjType( mjTile )
...
function PlaySceneGA:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneGA:onRecUserReplaceCard( msgTbl )
    -- dump(msgTbl)
    gt.log("开始换三张 。。。。。。。。。。 ")
...
function PlaySceneGA:checkReplaceOkBtn()
...
function PlaySceneGA:onRecUserReplaceCardComplate( msgTbl )
...
function PlaySceneGA:removeThreeCardFormPlayer()
...
function PlaySceneGA:showLastFourCard()
