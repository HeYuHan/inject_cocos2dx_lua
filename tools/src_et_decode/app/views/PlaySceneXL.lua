...
local PlaySceneXL = class("PlaySceneXL", function()
...
function PlaySceneXL:ctor(enterRoomMsgTbl)
	...
	-- if enterRoomMsgTbl.m_state == 3 then
	local csbNode, animation = gt.createCSAnimation("PlaySceneXL.csb")
	-- gt.seekNodeByName(csbNode, "Text_TableType"):setPositionX(700)
	...
	--胡牌框根节点
	local Node_HuGroup_2 = gt.seekNodeByName(self.rootNode, "Node_HuGroup_2")
	Node_HuGroup_2:setPosition(cc.p(Node_HuGroup_2:getPositionX(),Node_HuGroup_2:getPositionY()-20))
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneXL.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(selfDrawnDcsNode, PlaySceneXL.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneXL.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneXL.ZOrder.SETTING,102)
	end)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneXL.ZOrder.CHAT , 101)
	end)

	-- 麻将层
	local playMjLayer = cc.Layer:create()
	self.rootNode:addChild(playMjLayer, PlaySceneXL.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneXL.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneXL.ZOrder.DISMISS_ROOM)
...
function PlaySceneXL:playYuYinAnimation()
	local action = nil
...
function PlaySceneXL:cancelYuYinAnimation()
...
function PlaySceneXL:getLuaBridge()
...
function PlaySceneXL:unregisterAllMsgListener()
...
-- end --
function PlaySceneXL:onRcvLoginServer(msgTbl)
...
function PlaySceneXL:onRecUserRemoveBarCard( msgTbl )
...
function PlaySceneXL:onRcvLogin(msgTbl)
	-- print("========重连登录4")
	-- 去掉转圈
	-- gt.removeLoadingTips()
...
--服务器返回gate登录
function PlaySceneXL:onRcvLoginGate( msgTbl )
...
function PlaySceneXL:onNodeEvent(eventName)
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneXL:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneXL:onEnterBackground()
...
function PlaySceneXL:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneXL:onTouchBegan(touch, event)
...
function PlaySceneXL:onTouchMoved(touch, event)
...
function PlaySceneXL:onTouchEnded(touch, event)
...
function PlaySceneXL:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneXL:update(delta)
...
function PlaySceneXL:checkVersion()
...
function PlaySceneXL:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneXL:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneXL:updateWifi()
...
function PlaySceneXL:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneXL:checkRightCradNum()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvAddPlayer(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneXL:onRcvSyncRoomState(msgTbl)
	...
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		...
		--已胡牌
		local huMjTilesAry = msgTbl["m_hCard" .. turnPos]
		if huMjTilesAry and #huMjTilesAry > 0 then
			for i, v in ipairs(huMjTilesAry) do
				self:addAlreadyHuMjTiles(seatIdx, v[1], v[2], i)
				--校验胡牌
			end
...
--检测总分和是否为0
function PlaySceneXL:CheckScore()
...
-- 校验换三张
function PlaySceneXL:onRcvSyncReplaceCardType( )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:onRcvTurnShowMjTile(msgTbl)
	dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneXL.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneXL.FLIMTYPE.FLIMLAYER_BU)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneXL.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneXL.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneXL.ZOrder.FLIMLAYER,PlaySceneXL.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneXL.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneXL.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneXL.ZOrder.FLIMLAYER,PlaySceneXL.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
-- @description 广播玩家出牌
-- end --
function PlaySceneXL:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		...
		self:showDecisionAnimation(seatIdx, PlaySceneXL.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)

		if msgTbl.m_rewardCardNum and tonumber(msgTbl.m_rewardCardNum) > 0 then
			self:showFangjianYuAnimation(msgTbl, seatIdx)
		end

		self:showHuPaiType(seatIdx,msgTbl.m_color, msgTbl.m_number,true)

	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneXL.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)

		if msgTbl.m_rewardCardNum and tonumber(msgTbl.m_rewardCardNum) > 0 then
			self:showFangjianYuAnimation(msgTbl, seatIdx)
		end

		self:showHuPaiType(seatIdx,msgTbl.m_color, msgTbl.m_number,false)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneXL.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneXL.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneXL.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneXL.DecisionType.BRIGHT_BU)
		end
	end
end
...
-- end --
function PlaySceneXL:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneXL.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneXL.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneXL.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneXL.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneXL:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneXL:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneXL.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneXL:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneXL:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneXL.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneXL.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneXL.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneXL.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneXL.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
-- end --
function PlaySceneXL:onRcvStartDecision(msgTbl)
...
-- 起手胡,过之外的按钮
function PlaySceneXL:createStartHuDecisionMenu()
...
function PlaySceneXL:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneXL:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
	local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
	decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneXL:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneXL:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneXL.DecisionType.QIANG_GANG, msgTbl.m_hu)
			if #self.mjMarkTable ~= 0 then
				for _, mjTile in ipairs(self.mjMarkTable) do
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneXL.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneXL.DecisionType.EAT)
...
function PlaySceneXL:onRcvChatMsg(msgTbl)
...
function PlaySceneXL:onRcvRoundXueliu(msgTbl)
	if msgTbl~=nil then
		--处理血流结算
		local playerScore = {}
		for i=1,4 do
...
function PlaySceneXL:showFangjianYuAnimation(msgTbl, seatIdx)
...
function PlaySceneXL:showPaiType(roundReportMsg, seatIdx)
	if not self.paiTypeActivity then
		...
		self:addChild(self.paiTypeActivity, PlaySceneXL.ZOrder.ROUND_REPORT_Activity)
	end
end


--单局结束
function PlaySceneXL:onRcvRoundReport(msgTbl)
	self.roundReportMsg = nil
	gt.log("00000---")
	gt.dump(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
--显示单局结算
function PlaySceneXL:showRoundReport(msgTbl)
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			local roundReport = require("app/views/RoundReport"):create(self.roomPlayers, self.playerSeatIdx, msgTbl, msgTbl.m_end,self.playerScore)
			self:addChild(roundReport, PlaySceneXL.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
			local roundReport = require("app/views/RoundReport"):create(self.curRoomPlayers, self.playerSeatIdx, msgTbl, msgTbl.m_end,self.playerScore)
			self:addChild(roundReport, PlaySceneXL.ZOrder.ROUND_REPORT)
		end
...
function PlaySceneXL:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneXL:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneXL:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneXL.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneXL:updateCurrentTime()
...
function PlaySceneXL:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneXL:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXL:playerEnterRoom(msgTbl)
	-- 房间中的玩家
	self.roomPlayers = {}
	-- 玩家自己放入到房间玩家中
	local roomPlayer = {}
	...
	if roomPlayer.readyState == 0 then
		...
		for i = 1, 4 do
			...
			for j = 1, 24 do
				local Spr_mjTileHu_1 = gt.seekNodeByName(Node_HuGroup, "Spr_mjTileHu_" .. j)
				Spr_mjTileHu_1:removeAllChildren()
				Spr_mjTileHu_1:setVisible(false)
			end
		end
	end
...
-- end --
function PlaySceneXL:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneXL:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneXL:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneXL:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneXL.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneXL:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneXL:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
...
-- @return
-- end --
function PlaySceneXL:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneXL:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneXL:addMjTileToPlayer(mjColor, mjNumber, replaceType)
...
-- end --
function PlaySceneXL:sortPlayerMjTiles( isthreeCard )
...
-- @return 选中的麻将牌
-- end --
function PlaySceneXL:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneXL:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneXL:addAlreadyHuMjTiles(seatIdx, mjColor, mjNumber, index)
	...
	--  	mjTileSpr:setVisible( false )
	-- end
	local Node_HuGroup = gt.seekNodeByName(self.rootNode, "Node_HuGroup_" .. roomPlayer.displaySeatIdx)
	local Spr_mjTileHu = gt.seekNodeByName(Node_HuGroup, "Spr_mjTileHu_" .. index)
	Spr_mjTileHu:setSpriteFrame(string.format(gt.MJSprFrameOut,roomPlayer.displaySeatIdx, mjColor, mjNumber))
	Spr_mjTileHu:setVisible(true)
end

--出牌检查 检查要出的牌在牌桌上是否有相同的
function PlaySceneXL:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneXL:checkMjTile()
...
function PlaySceneXL:checkPengAndGang( isGang, color, number )
	
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
			--gt.log("7766666665555==4444==" .. outMjTile.mjColor .. outMjTile.mjNumber)
			if tonumber(outMjTile.mjColor) == tonumber(color) and tonumber(outMjTile.mjNumber) == tonumber(number) then

				mjNumber = mjNumber + 1
				--gt.log("7766666665555==4444=333344=" .. mjNumber)
			end
		end
	end
...
-- end --
function PlaySceneXL:removePreRoomPlayerOutMjTile(color, number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneXL:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneXL:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneXL:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneXL:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneXL:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneXL:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneXL:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneXL:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
function  PlaySceneXL:updatePlayerInfo()
	for i = 1, 4 do
		local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. i)
		local scoreLabel = gt.seekNodeByName(playerInfoNode, "Label_score")
		for j = 1 , 4  do 
			if self.roomPlayers[j].displaySeatIdx == i then
				...
				if self.roomPlayers[j] and self.roomPlayers ~= nil and self.roomPlayers[j] ~= nil then
...
function PlaySceneXL:removeHuMj()
	for i = 1, 4 do
		local Node_HuGroup = gt.seekNodeByName(self.rootNode, "Node_HuGroup_" .. i)
		for m = 1, 3 do
			local Img_kuang = gt.seekNodeByName(Node_HuGroup, "Img_kuang_" .. m)
...
-- @param decisionType 决策类型
-- end --
function PlaySceneXL:showDecisionAnimation(seatIdx, decisionType, huCard)

	
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneXL.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneXL.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneXL.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneXL.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneXL.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneXL.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneXL.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneXL.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneXL.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneXL.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneXL.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneXL:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneXL:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneXL:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneXL:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneXL.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneXL.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneXL.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneXL.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneXL.TAG.FLIMLAYER_BU)
	else
...
-- @param isShow 显示标志
-- end --
function PlaySceneXL:LaoYueNodeVisible(isShow)
	-- body
	local laoYueNode = gt.seekNodeByName(self.rootNode, "Node_Laoyue")
	if(isShow)then
		self.rootNode:reorderChild(laoYueNode, PlaySceneXL.ZOrder.HAIDILAOYUE)
...
function PlaySceneXL:startAudio()
...
function PlaySceneXL:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneXL:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
-- @param msgTbl
-- end --
function PlaySceneXL:onRecUserDingQue( msgTbl )
...
-- 定缺完成
function PlaySceneXL:onRecUserDingQueComplate( msgTbl )
...
function PlaySceneXL:showDingQueType( seatIdx, color )
...
function PlaySceneXL:dingquePrompt()
...
function PlaySceneXL:jianAnimate( node )
...
function PlaySceneXL:setHuTag(seatIdx)
	local hutypeNode = gt.seekNodeByName(self.rootNode,"Node_hutype")
	...
	gt.log("0-000----" .. #roomPlayer.huMjTiles)
...
function PlaySceneXL:showHuPaiType(seatIdx ,color, number , mjhuPaiType)
	...
	self:addAlreadyHuMjTiles(seatIdx, color, number, #roomPlayer.huMjTiles + 1)

	self:setHuTag(seatIdx)
end

-- 确定可以换的花色
function PlaySceneXL:promptReplaceThreeCard()
...
function PlaySceneXL:exhReplaceThreeCard()
...
function PlaySceneXL:replaceThreeCardAction()
...
function PlaySceneXL:ThreeCardInsertMjType( mjTile )
...
function PlaySceneXL:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneXL:onRecUserReplaceCard( msgTbl )
	dump(msgTbl)
	gt.log("开始换三张 。。。。。。。。。。 ")
...
function PlaySceneXL:checkReplaceOkBtn()
...
function PlaySceneXL:onRecUserReplaceCardComplate( msgTbl )
...
function PlaySceneXL:removeThreeCardFormPlayer()
...
function PlaySceneXL:IsSameIp()
	...
	-- 				if roomPlayer.ip ==  self.roomPlayers[4].ip then
	-- 					showStr = showStr .. "玩家：".. roomPlayer.nickname .. "与玩家："..self.roomPlayers[2].nickname .. "与玩家："..self.roomPlayers[3].nickname .. "与玩家："..self.roomPlayers[4].nickname .. "为同一IP \n"
	-- 				else
	-- 					showStr = showStr .. "玩家：".. roomPlayer.nickname .. "与玩家："..self.roomPlayers[2].nickname .. "与玩家："..self.roomPlayers[3].nickname .. "为同一IP \n"
	-- 				end
	-- 			elseif roomPlayer.ip ==  self.roomPlayers[4].ip then
	-- 				showStr = showStr .. "玩家：".. roomPlayer.nickname .. "与玩家："..self.roomPlayers[2].nickname .. "与玩家："..self.roomPlayers[4].nickname .."为同一IP \n"
	-- 			else
	...
	-- 			if roomPlayer.ip ==  self.roomPlayers[4].ip then
	-- 				showStr = showStr .. "玩家：".. roomPlayer.nickname .. "与玩家："..self.roomPlayers[3].nickname .. "与玩家："..self.roomPlayers[4].nickname .."为同一IP \n"
	-- 			else
	-- 				showStr = showStr .. "玩家：".. roomPlayer.nickname .. "与玩家："..self.roomPlayers[3].nickname .."为同一IP \n"
	-- 			end
	-- 		elseif roomPlayer.ip == self.roomPlayers[4].ip then
	-- 			showStr = showStr .. "玩家：".. roomPlayer.nickname .. "与玩家："..self.roomPlayers[4].nickname .."为同一IP \n"
	-- 		end
	-- 	end
	...
	-- 		end
	-- 	end
	-- 	if seatIdx == 3 and self.roomPlayers[3].ip ~= self.roomPlayers[1].ip and self.roomPlayers[3].ip ~= self.roomPlayers[2].ip then
	-- 		if roomPlayer.ip ==  self.roomPlayers[4].ip then
	-- 			showStr = showStr .. "玩家：".. roomPlayer.nickname .. "与玩家："..self.roomPlayers[4].nickname .. "为同一IP \n"
	-- 		end
	-- 	end
...
function PlaySceneXL:showLastFourCard()
