...
local PlayManagerSRSF = gt.PlayManager_Base:new()
gt.PlayManagerSRSF = PlayManagerSRSF

function PlayManagerSRSF:new(rootNode, paramTbl, o)
	o = o or {}
	setmetatable(o, self)
	self.__index = self
...
function PlayManagerSRSF:initUI(msgTbl)
	-- 隐藏玩家麻将参考位置
	...
	local mjCount = msgTbl.mjCount
	for i = 1, 3 do
		local node_playerMjTiles = gt.seekNodeByName(playNode, "Node_playerMjTiles_" .. i)
		local node_playerInfo = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. i)
		if i == 3 then
			node_playerInfo:setPosition(cc.p(node_playerInfo:getPositionX(), node_playerInfo:getPositionY() - 160 + mjCount * 10))
		end

		for _, node_playerMjTilesChild in ipairs(node_playerMjTiles:getChildren()) do
			if i == 1 then
				node_playerMjTilesChild:setPosition(cc.p(node_playerMjTilesChild:getPositionX(), node_playerMjTilesChild:getPositionY() + 260 - mjCount * 20))
			elseif i == 2 then
				node_playerMjTilesChild:setPosition(cc.p(node_playerMjTilesChild:getPositionX() , node_playerMjTilesChild:getPositionY() - 260 + mjCount * 20))
			elseif i == 3 then
				node_playerMjTilesChild:setPosition(cc.p(node_playerMjTilesChild:getPositionX() + 650 - mjCount * 50, node_playerMjTilesChild:getPositionY()))
				for j = 1, 3 do
	...
	local str = str1 .. str2 .. str3 .. str4 .. str5 .. str6 .. str7 .. str8 .. str9 .. str10
...
-- @param seatIdx 座位编号
-- end --
function PlayManagerSRSF:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
	-- 显示当先座位标识
	gt.log("=======00====" .. seatIdx)
...
-- end --
function PlayManagerSRSF:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
