...
local PlaySceneNJ = class("PlaySceneNJ", function()
...
function PlaySceneNJ:ctor(enterRoomMsgTbl)
	...
	local csbNode, animation = gt.createCSAnimation("PlaySceneNJ_TEN.csb")
	self.mjCount = 10
	for i=1,#enterRoomMsgTbl.m_playtype do
		...
		elseif enterRoomMsgTbl.m_playtype[i] == 36 then
			self.mjCount = 10
			csbNode, animation = gt.createCSAnimation("PlaySceneNJ_TEN.csb")
			break
		elseif enterRoomMsgTbl.m_playtype[i] == 37 then
			self.mjCount = 13
			csbNode, animation = gt.createCSAnimation("PlaySceneNJ_THR.csb")
			break
		end
	end
	...
	if display.autoscale == "FIXED_HEIGHT" then
		...
		-- gt.seekNodeByName(csbNode, "SpVoice"):setPosition(900,300)--语音
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneNJ.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(selfDrawnDcsNode, PlaySceneNJ.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneNJ.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneNJ.ZOrder.SETTING,102)
	end)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneNJ.ZOrder.CHAT, 101)
	end)

	-- 麻将层
	local playMjLayer = cc.Layer:create()
	self.rootNode:addChild(playMjLayer, PlaySceneNJ.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneNJ.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneNJ.ZOrder.DISMISS_ROOM)
...
function PlaySceneNJ:playYuYinAnimation()
	local action = nil
...
function PlaySceneNJ:cancelYuYinAnimation()
...
function PlaySceneNJ:getLuaBridge()
...
function PlaySceneNJ:unregisterAllMsgListener()
...
-- end --
function PlaySceneNJ:onRcvLoginServer(msgTbl)
...
function PlaySceneNJ:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneNJ:onRcvLoginGate( msgTbl )
...
function PlaySceneNJ:onNodeEvent(eventName)
...
function PlaySceneNJ:onRecUserRemoveBarCard( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneNJ:onEnterBackground()
...
function PlaySceneNJ:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneNJ:onTouchBegan(touch, event)
...
function PlaySceneNJ:onTouchMoved(touch, event)
...
function PlaySceneNJ:onTouchEnded(touch, event)
...
function PlaySceneNJ:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneNJ:update(delta)
...
function PlaySceneNJ:checkVersion()
...
function PlaySceneNJ:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneNJ:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneNJ:updateWifi()
...
function PlaySceneNJ:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneNJ:checkRightCradNum()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvAddPlayer(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneNJ:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneNJ:CheckScore()
...
function PlaySceneNJ:onRcvWinCard(msgTbl)
	gt.log("onRcvWinCard:================")
	dump(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:onRcvTurnShowMjTile(msgTbl)
	dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneNJ.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneNJ.FLIMTYPE.FLIMLAYER_BU)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneNJ.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneNJ.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneNJ.ZOrder.FLIMLAYER,PlaySceneNJ.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneNJ.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneNJ.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneNJ.ZOrder.FLIMLAYER,PlaySceneNJ.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
function  PlaySceneNJ:updatePlayerInfo()
	for i = 1, 4 do
...
-- @description 广播玩家出牌
-- end --
function PlaySceneNJ:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		-- 自摸胡
		self:showDecisionAnimation(seatIdx, PlaySceneNJ.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneNJ.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneNJ.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneNJ.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneNJ.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneNJ.DecisionType.BRIGHT_BU)
		end
	end
...
-- end --
-- function PlaySceneNJ:onRcvSyncStartDecision(msgTbl)
-- 	local seatIdx = msgTbl.m_pos + 1
-- 	if msgTbl.m_type == 1 then
-- 		-- 缺一色
-- 		self:showStartDecisionAnimation(seatIdx, PlaySceneNJ.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
-- 	elseif msgTbl.m_type == 2 then
-- 		-- 板板胡
-- 		self:showStartDecisionAnimation(seatIdx, PlaySceneNJ.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
-- 	elseif msgTbl.m_type == 3 then
-- 		-- 大四喜
-- 		self:showStartDecisionAnimation(seatIdx, PlaySceneNJ.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
-- 	elseif msgTbl.m_type == 4 then
-- 		-- 六六顺
-- 		self:showStartDecisionAnimation(seatIdx, PlaySceneNJ.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
-- 	end
-- end
...
-- end --
function PlaySceneNJ:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneNJ:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneNJ.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneNJ:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
-- function PlaySceneNJ:showStartDecisionAnimation(seatIdx, decisionType, showCard)
-- 	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
...
-- 	self.rootNode:addChild(decisionSignSpr, PlaySceneNJ.ZOrder.DECISION_SHOW)
-- 	-- 标识显示动画
...
-- 	-- 展示起手胡牌型
-- 	local copyNum = 1
-- 	if decisionType == PlaySceneNJ.StartDecisionType.TYPE_QUEYISE then
-- 		copyNum = 1
-- 	elseif decisionType == PlaySceneNJ.StartDecisionType.TYPE_BANBANHU then
-- 		copyNum = 1
-- 	elseif decisionType == PlaySceneNJ.StartDecisionType.TYPE_DASIXI then
-- 		copyNum = 4
-- 	elseif decisionType == PlaySceneNJ.StartDecisionType.TYPE_LIULIUSHUN then
-- 		copyNum = 3
-- 	end
...
-- end --
-- function PlaySceneNJ:onRcvStartDecision(msgTbl)
...
-- end
-- 起手胡,过之外的按钮
-- function PlaySceneNJ:createStartHuDecisionMenu()
...
-- function PlaySceneNJ:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
-- function PlaySceneNJ:onSendMSg66( cartType, cardarray )
-- 	self.isPlayerDecision = false
-- 	-- 隐藏决策按键
-- 	local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
-- 	decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneNJ:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneNJ:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneNJ.DecisionType.QIANG_GANG, msgTbl.m_hu)
			if #self.mjMarkTable ~= 0 then
				for _, mjTile in ipairs(self.mjMarkTable) do
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneNJ.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneNJ.DecisionType.EAT)
...
function PlaySceneNJ:onRcvChatMsg(msgTbl)
...
function PlaySceneNJ:showFangjianYuAnimation(msgTbl, seatIdx)
...
function PlaySceneNJ:showPaiType(roundReportMsg, seatIdx)
	if not self.paiTypeActivity then
		...
		self:addChild(self.paiTypeActivity, PlaySceneNJ.ZOrder.ROUND_REPORT_Activity)
	end
end


--单局结束
function PlaySceneNJ:onRcvRoundReport(msgTbl)
	self.roundReportMsg = nil
	gt.log("00000---")
	gt.dump(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
--显示单局结算
function PlaySceneNJ:showRoundReport(msgTbl)
	--删除聊天框
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			...
			self:addChild(roundReport, PlaySceneNJ.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
...
function PlaySceneNJ:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneNJ:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneNJ:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneNJ.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneNJ:updateCurrentTime()
...
function PlaySceneNJ:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneNJ:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneNJ:playerEnterRoom(msgTbl)
...
-- end --
function PlaySceneNJ:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneNJ:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneNJ:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneNJ:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneNJ.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneNJ:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneNJ:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
	-- 显示当先座位标识
...
-- @return
-- end --
function PlaySceneNJ:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneNJ:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneNJ:addMjTileToPlayer(mjColor, mjNumber, replaceType)
...
-- end --
function PlaySceneNJ:sortPlayerMjTiles()
	...
	if not self.ownerWin then
		for k, mjTile in ipairs(transMjTiles) do
			...
			-- if mjTile.mjTileSpr == newTile and k ~= num then
			-- 	local seqAction = cc.Sequence:create( cc.MoveTo:create(0.1, cc.p( newTileX, newTileY + 120 ) )
			-- 		, cc.DelayTime:create(0.2)
			-- 		, cc.EaseSineOut:create(cc.MoveTo:create(0.2, cc.p( mjTilePos.x, newTileY + 120 ) ))
			-- 		, cc.DelayTime:create(0.2)
			-- 		, cc.MoveTo:create(0.1, mjTilePos) )
			-- 	mjTile.mjTileSpr:runAction(seqAction)
			-- else
				mjTile.mjTileSpr:setPosition(mjTilePos)
			-- end
			self.playMjLayer:reorderChild(mjTile.mjTileSpr, (gt.winSize.height - mjTilePos.y))
			mjTilePos = cc.pAdd(mjTilePos, mjTilesReferPos.holdSpace)
			if k == self.mjCount then -- 如果手里有14张得话，那么说明是庄家
				mjTilePos = cc.pAdd(mjTilePos, cc.p(36, 0))
			end
		end
	end
...
-- @return 选中的麻将牌
-- end --
function PlaySceneNJ:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneNJ:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneNJ:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneNJ:checkMjTile()
...
function PlaySceneNJ:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneNJ:removePreRoomPlayerOutMjTile(color,number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneNJ:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneNJ:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneNJ:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneNJ:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneNJ:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneNJ:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneNJ:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneNJ:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneNJ:showDecisionAnimation(seatIdx, decisionType, huCard)

	
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneNJ.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneNJ.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneNJ.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneNJ.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneNJ.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneNJ.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneNJ.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneNJ.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneNJ.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneNJ.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneNJ.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneNJ:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneNJ:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneNJ:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneNJ:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneNJ.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneNJ.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneNJ.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneNJ.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneNJ.TAG.FLIMLAYER_BU)
	else
...
-- @param isShow 显示标志
-- end --
function PlaySceneNJ:LaoYueNodeVisible(isShow)
	-- body
	local laoYueNode = gt.seekNodeByName(self.rootNode, "Node_Laoyue")
	if(isShow)then
		self.rootNode:reorderChild(laoYueNode, PlaySceneNJ.ZOrder.HAIDILAOYUE)
...
function PlaySceneNJ:startAudio()
...
function PlaySceneNJ:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneNJ:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
function PlaySceneNJ:showHuPaiType( seatIdx ,color, number , mjhuPaiType,isboolBreak, rcvWinCard)
...
function PlaySceneNJ:exhReplaceThreeCard()
...
function PlaySceneNJ:showLastFourCard()
