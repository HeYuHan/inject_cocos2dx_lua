...
local PlaySceneTX = class("PlaySceneTX", function()
...
function PlaySceneTX:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	local csbNode, animation = gt.createCSAnimation("PlaySceneTX.csb")
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneTX.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(selfDrawnDcsNode, PlaySceneTX.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneTX.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneTX.ZOrder.SETTING,102)
	end)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneTX.ZOrder.CHAT , 101)
	end)

	-- 麻将层
	local playMjLayer = cc.Layer:create()
	self.rootNode:addChild(playMjLayer, PlaySceneTX.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneTX.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneTX.ZOrder.DISMISS_ROOM)
...
function PlaySceneTX:playYuYinAnimation()
	local action = nil
...
function PlaySceneTX:cancelYuYinAnimation()
...
function PlaySceneTX:getLuaBridge()
...
function PlaySceneTX:unregisterAllMsgListener()
...
-- end --
function PlaySceneTX:onRcvLoginServer(msgTbl)
...
function PlaySceneTX:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneTX:onRcvLoginGate( msgTbl )
...
function PlaySceneTX:onNodeEvent(eventName)
...
function PlaySceneTX:onRecUserRemoveBarCard( msgTbl )
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneTX:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneTX:onEnterBackground()
...
function PlaySceneTX:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneTX:onTouchBegan(touch, event)
...
function PlaySceneTX:onTouchMoved(touch, event)
...
function PlaySceneTX:onTouchEnded(touch, event)
...
function PlaySceneTX:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneTX:update(delta)
...
function PlaySceneTX:checkVersion()
...
function PlaySceneTX:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneTX:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneTX:updateWifi()
...
function PlaySceneTX:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneTX:checkRightCradNum()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvAddPlayer(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	dump(msgTbl)
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneTX:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneTX:CheckScore()
...
function PlaySceneTX:onRcvWinCard(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:onRcvTurnShowMjTile(msgTbl)
	gt.dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneTX.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneTX.FLIMTYPE.FLIMLAYER_BU)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneTX.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneTX.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneTX.ZOrder.FLIMLAYER,PlaySceneTX.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneTX.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneTX.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneTX.ZOrder.FLIMLAYER,PlaySceneTX.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
-- @description 广播玩家出牌
-- end --
function PlaySceneTX:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		...
		self:showDecisionAnimation(seatIdx, PlaySceneTX.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneTX.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneTX.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneTX.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneTX.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneTX.DecisionType.BRIGHT_BU)
		end
	end
...
-- end --
function PlaySceneTX:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneTX.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneTX.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneTX.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneTX.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneTX:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneTX:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneTX.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneTX:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneTX:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneTX.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneTX.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneTX.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneTX.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneTX.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
-- end --
function PlaySceneTX:onRcvStartDecision(msgTbl)
...
-- 起手胡,过之外的按钮
function PlaySceneTX:createStartHuDecisionMenu()
...
function PlaySceneTX:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneTX:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
	local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
	decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneTX:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneTX:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneTX.DecisionType.QIANG_GANG, msgTbl.m_hu)
		end
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneTX.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneTX.DecisionType.EAT)
...
function PlaySceneTX:onRcvChatMsg(msgTbl)
...
function PlaySceneTX:showFangjianYuAnimation(msgTbl, seatIdx)
...
function PlaySceneTX:showPaiType(roundReportMsg, seatIdx)
	self.paiTypeActivity = require("app/views/PaiTypeActivity"):create(roundReportMsg, self.playerSeatIdx, self.curRoomPlayers)
	self:addChild(self.paiTypeActivity, PlaySceneTX.ZOrder.ROUND_REPORT_Activity)
end


--单局结束
function PlaySceneTX:onRcvRoundReport(msgTbl)
	gt.log("00000---")
	self.roundReportMsg = nil
	gt.dump(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
--显示单局结算
function PlaySceneTX:showRoundReport(msgTbl)
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			...
			self:addChild(roundReport, PlaySceneTX.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
...
function PlaySceneTX:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneTX:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneTX:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneTX.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneTX:updateCurrentTime()
...
function PlaySceneTX:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneTX:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneTX:playerEnterRoom(msgTbl)
...
-- end --
function PlaySceneTX:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneTX:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneTX:hidePlayersReadySign()
	for i = 1, 3 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneTX:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneTX.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneTX:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneTX:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
...
-- @return
-- end --
function PlaySceneTX:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneTX:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneTX:addMjTileToPlayer(mjColor, mjNumber)
...
-- end --
function PlaySceneTX:sortPlayerMjTiles()
...
-- @return 选中的麻将牌
-- end --
function PlaySceneTX:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneTX:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneTX:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneTX:checkMjTile()
...
function PlaySceneTX:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneTX:removePreRoomPlayerOutMjTile(color, number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneTX:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneTX:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneTX:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneTX:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneTX:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneTX:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneTX:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneTX:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneTX:showDecisionAnimation(seatIdx, decisionType, huCard)

	
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneTX.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneTX.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneTX.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneTX.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneTX.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneTX.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneTX.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneTX.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneTX.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneTX.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneTX.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneTX:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneTX:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneTX:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneTX:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneTX.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneTX.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneTX.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneTX.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneTX.TAG.FLIMLAYER_BU)
	else
...
-- @param isShow 显示标志
-- end --
function PlaySceneTX:LaoYueNodeVisible(isShow)
	-- body
	local laoYueNode = gt.seekNodeByName(self.rootNode, "Node_Laoyue")
	if(isShow)then
		self.rootNode:reorderChild(laoYueNode, PlaySceneTX.ZOrder.HAIDILAOYUE)
...
function PlaySceneTX:startAudio()
...
function PlaySceneTX:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneTX:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
function  PlaySceneTX:updatePlayerInfo()
	for i = 1, 3 do
...
function PlaySceneTX:showHuPaiType( seatIdx ,color, number , mjhuPaiType,isboolBreak, rcvWinCard)
	...
	if color ~= 0 and number ~= 0 then
		...
		if seatIdx == self.playerSeatIdx then 
			...
			if isboolBreak == true then--(断线重连)
				...
				local vv = roomPlayer.holdMjTiles[roomPlayer.mjTilesRemainCount].mjTileSpr
				vv:setVisible(true)
				local _offsetMap = {{x=0, y=30}, {x=0, y=-40}}
...
function PlaySceneTX:showLastFourCard()
