...
local PlaySceneXZ = class("PlaySceneXZ", function()	
	return cc.Scene:create()
end)

PlaySceneXZ.DecisionType = {
	-- 接炮胡
	TAKE_CANNON_WIN				= 1,
	-- 自摸胡
	SELF_DRAWN_WIN				= 2,
...
function PlaySceneXZ:ctor(enterRoomMsgTbl)
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneXZ.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(selfDrawnDcsNode, PlaySceneXZ.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneXZ.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneXZ.ZOrder.SETTING,102)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneXZ.ZOrder.CHAT, 101)
	...
	self.rootNode:addChild(playMjLayer, PlaySceneXZ.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneXZ.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneXZ.ZOrder.DISMISS_ROOM)
...
function PlaySceneXZ:playYuYinAnimation()
	local action = nil
...
function PlaySceneXZ:cancelYuYinAnimation()
...
function PlaySceneXZ:getLuaBridge()
...
function PlaySceneXZ:unregisterAllMsgListener()
...
-- end --
function PlaySceneXZ:onRcvLoginServer(msgTbl)
...
function PlaySceneXZ:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneXZ:onRcvLoginGate(msgTbl)
...
function PlaySceneXZ:onNodeEvent(eventName)
...
function PlaySceneXZ:onRecUserRemoveBarCard( msgTbl )
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneXZ:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneXZ:onEnterBackground()
...
function PlaySceneXZ:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneXZ:onTouchBegan(touch, event)
...
function PlaySceneXZ:onTouchMoved(touch, event)
...
function PlaySceneXZ:onTouchEnded(touch, event)
	if self.isPlayThreeCardState == false then
		-- 不需要换三张 
...
function PlaySceneXZ:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneXZ:update(delta)
...
function PlaySceneXZ:checkVersion()
...
function PlaySceneXZ:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneXZ:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneXZ:updateWifi()
...
function PlaySceneXZ:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneXZ:checkRightCradNum()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvAddPlayer(msgTbl)
	dump(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvRemovePlayer(msgTbl)
	gt.dump(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
...
-- @description 断线重连
-- end --
function PlaySceneXZ:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneXZ:CheckScore()
...
function PlaySceneXZ:onRcvWinCard(msgTbl)
	gt.log("onRcvWinCard:================")
	dump(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
...
-- 校验换三张
function PlaySceneXZ:onRcvSyncReplaceCardType( )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvReady(msgTbl)
	gt.dump(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:onRcvTurnShowMjTile(msgTbl)
	dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneXZ.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneXZ.FLIMTYPE.FLIMLAYER_BU)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneXZ.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneXZ.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneXZ.ZOrder.FLIMLAYER,PlaySceneXZ.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneXZ.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneXZ.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneXZ.ZOrder.FLIMLAYER,PlaySceneXZ.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
function PlaySceneXZ:createMaskLayer(opacity)
	if not opacity then
		-- 用默认透明度
...
function PlaySceneXZ:BtnTangCallBack(m_mjdata)
...
function  PlaySceneXZ:updatePlayerInfo()
	for i = 1, 4 do
		local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. i)
...
-- @description 广播玩家出牌
-- end --
function PlaySceneXZ:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		-- 自摸胡
		self:showDecisionAnimation(seatIdx, PlaySceneXZ.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneXZ.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneXZ.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneXZ.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneXZ.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneXZ.DecisionType.BRIGHT_BU)
		end
	end
...
-- end --
function PlaySceneXZ:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneXZ.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneXZ.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneXZ.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneXZ.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneXZ:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneXZ:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneXZ.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneXZ:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneXZ:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneXZ.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneXZ.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneXZ.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneXZ.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneXZ.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
function PlaySceneXZ:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneXZ:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
	-- local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
	-- decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneXZ:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneXZ:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneXZ.DecisionType.QIANG_GANG, msgTbl.m_hu)
			if #self.mjMarkTable ~= 0 then
				for _, mjTile in ipairs(self.mjMarkTable) do
					if mjTile.mjTileSpr then
						mjTile.mjTileSpr:setColor(cc.c3b(255,255,255))
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneXZ.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneXZ.DecisionType.EAT)
...
function PlaySceneXZ:onRcvChatMsg(msgTbl)
...
--单局结束
function PlaySceneXZ:onRcvRoundReport(msgTbl)
	gt.log("00000---")
	gt.dump(msgTbl)
	self.roundReportMsg = nil
...
--显示单局结算
function PlaySceneXZ:showRoundReport(msgTbl)
	--删除聊天框
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			...
			self:addChild(roundReport, PlaySceneXZ.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
...
function PlaySceneXZ:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneXZ:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneXZ:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneXZ.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneXZ:updateCurrentTime()
...
function PlaySceneXZ:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneXZ:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneXZ:playerEnterRoom(msgTbl)
...
-- end --
function PlaySceneXZ:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneXZ:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneXZ:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneXZ:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneXZ.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneXZ:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneXZ:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
	-- 显示当先座位标识
...
-- @return
-- end --
function PlaySceneXZ:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneXZ:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneXZ:addMjTileToPlayer(mjColor, mjNumber, replaceType)
	--@hlx 玩家正常显示牌的精灵名称，只需要把其他玩家的牌换成这个就行
...
-- end --
function PlaySceneXZ:sortPlayerMjTiles( isthreeCard )
...
-- @return 选中的麻将牌
-- end --
function PlaySceneXZ:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneXZ:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneXZ:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneXZ:checkMjTile()
...
function PlaySceneXZ:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneXZ:removePreRoomPlayerOutMjTile(color,number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneXZ:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneXZ:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneXZ:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneXZ:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneXZ:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneXZ:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneXZ:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneXZ:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneXZ:showDecisionAnimation(seatIdx, decisionType, huCard)
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneXZ.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneXZ.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneXZ.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneXZ.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneXZ.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneXZ.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneXZ.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneXZ.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneXZ.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneXZ.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneXZ.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneXZ:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneXZ:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneXZ:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneXZ:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneXZ.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneXZ.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneXZ.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneXZ.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneXZ.TAG.FLIMLAYER_BU)
	else
...
function PlaySceneXZ:startAudio()
...
function PlaySceneXZ:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneXZ:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
-- @param msgTbl
-- end --
function PlaySceneXZ:onRecUserDingQue( msgTbl )
...
-- 定缺完成
function PlaySceneXZ:onRecUserDingQueComplate( msgTbl )
...
function PlaySceneXZ:showDingQueType( seatIdx, color )
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneXZ:dingquePrompt()
...
function PlaySceneXZ:jianAnimate( node )
...
function PlaySceneXZ:showHuPaiType( seatIdx ,color, number , mjhuPaiType, isboolBreak, rcvWinCard )
...
-- 确定可以换的花色
function PlaySceneXZ:promptReplaceThreeCard()
...
function PlaySceneXZ:exhReplaceThreeCard()
...
function PlaySceneXZ:replaceThreeCardAction()
...
function PlaySceneXZ:showFangjianYuAnimation(msgTbl, seatIdx)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneXZ:showPaiType(roundReportMsg, seatIdx)
	self.paiTypeActivity = require("app/views/PaiTypeActivity"):create(roundReportMsg, self.playerSeatIdx, self.curRoomPlayers)
	self:addChild(self.paiTypeActivity, PlaySceneXZ.ZOrder.ROUND_REPORT_Activity)
end


function PlaySceneXZ:ThreeCardInsertMjType( mjTile )
...
function PlaySceneXZ:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneXZ:onRecUserReplaceCard( msgTbl )
	-- dump(msgTbl)
	gt.log("开始换三张 。。。。。。。。。。 ")
...
function PlaySceneXZ:checkReplaceOkBtn()
...
function PlaySceneXZ:onRecUserReplaceCardComplate( msgTbl )
...
function PlaySceneXZ:removeThreeCardFormPlayer()
...
function PlaySceneXZ:showLastFourCard()
