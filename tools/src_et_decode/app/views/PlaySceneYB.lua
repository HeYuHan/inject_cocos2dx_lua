...
local PlaySceneYB = class("PlaySceneYB", function()	
	return cc.Scene:create()
end)

PlaySceneYB.DecisionType = {
	-- 接炮胡
	TAKE_CANNON_WIN				= 1,
	-- 自摸胡
	SELF_DRAWN_WIN				= 2,
...
function PlaySceneYB:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	local csbNode, animation = gt.createCSAnimation("PlaySceneYB.csb")
	...
	if display.autoscale == "FIXED_HEIGHT" then
		...
		gt.seekNodeByName(csbNode, "Text_TableType"):setPositionY(808)
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneYB.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(selfDrawnDcsNode, PlaySceneYB.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneYB.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneYB.ZOrder.SETTING,102)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneYB.ZOrder.CHAT, 101)
	...
	self.rootNode:addChild(playMjLayer, PlaySceneYB.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneYB.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneYB.ZOrder.DISMISS_ROOM)
...
function PlaySceneYB:playYuYinAnimation()
	local action = nil
...
function PlaySceneYB:cancelYuYinAnimation()
...
function PlaySceneYB:getLuaBridge()
...
function PlaySceneYB:unregisterAllMsgListener()
...
-- end --
function PlaySceneYB:onRcvLoginServer(msgTbl)
...
function PlaySceneYB:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneYB:onRcvLoginGate(msgTbl)
...
function PlaySceneYB:onNodeEvent(eventName)
...
function PlaySceneYB:onRecUserRemoveBarCard( msgTbl )
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneYB:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneYB:onEnterBackground()
...
function PlaySceneYB:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneYB:onTouchBegan(touch, event)
	...
	if self.isPlayThreeCardState == false then
		...
		for i,v in ipairs(self.WanNengPaiCarTable) do
			if touchMjTile.mjColor == v[1] and touchMjTile.mjNumber == v[2] then
				return false
			end
		end
...
function PlaySceneYB:onTouchMoved(touch, event)
...
function PlaySceneYB:onTouchEnded(touch, event)
	if self.isPlayThreeCardState == false then
		-- 不需要换三张 
...
function PlaySceneYB:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneYB:update(delta)
...
function PlaySceneYB:checkVersion()
...
function PlaySceneYB:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneYB:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneYB:updateWifi()
...
function PlaySceneYB:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneYB:checkRightCradNum()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvAddPlayer(msgTbl)
	dump(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneYB:onRcvSyncRoomState(msgTbl)
	...
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		...
		local fhArray = msgTbl["m_fhCard" .. turnPos]
		if fhArray then
			local barTable = {}
			local group1 = {}
			local group2 = {}
			...
			for j, barTile in pairs(barTable) do
				local feiData = {}
				feiData.mjColor = barTile[2][1]
				feiData.mjNumber = barTile[2][2]

				local feiGroup = {}
				table.insert(feiGroup,{barTile[2][1], barTile[2][2]})
				table.insert(feiGroup,{barTile[1][1], barTile[1][2]})
				table.insert(feiGroup,{barTile[2][1], barTile[2][2]})
				...
				for i = 1, 3 do
					local mjTileName = string.format(gt.MJSprFrameOut, roomPlayer.displaySeatIdx, tonumber(feiGroup[i][1]), tonumber(feiGroup[i][2]))
					local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
...
--检测总分和是否为0
function PlaySceneYB:CheckScore()
...
function PlaySceneYB:onRcvWinCard(msgTbl)
	gt.log("onRcvWinCard:================")
	dump(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
...
function PlaySceneYB:addFanPiGuCard( msgTbl )
	if msgTbl.m_hunCard then
		local h = 580
		if display.autoscale == "FIXED_HEIGHT" then
			h = 720
		end
		for i, v in ipairs(msgTbl.m_hunCard) do
			-- self:addMjTileToPlayer(v[1], v[2])
			if v[1] ~= 0 and v[2] ~= 0 then
				local mjTileName = string.format(gt.MJSprFrameOut, self.playerFixDispSeat, v[1], v[2])
				local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
				if i == 2 then
					mjTileSpr:setPosition( cc.p(55 * 3, h) )
					mjTileSpr:setColor(cc.c3b(255,255,100))
					table.insert( self.WanNengPaiCarTable, v )
				elseif i == 3 then
					mjTileSpr:setPosition( cc.p(55 * 2, h) )
				else
					mjTileSpr:setPosition( cc.p(55, h) )
					mjTileSpr:setColor(cc.c3b(255,255,100))
					table.insert( self.WanNengPaiCarTable, v )
				end
...
-- 校验换三张
function PlaySceneYB:onRcvSyncReplaceCardType( )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:onRcvTurnShowMjTile(msgTbl)
	dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
		...
		if #decisionTypes > 0 or #msgTbl.m_ting > 0 then
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneYB.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneYB.FLIMTYPE.FLIMLAYER_BU)
			...
			for idx, decisionData in ipairs(decisionTypes) do
				...
				elseif decisionData.flag == 13 then
					--提 
					decisionBtn = gt.seekNodeByName(selfDrawnDcsNode, "Btn_decision_ti")
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneYB.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneYB.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneYB.ZOrder.FLIMLAYER,PlaySceneYB.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneYB.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneYB.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneYB.ZOrder.FLIMLAYER,PlaySceneYB.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
			...
			-- if #msgTbl.m_ting > 0 then
			-- 	gt.log("展示躺决策+++++++++++++++++++")
			-- 	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
			-- 	local decisionBtn_Tang = gt.seekNodeByName(selfDrawnDcsNode, "Btn_decision_tang")
			-- 	--点完躺决策之后出现的返回按钮
			-- 	local Btn_decision_return = gt.seekNodeByName(self.rootNode, "Btn_decision_return")
			-- 	gt.addBtnPressedListener(Btn_decision_return, function (sender)
			-- 		local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
			-- 		selfDrawnDcsNode:setVisible(true)
			-- 		sender:setVisible(false)
			-- 		for _, mjTile in ipairs(roomPlayer.holdMjTiles) do
			...
			-- 	decisionBtn_Tang:setVisible(true)
			-- 	gt.addBtnPressedListener(decisionBtn_Tang, function (sender)
			-- 			local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
			-- 			selfDrawnDcsNode:setVisible(false)
			-- 			--点完躺决策之后出现的返回按钮
			-- 			local Btn_decision_return = gt.seekNodeByName(self.rootNode, "Btn_decision_return")
			...
			-- 			self.isPlayerDecision = false
			-- 			-- 遍历手牌，除 m_ting 里的其余手牌都隐藏
			-- 			local mjTangTable = {}
			-- 			for _, mjTile in ipairs(roomPlayer.holdMjTiles) do
			-- 				mjTile.mjTileSpr:setColor(cc.c3b(100,100,100))
			-- 				table.insert(mjTangTable,mjTile)
			-- 				for j,m_data in ipairs(msgTbl.m_ting) do
			-- 					if mjTile.mjColor == m_data[1][1] and mjTile.mjNumber == m_data[1][2] then
			-- 						mjTile.mjTileSpr:setColor(cc.c3b(255,255,255))
			-- 						table.remove(mjTangTable)
			-- 					end
			-- 				end
			-- 			end
			-- 			gt.dump(mjTangTable)
			-- 			self.mjTangTable = mjTangTable
			-- 			gt.log("隐藏的牌")
			-- 			self.IsShowTang = true
			-- 			self.m_tang = msgTbl.m_ting
			-- 			-- self:BtnTangCallBack(msgTbl.m_ting)
			-- 		 end)
			-- 	table.insert(btn_presentList,{1,decisionBtn_Tang})
			-- end
			-- 根据显示优先级进行排序
...
function PlaySceneYB:createMaskLayer(opacity)
	if not opacity then
		-- 用默认透明度
...
function PlaySceneYB:BtnTangCallBack(m_mjdata)
...
function  PlaySceneYB:updatePlayerInfo()
	for i = 1, 4 do
		local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. i)
...
-- @description 广播玩家出牌
-- end --
function PlaySceneYB:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		-- 自摸胡
		self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.BRIGHT_BU)
		end
	elseif msgTbl.m_type == 13 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			--todo
			self:changeFeiToTi(seatIdx,msgTbl.m_think)
			self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.TI)
		end
	end
...
-- end --
function PlaySceneYB:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneYB.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneYB.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneYB.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneYB.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneYB:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneYB:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneYB.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneYB:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneYB:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneYB.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneYB.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneYB.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneYB.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneYB.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
function PlaySceneYB:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneYB:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
	-- local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
	-- decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneYB:onRcvMakeDecision(msgTbl)
...
--msgTbl.m_think = {{6, {{3,4},{3,5}}}, {6, {{3,7}, {3,8}}}, {6, {{3,5},{3,7}}}   }
	...
	if msgTbl.m_flag == 1 then
		...
		for i, v in ipairs(noSame) do
			...
			local x = startPosX - (posTag - i) * Btn_decision_0:getContentSize().width * 2
				...
				elseif decisionType == 12 then
					...
					for _, m in pairs(decisionTypes) do
						if m[1] == 12 then
							...
							msgToSend.m_msgId = gt.CG_PLAYER_DECISION
							msgToSend.m_type = decisionType
							msgToSend.m_think = {{m[2][1][1],m[2][1][2]},{m[2][2][1],m[2][2][2]}}
							gt.socketClient:sendMessage(msgToSend)
						end
					end
				else
...
-- @description 广播决策结果
-- end --
function PlaySceneYB:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.QIANG_GANG, msgTbl.m_hu)
			if #self.mjMarkTable ~= 0 then
				for _, mjTile in ipairs(self.mjMarkTable) do
					if mjTile.mjTileSpr then
						mjTile.mjTileSpr:setColor(cc.c3b(255,255,255))
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.EAT)
	...
	elseif msgTbl.m_think[1] == 7 or msgTbl.m_think[1] == 8 then
		...
		-- 移除上家打出的牌
		self:removePreRoomPlayerOutMjTile(msgTbl.m_color, msgTbl.m_number)
	elseif msgTbl.m_think[1] == 12 then
		--飞

		local feiData = {}
		feiData.mjColor = msgTbl.m_think[2][2][1]
		feiData.mjNumber = msgTbl.m_think[2][2][2]

		local feiGroup = {}
		table.insert(feiGroup,{msgTbl.m_think[2][2][1], msgTbl.m_think[2][2][2]})
		table.insert(feiGroup,{msgTbl.m_think[2][1][1], msgTbl.m_think[2][1][2]})
		table.insert(feiGroup,{msgTbl.m_think[2][2][1], msgTbl.m_think[2][2][2]})
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneYB.DecisionType.FEI)
		-- 隐藏持有牌中打出的牌
		self:hideOtherPlayerMjTiles(seatIdx, false)
		-- 移除上家打出的牌
		self:removePreRoomPlayerOutMjTile()
		...
		for idx=1,2 do
			...
			for i=num,1,-1 do
				if roomPlayer.holdMjTiles[i].mjColor == msgTbl.m_think[2][idx][1] and roomPlayer.holdMjTiles[i].mjNumber == msgTbl.m_think[2][idx][2] then
...
function PlaySceneYB:onRcvChatMsg(msgTbl)
...
function PlaySceneYB:showFangjianYuAnimation(msgTbl, seatIdx)
...
function PlaySceneYB:showPaiType(roundReportMsg, seatIdx)
	if not self.paiTypeActivity then
		...
		self:addChild(self.paiTypeActivity, PlaySceneYB.ZOrder.ROUND_REPORT_Activity)
	end
end

--单局结束
function PlaySceneYB:onRcvRoundReport(msgTbl)
	self.roundReportMsg = nil
	gt.log("00000---")
	gt.dump(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneYB:showRoundReport(msgTbl)
	--删除聊天框
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			if self.isPiaoType then
				local roundReport = require("app/views/RoundReport_YB"):create(self.roomPlayers, self.playerSeatIdx, msgTbl, msgTbl.m_end)
				self:addChild(roundReport, PlaySceneYB.ZOrder.ROUND_REPORT)
			else
			...
			gt.isZan = true
			if self.isPiaoType then
				local roundReport = require("app/views/RoundReport_YB"):create(self.curRoomPlayers, self.playerSeatIdx, msgTbl, msgTbl.m_end)
				self:addChild(roundReport, PlaySceneYB.ZOrder.ROUND_REPORT)
			else
...
function PlaySceneYB:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneYB:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneYB:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneYB.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneYB:updateCurrentTime()
...
function PlaySceneYB:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneYB:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYB:playerEnterRoom(msgTbl)
	...
	for i=1,#msgTbl.m_playtype do
		...
		elseif msgTbl.m_playtype[i] == 1 then
			tableStr = tableStr .. "自摸胡 "
		elseif msgTbl.m_playtype[i] == 101 then
			tableStr = tableStr .. "5番 "
		elseif msgTbl.m_playtype[i] == 102 then
			tableStr = tableStr .. "6番 "
		elseif msgTbl.m_playtype[i] == 103 then
			tableStr = tableStr .. "点炮胡 "
		elseif msgTbl.m_playtype[i] == 104 then
			tableStr = tableStr .. "可飘 "
			self.isPiaoType = true
		end
	end
...
-- end --
function PlaySceneYB:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneYB:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneYB:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneYB:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneYB.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneYB:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneYB:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
	-- 显示当先座位标识
...
-- @return
-- end --
function PlaySceneYB:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneYB:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneYB:addMjTileToPlayer(mjColor, mjNumber, replaceType)
	local mjTileName = string.format(gt.MJSprFrame, self.playerFixDispSeat, mjColor, mjNumber)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	self.playMjLayer:addChild(mjTileSpr)
	for i,v in ipairs(self.WanNengPaiCarTable) do
		if mjColor == v[1] and mjNumber == v[2] then
			mjTileSpr:setColor(cc.c3b(255,255,100))
		end
	end
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- end --
function PlaySceneYB:sortPlayerMjTiles( isthreeCard )
...
-- @return 选中的麻将牌
-- end --
function PlaySceneYB:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneYB:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneYB:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneYB:checkMjTile()
...
function PlaySceneYB:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneYB:removePreRoomPlayerOutMjTile(color,number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneYB:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneYB:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneYB:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneYB:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneYB:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneYB:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneYB:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar, mThinkList )
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- 飞变提
function PlaySceneYB:changeFeiToTi( seatIdx, mThink )
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:sortPlayerMjTiles()
	-- -- 添加到碰列表
	-- self:addMjTilePung(seatIdx, mjColor, mjNumber)
end
-- start --
--------------------------------
-- @class function
...
-- @param mjNumber
-- end --
function PlaySceneYB:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
			if mjTile.mjColor == mjColor and mjTile.mjNumber == mjNumber then
...
-- @param decisionType 决策类型
-- end --
function PlaySceneYB:showDecisionAnimation(seatIdx, decisionType, huCard)
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneYB.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneYB.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneYB.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneYB.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneYB.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneYB.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneYB.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneYB.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneYB.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneYB.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneYB.DecisionType.QIANG_GANG then
			gt.soundManager:PlaySpeakSound(roomPlayer.sex, "hu", roomPlayer)
		else
			gt.soundManager:PlaySpeakSound(roomPlayer.sex, "zimo", roomPlayer)
		end
	elseif decisionType == PlaySceneYB.DecisionType.FEI  then
		--fei
	...
	elseif decisionType == PlaySceneYB.DecisionType.TI  then
		--fei
...
-- @param seatIdx 座次
-- end --
function PlaySceneYB:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneYB:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneYB:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneYB:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneYB.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneYB.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneYB.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneYB.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneYB.TAG.FLIMLAYER_BU)
	else
...
function PlaySceneYB:startAudio()
...
function PlaySceneYB:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneYB:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
-- @param msgTbl
-- end --
function PlaySceneYB:onRecUserDingQue( msgTbl )
	...
	if ownerChooseQue == 1 then
		...
		self.dingqueColorState = false
		local roomPlayer = self.roomPlayers[self.playerSeatIdx]
		local wanBtn = gt.seekNodeByName(self.piaoNode,"Btn_piao")
		...
		local tongBtn = gt.seekNodeByName(self.piaoNode,"Btn_bupiao")
...
-- 定缺完成
function PlaySceneYB:onRecUserDingQueComplate( msgTbl )
	dump(msgTbl)
...
function PlaySceneYB:showDingQueType( seatIdx, color )
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	if color == 2 then
		...
		gt.log("roomPlayer.piaoNum = "..roomPlayer.piaoNum)
...
function PlaySceneYB:dingquePrompt()
...
function PlaySceneYB:jianAnimate( node )
...
function PlaySceneYB:showHuPaiType( seatIdx ,color, number , mjhuPaiType, isboolBreak, rcvWinCard )
...
-- 确定可以换的花色
function PlaySceneYB:promptReplaceThreeCard()
...
function PlaySceneYB:exhReplaceThreeCard()
...
function PlaySceneYB:replaceThreeCardAction()
...
function PlaySceneYB:ThreeCardInsertMjType( mjTile )
...
function PlaySceneYB:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneYB:onRecUserReplaceCard( msgTbl )
	-- dump(msgTbl)
	gt.log("开始换三张 。。。。。。。。。。 ")
...
function PlaySceneYB:checkReplaceOkBtn()
...
function PlaySceneYB:onRecUserReplaceCardComplate( msgTbl )
...
function PlaySceneYB:removeThreeCardFormPlayer()
...
function PlaySceneYB:showLastFourCard()
