...
local PlaySceneZGSR = class("PlaySceneZGSR", function()
...
PlaySceneZGSR.StartDecisionType = {
	-- 缺一色
	TYPE_QUEYISE				= 1,
	-- 板板胡
	TYPE_BANBANHU				= 2,
...
function PlaySceneZGSR:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	-- local csbNode, animation = gt.createCSAnimation("PlaySceneXZ.csb")

	
	local csbNode, animation = gt.createCSAnimation("PlaySceneNJ_THR.csb")
	self.mjCount = 13
	for i=1,#enterRoomMsgTbl.m_playtype do
		if enterRoomMsgTbl.m_playtype[i] == 35 then
			self.mjCount = 7
			csbNode, animation = gt.createCSAnimation("PlaySceneNJ_SEV.csb")
			break
		end
	end
	
	gt.seekNodeByName(csbNode, "mahjong_title"):setSpriteFrame("playScene49.png")
	
	-- gt.seekNodeByName(csbNode, "Text_TableType"):setPositionX(690)
	gt.seekNodeByName(csbNode, "Text_TableType"):setContentSize(cc.size(900,30))
	-- gt.seekNodeByName(csbNode, "Btn_inviteFriend"):setPosition(640,320)
	-- gt.seekNodeByName(csbNode, "Btn_setting"):setPosition(1200,640)
	-- gt.seekNodeByName(csbNode, "Btn_message"):setPosition(1200,540)
	...
	--报叫
	self.baojiao = gt.seekNodeByName(self.rootNode,"Node_baojiao")
	self.baojiao:setVisible(false)
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneZGSR.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(selfDrawnDcsNode, PlaySceneZGSR.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneZGSR.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneZGSR.ZOrder.SETTING,102)
	end)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneZGSR.ZOrder.CHAT , 101)
	end)

	-- 麻将层
	local playMjLayer = cc.Layer:create()
	self.rootNode:addChild(playMjLayer, PlaySceneZGSR.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneZGSR.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneZGSR.ZOrder.DISMISS_ROOM)
	...
	gt.socketClient:registerMsgListener(gt.CG_USER_DING_QUE, self, self.onRecBaoJiao)
	gt.socketClient:registerMsgListener(gt.CG_USER_DING_QUE_COMPLATE,self,self.onRecUserDBaoJiaoComplate)

end


function PlaySceneZGSR:playYuYinAnimation()
	local action = nil
...
function PlaySceneZGSR:cancelYuYinAnimation()
...
function PlaySceneZGSR:getLuaBridge()
...
function PlaySceneZGSR:unregisterAllMsgListener()
...
-- end --
function PlaySceneZGSR:onRcvLoginServer(msgTbl)
...
-- 断线重连,走一次登录流程
function PlaySceneZGSR:reLogin()
	print("========重连登录1")
...
function PlaySceneZGSR:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneZGSR:onRcvLoginGate( msgTbl )
...
function PlaySceneZGSR:onNodeEvent(eventName)
...
function PlaySceneZGSR:onRecUserRemoveBarCard( msgTbl )
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneZGSR:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneZGSR:onEnterBackground()
...
function PlaySceneZGSR:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneZGSR:onTouchBegan(touch, event)
...
function PlaySceneZGSR:onTouchMoved(touch, event)
	...
	local touchPoint = self.playMjLayer:convertTouchToNodeSpace(touch)
	if not self.isBaoJiaoDecision then
...
function PlaySceneZGSR:onTouchEnded(touch, event)
	...
	if cc.pDistanceSQ(self.preTouchPoint, touchPoint) > 400 then
		...
		if self.chooseMjTile ~= self.preClickMjTile then
			...
			--显示狼选择牌
			if self.isBaoJiaoDecision then
...
function PlaySceneZGSR:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneZGSR:update(delta)
...
function PlaySceneZGSR:checkVersion()
...
function PlaySceneZGSR:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneZGSR:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
				...
				if respJson ~= nil and #respJson >0 then
					for i,v in ipairs(respJson) do
						...
						if v.GameType == "All" then
							gt.FreeGameType = {101,102,103,104,105,106,107,108,109,110}
						else
...
function PlaySceneZGSR:initWifi()
...
function PlaySceneZGSR:updateWifi()
...
function PlaySceneZGSR:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneZGSR:checkRightCradNum()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvAddPlayer(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	dump(msgTbl)
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneZGSR:onRcvSyncRoomState(msgTbl)
	...
	self.bankerSeatIdx = bankerSeatIdx
	-- 其他玩家牌
...
--检测总分和是否为0
function PlaySceneZGSR:CheckScore()
...
function PlaySceneZGSR:onRcvWinCard(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:onRcvTurnShowMjTile(msgTbl)
	-- dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
	...
	if seatIdx == self.playerSeatIdx then
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							if self.isBaoJiaoState and ((self.m_dCount == 19 and self.mjCount == 13) or (self.m_dCount == 43 and self.mjCount == 7)) then
							
								if self.isBaoJiaoSelected then
							...
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneZGSR.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneZGSR.FLIMTYPE.FLIMLAYER_BU)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneZGSR.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneZGSR.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneZGSR.ZOrder.FLIMLAYER,PlaySceneZGSR.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneZGSR.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneZGSR.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneZGSR.ZOrder.FLIMLAYER,PlaySceneZGSR.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
		...
		elseif self.isBaoJiaoState and ((self.m_dCount ~= 19 and self.mjCount == 13) or (self.m_dCount ~= 43 and self.mjCount == 7)) then
...
function PlaySceneZGSR:setAllMaskLayer(isMark, isAllMask)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
function PlaySceneZGSR:checkOtherTingCards(roomPlayer, m_tingCards)
	local maskCardTbl = {}
...
--isOther：是否是自己
function PlaySceneZGSR:setMySelfMaskLayer(roomPlayer, tingInfo)
	local num = #tingInfo
	gt.log("==bb====")
	gt.dump(tingInfo)
	if num < 1 then
...
--报叫完成
function PlaySceneZGSR:onRecUserDBaoJiaoComplate( msgTbl )
	gt.log("==========bb==a=====")
	--报叫结束
...
-- @description 广播玩家出牌
-- end --
function PlaySceneZGSR:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		...
		self:showDecisionAnimation(seatIdx, PlaySceneZGSR.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneZGSR.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneZGSR.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneZGSR.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneZGSR.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneZGSR.DecisionType.BRIGHT_BU)
		end
	end
...
-- end --
function PlaySceneZGSR:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneZGSR.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneZGSR.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneZGSR.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneZGSR.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneZGSR:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneZGSR:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneZGSR.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneZGSR:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneZGSR:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneZGSR.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneZGSR.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneZGSR.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneZGSR.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneZGSR.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
-- end --
function PlaySceneZGSR:onRcvStartDecision(msgTbl)
...
-- 起手胡,过之外的按钮
function PlaySceneZGSR:createStartHuDecisionMenu()
...
function PlaySceneZGSR:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneZGSR:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
	local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
	decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneZGSR:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneZGSR:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneZGSR.DecisionType.QIANG_GANG, msgTbl.m_hu)
		end
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneZGSR.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneZGSR.DecisionType.EAT)
...
function PlaySceneZGSR:onRcvChatMsg(msgTbl)
...
function PlaySceneZGSR:showFangjianYuAnimation(msgTbl, seatIdx)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
	self.roundReportMsg = nil
	gt.log("self.roundReportMsg == nil")
	...
	local callFunc1 = cc.CallFunc:create(function(sender)
		...
		local function callFunc2()
			...
			local callFunc3 = cc.CallFunc:create(function(sender)
				...
				gt.log("进入动画牌型")
				if self.isShowPaiType then
					gt.log("self.isShowPaiType == true")
				else
					gt.log("self.isShowPaiType == false")
				end
				if self.roundReportMsg then
					gt.log("self.roundReportMsg == true")
				else
					gt.log("self.roundReportMsg == false")
				end
				-- 出的牌标识动画
				gt.log("进入玩家自己的结算")
				if self.roundReportMsg then
...
function PlaySceneZGSR:showPaiType(roundReportMsg, seatIdx)
	self.paiTypeActivity = require("app/views/PaiTypeActivity"):create(roundReportMsg, self.playerSeatIdx, self.curRoomPlayers)
	self:addChild(self.paiTypeActivity, PlaySceneZGSR.ZOrder.ROUND_REPORT_Activity)
end


--单局结束
function PlaySceneZGSR:onRcvRoundReport(msgTbl)
	gt.log("00000---")
	self.roundReportMsg = nil
	gt.dump(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
--全部屏蔽遮罩层true:遮挡false:不遮挡
function PlaySceneZGSR:setAllMaskLayer2(isMark, isAllMask)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
	for j, mjTile in ipairs(roomPlayer.holdMjTiles) do
		if isAllMask == 1 then
			mjTile.mjTileSpr:setColor(cc.c3b(100,100,100))
...
--显示单局结算
function PlaySceneZGSR:showRoundReport(msgTbl)
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			...
			self:addChild(roundReport, PlaySceneZGSR.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
...
function PlaySceneZGSR:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneZGSR:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneZGSR:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneZGSR.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneZGSR:updateCurrentTime()
...
function PlaySceneZGSR:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneZGSR:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneZGSR:playerEnterRoom(msgTbl)
	...
	for i=1,#msgTbl.m_playtype do
		...
		elseif msgTbl.m_playtype[i] == 0 then
		...
		elseif msgTbl.m_playtype[i] == 184 then
			tableStr = tableStr .. "转雨 "
		elseif msgTbl.m_playtype[i] == 185 then
			tableStr = tableStr .. "查叫退税 "
		elseif msgTbl.m_playtype[i] == 187 then
		...
		elseif msgTbl.m_playtype[i] == 188 then
			tableStr = tableStr .. "4番(极品10分) "
		elseif msgTbl.m_playtype[i] == 189 then
			tableStr = tableStr .. "4番(极品16分) "
		elseif msgTbl.m_playtype[i] == 186 then
			tableStr = tableStr .. "过水加番可胡 "
		elseif msgTbl.m_playtype[i] == 51 then
...
-- end --
function PlaySceneZGSR:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneZGSR:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneZGSR:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneZGSR:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneZGSR.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneZGSR:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneZGSR:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
	local displayIdx = (seatIdx + self.seatOffset - 1) % 4 + 1
...
-- @return
-- end --
function PlaySceneZGSR:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneZGSR:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneZGSR:addMjTileToPlayer(mjColor, mjNumber)
...
-- end --
function PlaySceneZGSR:sortPlayerMjTiles()
...
-- @return 选中的麻将牌
-- end --
function PlaySceneZGSR:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneZGSR:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneZGSR:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneZGSR:checkMjTile()
...
function PlaySceneZGSR:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneZGSR:removePreRoomPlayerOutMjTile(color, number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneZGSR:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneZGSR:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneZGSR:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneZGSR:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneZGSR:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneZGSR:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneZGSR:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneZGSR:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneZGSR:showDecisionAnimation(seatIdx, decisionType, huCard)

	
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneZGSR.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneZGSR.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneZGSR.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneZGSR.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneZGSR.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneZGSR.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneZGSR.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneZGSR.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneZGSR.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneZGSR.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneZGSR.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneZGSR:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneZGSR:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneZGSR:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneZGSR:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneZGSR.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneZGSR.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneZGSR.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneZGSR.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneZGSR.TAG.FLIMLAYER_BU)
	else
...
-- @param isShow 显示标志
-- end --
function PlaySceneZGSR:LaoYueNodeVisible(isShow)
	-- body
	local laoYueNode = gt.seekNodeByName(self.rootNode, "Node_Laoyue")
	if(isShow)then
		self.rootNode:reorderChild(laoYueNode, PlaySceneZGSR.ZOrder.HAIDILAOYUE)
...
function PlaySceneZGSR:startAudio()
...
function PlaySceneZGSR:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneZGSR:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
function  PlaySceneZGSR:updatePlayerInfo()
	for i = 1, 4 do
...
function PlaySceneZGSR:showHuPaiType( seatIdx ,color, number , mjhuPaiType,isboolBreak, rcvWinCard)
...
function PlaySceneZGSR:IsSameIp()
...
function PlaySceneZGSR:onRecBaoJiao( msgTbl )
	
	gt.log("-----------ss-----")
	gt.dump(msgTbl)

	-- 一、通知报叫（如果没接到此消息，代表本局无需报叫）

	-- 消息号：enum LMSG_ID MSG_S_2_C_USER_DINGQUE_START= 110		//接收到此消息，代表有玩家需要决策报叫

	-- 变量：  m_state[4]   // 1代表这个位置玩家需要决策报叫， 0代表这个位置玩家不需要决策报叫 2：报叫成功


	-- 二、报叫选择消息（客户端发给服务端）

	-- 消息号:	enum LMSG_ID MSG_C_2_S_USER_DINGQUE= 111

	-- 变量： m_pos  玩家位置
 --    m_color   选择过，赋值为0，选择报叫 赋值为1


	-- 三、所有玩家完成报叫决策 （存在报叫时，所有玩家都决策过报叫了，那么客户端会接到此消息）

	-- 消息号:	enum LMSG_ID MSG_S_2_C_USER_DINGQUE_COMPLETE= 116
	...
	for k,v in pairs(msgTbl.m_state) do
		if k == self.playerSeatIdx then
			gt.log("=====ggg==")
			--ownerChooseBao = v --1：需要显示报叫 0：--没有报叫也不遮挡听牌 2：--没有报叫也不遮挡听牌

			if v == 1 then  --1：需要显示报叫
			
				--显示报叫
				self.isPlayerDecision = true
				self.baojiao:setVisible(true)
				self.baoColorState = false

				local btn_guo = gt.seekNodeByName(self.baojiao,"Btn_guo")
				local btn_baojiao = gt.seekNodeByName(self.baojiao,"Btn_baojiao")
				
				--过点击事件
				gt.addBtnPressedListener(btn_guo, function()
			    	-- 发送决策消息
					local msgToSend = {}
				...
				--报叫点击事件
				gt.addBtnPressedListener(btn_baojiao, function()
				...
				self.roomPlayers[k].isBaoJiaoState = false
				self.isBaoJiaoState = false
			elseif v == 0 then--没有报叫也不遮挡听牌
				gt.log("=====fs=222=")
				self.baojiao:setVisible(false)
				self.roomPlayers[k].isBaoJiaoState = false
				self.isBaoJiaoState = false
				self.isBaoJiaoSelected = false
			elseif v == 2 then--有报叫遮挡听牌
				gt.log("=====fs=11=")
				self.roomPlayers[k].isBaoJiaoState = true
				self.isBaoJiaoState = true
				self.isBaoJiaoSelected = true
				self.baojiao:setVisible(false)
					
				if (self.m_dCount ~= 19 and self.mjCount == 13) or (self.m_dCount ~= 43 and self.mjCount == 7) then
					self:setAllMaskLayer2(true)
				end
			elseif v == -1 then --刚点完动画
				gt.log("=====fs==")
				self.baojiao:setVisible(false)
				self.roomPlayers[k].isBaoJiaoState = true
				self.isBaoJiaoState = true
...
-- @param decisionType 决策类型
-- end --
function PlaySceneZGSR:showBCDecisionAnimation(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
   	-- 胡牌动画 
   	local decisionSignSpr = cc.Sprite:createWithSpriteFrameName("btn_baojiao_cs.png")
	decisionSignSpr:setPosition(roomPlayer.mjTilesReferPos.showMjTilePos)
	self.rootNode:addChild(decisionSignSpr, 100)
	decisionSignSpr:setScale(0)
	local scaleToAction = cc.ScaleTo:create(0.5, 1)
	local easeBackAction = cc.EaseBackOut:create(scaleToAction)
	local fadeOutAction = cc.FadeOut:create(1)
...
function PlaySceneZGSR:showLastFourCard()
