...
local PlaySceneLZ = class("PlaySceneLZ", function()	
	return cc.Scene:create()
end)

PlaySceneLZ.DecisionType = {
	-- 接炮胡
	TAKE_CANNON_WIN				= 1,
	-- 自摸胡
	SELF_DRAWN_WIN				= 2,
...
function PlaySceneLZ:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	local csbNode, animation = gt.createCSAnimation("PlaySceneLZ.csb")
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneLZ.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	self.selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(self.selfDrawnDcsNode, PlaySceneLZ.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneLZ.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneLZ.ZOrder.SETTING,102)
	end)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneLZ.ZOrder.CHAT, 101)
	end)

	-- 麻将层
	local playMjLayer = cc.Layer:create()
	self.rootNode:addChild(playMjLayer, PlaySceneLZ.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneLZ.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneLZ.ZOrder.DISMISS_ROOM)
...
function PlaySceneLZ:playYuYinAnimation()
	local action = nil
...
function PlaySceneLZ:cancelYuYinAnimation()
...
function PlaySceneLZ:getLuaBridge()
...
function PlaySceneLZ:unregisterAllMsgListener()
...
-- end --
function PlaySceneLZ:onRcvLoginServer(msgTbl)
...
function PlaySceneLZ:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneLZ:onRcvLoginGate(msgTbl)
...
function PlaySceneLZ:onNodeEvent(eventName)
...
function PlaySceneLZ:onRecUserRemoveBarCard( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneLZ:onEnterBackground()
...
function PlaySceneLZ:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneLZ:onTouchBegan(touch, event)
...
function PlaySceneLZ:onTouchMoved(touch, event)
...
function PlaySceneLZ:onTouchEnded(touch, event)
...
function PlaySceneLZ:outPaiBigAnimate(seatIdx, color, number ,isShow)
	--播放出牌音效
...
function PlaySceneLZ:update(delta)
...
function PlaySceneLZ:checkVersion()
...
function PlaySceneLZ:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneLZ:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneLZ:updateWifi()
...
function PlaySceneLZ:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneLZ:checkRightCradNum()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvAddPlayer(msgTbl)
	dump(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneLZ:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneLZ:CheckScore()
...
--检验玩法
function PlaySceneLZ:CheckPlayType(msgTbl)
...
function PlaySceneLZ:onRcvWinCard(msgTbl)
	gt.log("onRcvWinCard:================")
	dump(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:onRcvTurnShowMjTile(msgTbl)
	dump(msgTbl)
	gt.log("通知玩家出牌")
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							self.selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneLZ.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneLZ.FLIMTYPE.FLIMLAYER_BU)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneLZ.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneLZ.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneLZ.ZOrder.FLIMLAYER,PlaySceneLZ.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneLZ.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneLZ.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneLZ.ZOrder.FLIMLAYER,PlaySceneLZ.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
function PlaySceneLZ:checkOtherTingCards(roomPlayer, m_tingCards)
	local maskCardTbl = {}
...
--isOther：是否是自己
function PlaySceneLZ:setMySelfMaskLayer(roomPlayer, tingInfo)
	local num = #tingInfo
	if num < 1 then
		return
	end
...
--tingInfo：要屏蔽的听的牌：
--isOther：是否是自己
function PlaySceneLZ:setMaskLayer(roomPlayer, tingInfo, isOther)
...
function PlaySceneLZ:setAllMaskLayer(isMark)
...
function PlaySceneLZ:createMaskLayer(opacity)
	if not opacity then
		-- 用默认透明度
...
--躺成功
function PlaySceneLZ:onRecErrorInfo(msgTbl)
	...
	if msgTbl.m_result == 0 then
		...
		self.rootNode:addChild(decisionSignSpr, PlaySceneLZ.ZOrder.DECISION_SHOW)
...
function PlaySceneLZ:setOtherMark()
...
function PlaySceneLZ:getPlayerTings()
	local maskCardTbl = {}
...
--移除手中狼起的牌
function PlaySceneLZ:sendSelectedMj(seatIdx)
	-- 发送出牌消息
	local roomPlayer = self.roomPlayers[seatIdx]
	if roomPlayer.seatIdx == self.playerSeatIdx then
		self.isPlayerShow = false
...
-- @return
-- end --
function PlaySceneLZ:langQReorderMjTiles(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
function  PlaySceneLZ:updatePlayerInfo()
	for i = 1, 4 do
...
-- @description 广播玩家出牌
-- end --
function PlaySceneLZ:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		-- 自摸胡
		self:showDecisionAnimation(seatIdx, PlaySceneLZ.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneLZ.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			if msgTbl.m_errorCode == -1 then
				...
				self:showDecisionAnimation(seatIdx, PlaySceneLZ.DecisionType.DARK_BAR)
			end
	...
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			if msgTbl.m_errorCode == -1 then
				...
				self:showDecisionAnimation(seatIdx, PlaySceneLZ.DecisionType.DARK_BU)
			end
			if #msgTbl.m_think > 1 then
				for idx=1,#msgTbl.m_think do
					local num = #roomPlayer.holdMjTiles
	...
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
				...
				if msgTbl.m_errorCode == -1 then
					...
					self:showDecisionAnimation(seatIdx, PlaySceneLZ.DecisionType.BRIGHT_BAR)
				end
				if #msgTbl.m_think > 1 then
					for idx=1,#msgTbl.m_think do
						local num = #roomPlayer.holdMjTiles
	...
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			if msgTbl.m_errorCode == -1 then
				...
				self:showDecisionAnimation(seatIdx, PlaySceneLZ.DecisionType.BRIGHT_BU)
			end
...
-- end --
function PlaySceneLZ:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneLZ.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneLZ.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneLZ.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneLZ.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneLZ:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneLZ:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneLZ.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneLZ:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneLZ:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneLZ.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneLZ.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneLZ.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneLZ.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
function PlaySceneLZ:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneLZ:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
...
-- @description 通知玩家决策
-- end --
function PlaySceneLZ:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneLZ:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneLZ.DecisionType.QIANG_GANG, msgTbl.m_hu)
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneLZ.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneLZ.DecisionType.EAT)
...
function PlaySceneLZ:onRcvChatMsg(msgTbl)
...
--单局结束
function PlaySceneLZ:onRcvRoundReport(msgTbl)
	self.roundReportMsg = nil
	gt.log("00000---")
	gt.dump(msgTbl)
...
function PlaySceneLZ:showRoundReport(msgTbl)
	--删除聊天框
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			...
			self:addChild(roundReport, PlaySceneLZ.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
...
function PlaySceneLZ:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneLZ:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneLZ:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneLZ.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneLZ:updateCurrentTime()
...
function PlaySceneLZ:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneLZ:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneLZ:playerEnterRoom(msgTbl)
	...
	local TypeStr,tableStr = gt.PalyTypeText(111,msgTbl.m_playtype)
...
-- end --
function PlaySceneLZ:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneLZ:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneLZ:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneLZ:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneLZ.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneLZ:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneLZ:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
	-- 显示当先座位标识
...
-- @return
-- end --
function PlaySceneLZ:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneLZ:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneLZ:addMjTileToPlayer(mjColor, mjNumber, replaceType)
...
-- end --
function PlaySceneLZ:sortPlayerMjTiles( isthreeCard )
...
-- @return 选中的麻将牌
-- end --
function PlaySceneLZ:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneLZ:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
...
function PlaySceneLZ:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneLZ:checkMjTile()
...
function PlaySceneLZ:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneLZ:removePreRoomPlayerOutMjTile(color,number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneLZ:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneLZ:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneLZ:addMjTilePung(seatIdx, mjColor, mjNumber, mthink)
...
-- end --
function PlaySceneLZ:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar, mthink)
	local roomPlayer = self.roomPlayers[seatIdx]
	local barData = {}
	if mthink and #mthink > 1 then
		-- 加入到列表中
...
-- end --
function PlaySceneLZ:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneLZ:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneLZ:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar, mthink )
	--gt.log("-------bbbb----")
	--dump( mthink )
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
...
-- @param mjNumber
-- end --
function PlaySceneLZ:changePungToBrightBar(seatIdx, mjColor, mjNumber, mjColor1, mjNumber1)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneLZ:showDecisionAnimation(seatIdx, decisionType, huCard)
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneLZ.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneLZ.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneLZ.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneLZ.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneLZ.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneLZ.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneLZ.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneLZ.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneLZ.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneLZ.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneLZ.DecisionType.QIANG_GANG then
...
-- @param seatIdx 座次
-- end --
function PlaySceneLZ:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneLZ:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneLZ:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneLZ:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneLZ.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneLZ.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneLZ.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneLZ.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneLZ.TAG.FLIMLAYER_BU)
	else
...
function PlaySceneLZ:startAudio()
...
function PlaySceneLZ:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneLZ:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
function PlaySceneLZ:showHuPaiType( seatIdx ,color, number , mjhuPaiType, isboolBreak, rcvWinCard )
...
-- 确定可以换的花色
function PlaySceneLZ:promptReplaceThreeCard()
...
function PlaySceneLZ:exhReplaceThreeCard()
...
function PlaySceneLZ:replaceThreeCardAction()
...
function PlaySceneLZ:ThreeCardInsertMjType( mjTile )
...
function PlaySceneLZ:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneLZ:checkReplaceOkBtn()
...
function PlaySceneLZ:removeThreeCardFormPlayer()
...
function PlaySceneLZ:showFangjianYuAnimation(msgTbl, seatIdx)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneLZ:showPaiType(roundReportMsg, seatIdx)
	if not self.paiTypeActivity then
		...
		self:addChild(self.paiTypeActivity, PlaySceneLZ.ZOrder.ROUND_REPORT_Activity)
	end
end


function PlaySceneLZ:showLastFourCard()
