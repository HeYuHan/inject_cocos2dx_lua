...
local PlaySceneYA = class("PlaySceneYA", function()	
	return cc.Scene:create()
end)

PlaySceneYA.DecisionType = {
	-- 接炮胡
	TAKE_CANNON_WIN				= 1,
	-- 自摸胡
	SELF_DRAWN_WIN				= 2,
...
function PlaySceneYA:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	local csbNode, animation = gt.createCSAnimation("PlaySceneYA.csb")
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneYA.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	self.selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(self.selfDrawnDcsNode, PlaySceneYA.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneYA.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneYA.ZOrder.SETTING,102)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneYA.ZOrder.CHAT, 101)
	...
	self.rootNode:addChild(playMjLayer, PlaySceneYA.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneYA.ZOrder.OUTMJTILE_SIGN)
	 	...
	 	if v == 35 then
	 		...
	 		for j=1,4 do
				...
				if j==4 then
					...
					-- mjTilesReferNode:setPositionX(mjTilesReferNode:getPositionX()+250)
				elseif j == 1 then
					for _, child in ipairs(mjTilesReferNode:getChildren()) do
						if child:getName() ~= "Spr_mjTileOut_1" and child:getName() ~= "Spr_mjTileOut_2" and child:getName() ~= "Spr_mjTileOut_3" then
					...
					-- mjTilesReferNode:setPositionY(mjTilesReferNode:getPositionY()+40)
				elseif j == 2 then
					for _, child in ipairs(mjTilesReferNode:getChildren()) do
						if child:getName() ~= "Spr_mjTileOut_1" and child:getName() ~= "Spr_mjTileOut_2" and child:getName() ~= "Spr_mjTileOut_3" then
					...
					-- mjTilesReferNode:setPositionX(mjTilesReferNode:getPositionX()-120)
				elseif j == 3 then
					for _, child in ipairs(mjTilesReferNode:getChildren()) do
						if child:getName() ~= "Spr_mjTileOut_1" and child:getName() ~= "Spr_mjTileOut_2" and child:getName() ~= "Spr_mjTileOut_3" then
					...
					-- mjTilesReferNode:setPositionY(mjTilesReferNode:getPositionY()-170)
				end
	 		end
 		elseif v == 36 then
 			--10张
 			for j=1,4 do
				...
				if j==4 then
					...
					-- mjTilesReferNode:setPositionX(mjTilesReferNode:getPositionX()+150)
				elseif j == 1 then
					for _, child in ipairs(mjTilesReferNode:getChildren()) do
						if child:getName() ~= "Spr_mjTileOut_1" and child:getName() ~= "Spr_mjTileOut_2" and child:getName() ~= "Spr_mjTileOut_3" then
							child:setPositionY(child:getPositionY()+40)
				...
				elseif j == 2 then
					...
					-- mjTilesReferNode:setPositionX(mjTilesReferNode:getPositionX()-40)
				elseif j == 3 then
					for _, child in ipairs(mjTilesReferNode:getChildren()) do
						if child:getName() ~= "Spr_mjTileOut_1" and child:getName() ~= "Spr_mjTileOut_2" and child:getName() ~= "Spr_mjTileOut_3" then
	...
	self:addChild(self.applyDimissRoom, PlaySceneYA.ZOrder.DISMISS_ROOM)
	...
	gt.addBtnPressedListener(self.btn_backTang, function()
			...
			local num = #maskCardTbl
			for j, mjTile in ipairs(roomPlayer.holdMjTiles) do
				for i = 1, num do
					if mjTile.mjColor == maskCardTbl[i][1] and mjTile.mjNumber == maskCardTbl[i][2] then
						tingNum = tingNum+1
						break
					end
				end
			...
			if tingNum == #roomPlayer.holdMjTiles then
				...
				roomPlayer.holdMjTiles[#roomPlayer.holdMjTiles].mjTileSpr:setColor(cc.c3b(255,255,255))
				roomPlayer.holdMjTiles[#roomPlayer.holdMjTiles].isMark = false
			else
				if not roomPlayer.isLangSelectedState then
					local maskCardTbl = self:getPlayerTings()
					self:setMaskLayer(roomPlayer, maskCardTbl, true)
					gt.log("无躺的时候发牌 isLangSelectedState ＝ false")
...
function PlaySceneYA:playYuYinAnimation()
	local action = nil
...
function PlaySceneYA:cancelYuYinAnimation()
...
function PlaySceneYA:getLuaBridge()
...
function PlaySceneYA:unregisterAllMsgListener()
...
-- end --
function PlaySceneYA:onRcvLoginServer(msgTbl)
...
function PlaySceneYA:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneYA:onRcvLoginGate(msgTbl)
...
function PlaySceneYA:onNodeEvent(eventName)
...
function PlaySceneYA:onRecUserRemoveBarCard( msgTbl )
...
-- 当前房间的玩法   支持换三张吗
function PlaySceneYA:onTablePlayState( msgTbl )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvPlaySeceneCSMarquee(msgTbl)
...
--切到后台 恢复鼠标事件
function PlaySceneYA:onEnterBackground()
...
function PlaySceneYA:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneYA:onTouchBegan(touch, event)
...
function PlaySceneYA:onTouchMoved(touch, event)
	gt.log("------------------onTouchMoved----------------")
...
function PlaySceneYA:onTouchEnded(touch, event)
	gt.log("------------------onTouchEnded----------------")
	if self.isPlayThreeCardState == false then
		-- 不需要换三张 
...
function PlaySceneYA:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneYA:update(delta)
...
function PlaySceneYA:checkVersion()
...
function PlaySceneYA:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneYA:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneYA:updateWifi()
...
function PlaySceneYA:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneYA:checkRightCradNum()
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
	...
	-- 玩家持有牌
	local holdNum = #roomPlayer.holdMjTiles+#roomPlayer.langMjTiles
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvAddPlayer(msgTbl)
	dump(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvRemovePlayer(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 隐藏玩家信息
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @description 断线重连
-- end --
function PlaySceneYA:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneYA:CheckScore()
...
function PlaySceneYA:onRcvWinCard(msgTbl)
	gt.log("onRcvWinCard:================")
	dump(msgTbl)
	-- 同步断线后胡牌的状态
	self.ownerWin = false
...
-- 校验换三张
function PlaySceneYA:onRcvSyncReplaceCardType( )
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:onRcvTurnShowMjTile(msgTbl)
	dump(msgTbl)
	gt.log("通知玩家出牌")
	self.Node_Tang:setVisible(false)
	-- 牌局状态,剩余牌
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							self.selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneYA.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneYA.FLIMTYPE.FLIMLAYER_BU)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneYA.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneYA.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneYA.ZOrder.FLIMLAYER,PlaySceneYA.TAG.FLIMLAYER_BAR)
				...
				elseif decisionData.flag == 7 or decisionData.flag == 8 then   -- 补张
					if #cardList_bu == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneYA.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneYA.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
							self:addChild(flimLayer,PlaySceneYA.ZOrder.FLIMLAYER,PlaySceneYA.TAG.FLIMLAYER_BU)
							flimLayer:ignoreAnchorPointForPosition(false)
							flimLayer:setAnchorPoint(0.5,0)
							local pos_x = 0
							if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
function PlaySceneYA:checkOtherTingCards(roomPlayer, m_tingCards)
	local maskCardTbl = {}
...
--isOther：是否是自己
function PlaySceneYA:setMySelfMaskLayer(roomPlayer, tingInfo)
	local num = #tingInfo
	if num < 1 then
		return
	end
...
--tingInfo：要屏蔽的听的牌：
--isOther：是否是自己
function PlaySceneYA:setMaskLayer(roomPlayer, tingInfo, isOther)
...
function PlaySceneYA:setAllMaskLayer(isMark)
...
function PlaySceneYA:createMaskLayer(opacity)
	if not opacity then
		-- 用默认透明度
...
--显示选择狼起的牌
function PlaySceneYA:showSelectMj()
	gt.log("进入躺决策按钮回调")
	--添加不能触摸层
...
--躺成功
function PlaySceneYA:onRecErrorInfo(msgTbl)
	...
	if msgTbl.m_result == 0 then
		...
		if roomPlayer.seatIdx == self.playerSeatIdx then
			gt.log("自己躺牌")
			self.isPlayerDecision = false
			self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.LANG_QI)
...
function PlaySceneYA:setOtherMark()
...
function PlaySceneYA:getPlayerTings()
	local maskCardTbl = {}
...
--移除手中狼起的牌
function PlaySceneYA:sendSelectedMj(seatIdx)
	-- 发送出牌消息
	local roomPlayer = self.roomPlayers[seatIdx]
	if roomPlayer.seatIdx == self.playerSeatIdx then
		self.isPlayerShow = false
...
-- @return
-- end --
function PlaySceneYA:langQReorderMjTiles(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
function  PlaySceneYA:updatePlayerInfo()
	for i = 1, 4 do
		local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. i)
...
-- @description 广播玩家出牌
-- end --
function PlaySceneYA:onRcvSyncShowMjTile(msgTbl)
	...
	if msgTbl.m_type == 2 then
		-- 自摸胡
		self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 9 then
		-- 胡
		self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.DARK_BAR)
		end
	elseif msgTbl.m_type == 7 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:addMjTileBar(seatIdx, mj_color, mj_number, false)
			self:hideOtherPlayerMjTiles(seatIdx, true, false)
			self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.DARK_BU)
		end
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.BRIGHT_BAR)
		end
	elseif msgTbl.m_type == 8 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			local  mj_number = msgTbl.m_think[1][2]
			self:changePungToBrightBar(seatIdx, mj_color, mj_number)
			self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.BRIGHT_BU)
		end
	elseif msgTbl.m_type == 14 then
		-- 躺之后自动胡
        self.roomPlayers[seatIdx].mjTilesRemainCount = self.roomPlayers[seatIdx].mjTilesRemainCount + 1
		self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
...
-- end --
function PlaySceneYA:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneYA.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneYA.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneYA.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneYA.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneYA:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
-- @description 展示杠两张牌
-- end --
function PlaySceneYA:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneYA.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneYA:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneYA:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneYA.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneYA.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneYA.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneYA.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneYA.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
function PlaySceneYA:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneYA:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
	-- local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
	-- decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneYA:onRcvMakeDecision(msgTbl)
...
-- @description 广播决策结果
-- end --
function PlaySceneYA:onRcvSyncMakeDecision(msgTbl)
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		...
		if msgTbl.m_think[1] == 2 then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.QIANG_GANG, msgTbl.m_hu)
		...
		self.roomPlayers[seatIdx].tingCards = {}
		self:setAllMaskLayer(false)
		self:setOtherMark()
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneYA.DecisionType.EAT)
...
function PlaySceneYA:onRcvChatMsg(msgTbl)
...
function PlaySceneYA:showFangjianYuAnimation(msgTbl, seatIdx)
...
function PlaySceneYA:showPaiType(roundReportMsg, seatIdx)
	if not self.paiTypeActivity then
		...
		self:addChild(self.paiTypeActivity, PlaySceneYA.ZOrder.ROUND_REPORT_Activity)
	end
end


--单局结束
function PlaySceneYA:onRcvRoundReport(msgTbl)
	self.roundReportMsg = nil
	gt.log("00000---")
	gt.dump(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
--显示单局结算
function PlaySceneYA:showRoundReport(msgTbl)
	--删除聊天框
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			...
			self:addChild(roundReport, PlaySceneYA.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
...
function PlaySceneYA:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneYA:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneYA:showFinalReport(msgTbl)
	...
	self:addChild(self.finalReport, PlaySceneYA.ZOrder.REPORT)
...
-- @description 更新当前时间
-- end --
function PlaySceneYA:updateCurrentTime()
...
function PlaySceneYA:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneYA:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneYA:playerEnterRoom(msgTbl)
...
-- end --
function PlaySceneYA:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneYA:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneYA:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneYA:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneYA.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneYA:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneYA:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
	-- 显示当先座位标识
...
-- @return
-- end --
function PlaySceneYA:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneYA:playTimeCDUpdate(delta)
...
-- @param replaceType
-- end --
function PlaySceneYA:addMjTileToPlayer(mjColor, mjNumber, replaceType)
...
-- end --
function PlaySceneYA:sortPlayerMjTiles( isthreeCard )
...
-- @return 选中的麻将牌
-- end --
function PlaySceneYA:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneYA:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneYA:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneYA:checkMjTile()
...
function PlaySceneYA:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneYA:removePreRoomPlayerOutMjTile(color,number)
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneYA:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneYA:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneYA:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneYA:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneYA:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneYA:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	-- dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneYA:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneYA:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneYA:showDecisionAnimation(seatIdx, decisionType, huCard)
	local roomPlayer = self.roomPlayers[seatIdx]
	-- 四川麻将  杠就是刮风下雨
	if decisionType == PlaySceneYA.DecisionType.BRIGHT_BAR or 
	   decisionType == PlaySceneYA.DecisionType.BRIGHT_BU then
	   	-- 刮风
		...
		self.rootNode:addChild(brightBarAnimateNode, PlaySceneYA.ZOrder.MJBAR_ANIMATION)
	...
	elseif decisionType == PlaySceneYA.DecisionType.DARK_BAR or 
	       decisionType == PlaySceneYA.DecisionType.DARK_BU then
	   	-- 下雨
		local Node_DecisionAnimate = gt.seekNodeByName(self.rootNode,"Node_DecisionAnimate")
		local node_animate = gt.seekNodeByName(Node_DecisionAnimate,"node_animate_" .. roomPlayer.displaySeatIdx)
	...
	elseif decisionType == PlaySceneYA.DecisionType.TAKE_CANNON_WIN or
		   decisionType == PlaySceneYA.DecisionType.SELF_DRAWN_WIN or 
		   decisionType == PlaySceneYA.DecisionType.QIANG_GANG then
	   	-- 胡牌动画 现在只有一个胡的标志
	   	local decisionSignSpr = nil
	   	if decisionType == PlaySceneYA.DecisionType.QIANG_GANG then
		...
		if decisionType == PlaySceneYA.DecisionType.TAKE_CANNON_WIN or decisionType == PlaySceneYA.DecisionType.QIANG_GANG then
			gt.soundManager:PlaySpeakSound(roomPlayer.sex, "hu", roomPlayer)
		else
			gt.soundManager:PlaySpeakSound(roomPlayer.sex, "zimo", roomPlayer)
		end
	elseif decisionType == PlaySceneYA.DecisionType.LANG_QI then
		gt.log("廊起动画")
		local decisionSignSpr = cc.Sprite:createWithSpriteFrameName("decision_sign_cs_12.png")
		decisionSignSpr:setPosition(roomPlayer.mjTilesReferPos.showMjTilePos)
		self.rootNode:addChild(decisionSignSpr, PlaySceneYA.ZOrder.DECISION_SHOW)
		decisionSignSpr:setScale(0)
		local scaleToAction = cc.ScaleTo:create(0.5, 1)
...
-- @param seatIdx 座次
-- end --
function PlaySceneYA:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneYA:backMainSceneEvt(eventType, isRoomCreater, roomID)
...
function PlaySceneYA:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneYA:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneYA.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneYA.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneYA.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneYA.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneYA.TAG.FLIMLAYER_BU)
	else
...
function PlaySceneYA:startAudio()
...
function PlaySceneYA:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneYA:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
-- @param msgTbl
-- end --
function PlaySceneYA:onRecUserDingQue( msgTbl )
...
-- 定缺完成
function PlaySceneYA:onRecUserDingQueComplate( msgTbl )
...
function PlaySceneYA:showDingQueType( seatIdx, color )
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneYA:dingquePrompt()
...
function PlaySceneYA:jianAnimate( node )
...
function PlaySceneYA:showHuPaiType( seatIdx ,color, number , mjhuPaiType, isboolBreak, rcvWinCard )
...
-- 确定可以换的花色
function PlaySceneYA:promptReplaceThreeCard()
...
function PlaySceneYA:exhReplaceThreeCard()
...
function PlaySceneYA:replaceThreeCardAction()
...
function PlaySceneYA:ThreeCardInsertMjType( mjTile )
...
function PlaySceneYA:ThreeCardRemoveMjType( mjTile )
...
function PlaySceneYA:onRecUserReplaceCard( msgTbl )
	-- dump(msgTbl)
	gt.log("开始换三张 。。。。。。。。。。 ")
...
function PlaySceneYA:checkReplaceOkBtn()
...
function PlaySceneYA:onRecUserReplaceCardComplate( msgTbl )
...
function PlaySceneYA:removeThreeCardFormPlayer()
...
function PlaySceneYA:showLastFourCard()
...
function PlaySceneYA:showLastEightCard()
	...
	self.Image_ReplaceNotice = Image_ReplaceNotice
	local showLabel = gt.seekNodeByName(Image_ReplaceNotice,"label_replace_show")
	showLabel:setString("最后8张牌,不允许再躺了")
