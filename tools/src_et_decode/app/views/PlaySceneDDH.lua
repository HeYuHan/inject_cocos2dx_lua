...
local PlaySceneDDH = class("PlaySceneDDH", function()
...
PlaySceneDDH.StartDecisionType = {
	-- 缺一色
	TYPE_QUEYISE				= 1,
	-- 板板胡
	TYPE_BANBANHU				= 2,
...
function PlaySceneDDH:ctor(enterRoomMsgTbl)
	...
	-- 加载界面资源
	local csbNode, animation = gt.createCSAnimation("PlaySceneDDH.csb")
	...
	if display.autoscale == "FIXED_HEIGHT" then
		...
		gt.seekNodeByName(csbNode, "Btn_message"):setPositionY(640)
	...
	if self.marqueeNode then
		self.marqueeNode:setZOrder( 8888 ) -- 跑马灯优先级较高
		local marqueeMsg = require("app/MarqueeMsg"):create( self.marqueeNode )
		self.marqueeNode:addChild(marqueeMsg)
		self.marqueeMsg = marqueeMsg
		if gt.marqueeMsgTemp then
			self.marqueeMsg:showMsg(gt.marqueeMsgTemp, 3)
		end
		if gt.isIOSPlatform() and gt.isInReview then
			self.marqueeNode:setVisible( false )
		end
	...
	self.rootNode:reorderChild(decisionBtnNode, PlaySceneDDH.ZOrder.DECISION_BTN)
	decisionBtnNode:setVisible(false)
	-- 隐藏自摸决策暗杠，碰转明杠，自摸胡
	local selfDrawnDcsNode = gt.seekNodeByName(self.rootNode, "Node_selfDrawnDecision")
	self.rootNode:reorderChild(selfDrawnDcsNode, PlaySceneDDH.ZOrder.DECISION_BTN)
	...
	self.rootNode:reorderChild(chatBgNode, PlaySceneDDH.ZOrder.CHAT)
	...
	gt.addBtnPressedListener(settingBtn, function()
		...
		self:addChild(settingPanel, PlaySceneDDH.ZOrder.SETTING,102)
	end)
	...
	gt.addBtnPressedListener(messageBtn, function()
		...
		self:addChild(chatPanel, PlaySceneDDH.ZOrder.CHAT,101)
	end)

	-- 麻将层
	local playMjLayer = cc.Layer:create()
	self.rootNode:addChild(playMjLayer, PlaySceneDDH.ZOrder.MJTILES)
	...
	self.rootNode:addChild(outMjtileSignNode, PlaySceneDDH.ZOrder.OUTMJTILE_SIGN)
	...
	self:addChild(self.applyDimissRoom, PlaySceneDDH.ZOrder.DISMISS_ROOM)
...
function PlaySceneDDH:playYuYinAnimation()
	local action = nil
...
function PlaySceneDDH:cancelYuYinAnimation()
...
function PlaySceneDDH:getLuaBridge()
...
function PlaySceneDDH:unregisterAllMsgListener()
	...
	-- gt.socketClient:unregisterMsgListener(gt.GC_ZHANIAO)
end
...
-- end --
function PlaySceneDDH:onRcvLoginServer(msgTbl)
...
function PlaySceneDDH:onRcvLogin(msgTbl)
...
--服务器返回gate登录
function PlaySceneDDH:onRcvLoginGate( msgTbl )
...
function PlaySceneDDH:onNodeEvent(eventName)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvPlaySeceneCSMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
...
function PlaySceneDDH:onTouchCancelled(touch, event)
	self.isTouchBegan = false
end

function PlaySceneDDH:onTouchBegan(touch, event)
	...
	if self.playType == gt.RoomType.ROOM_CHANGSHA then
		...
		if roomPlayer.m_gang == 1 and mjTileIdx ~= #roomPlayer.holdMjTiles then
			return false
		end
	end
...
function PlaySceneDDH:onTouchMoved(touch, event)
...
function PlaySceneDDH:onTouchEnded(touch, event)
...
function PlaySceneDDH:outPaiBigAnimate(seatIdx, color, number ,isShow)
...
function PlaySceneDDH:update(delta)
...
function PlaySceneDDH:checkVersion()
...
function PlaySceneDDH:initPaoMaDeng()
	-- 跑马灯
...
function PlaySceneDDH:onRcvMarquee(msgTbl)
	if gt.isIOSPlatform() and gt.isInReview then
		-- local str_des = gt.getLocationString("LTKey_0048")
...
function PlaySceneDDH:initWifi()
...
function PlaySceneDDH:updateWifi()
...
function PlaySceneDDH:getStaticMethod(methodName)
	local ok = ""
	local result = ""
	if gt.isIOSPlatform() then
...
-- 检查手牌是否低于13张牌
function PlaySceneDDH:checkRightCradNum()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvRoomCard(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvEnterRoom(msgTbl)
	gt.removeLoadingTips()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvAddPlayer(msgTbl)
	-- 封装消息数据放入到房间玩家表中
	local roomPlayer = {}
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvRemovePlayer(msgTbl)
...
-- @description 断线重连
-- end --
function PlaySceneDDH:onRcvSyncRoomState(msgTbl)
...
--检测总分和是否为0
function PlaySceneDDH:CheckScore()
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvReady(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvOffLineState(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	local roomPlayer = self.roomPlayers[seatIdx]
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
	-- 离线标示
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvRoundState(msgTbl)
	-- 牌局状态,剩余牌
	local roundStateNode = gt.seekNodeByName(self.rootNode, "Node_roundState")
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvStartGame(msgTbl)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:onRcvTurnShowMjTile(msgTbl)
	-- testdata
	-- auto yang
	-- msgTbl.m_think = {
	-- 	{3, {{3,3}}},
	-- 	{4, {{1,5},{2,7}}},
	-- 	-- {m_type = 8, m_think = {{1,1},{2,2}}}
	-- }
	gt.log("通知玩家出牌")
	dump(msgTbl)
	-- 牌局状态,剩余牌
	...
	if seatIdx == self.playerSeatIdx then
		...
		if #decisionTypes > 0 then
				...
				if nodeName == "Btn_decisionPass" then
					...
					gt.addBtnPressedListener(decisionBtn, function()
						local function passDecision()
							...
							selfDrawnDcsNode:setVisible(false)
							-- 删除弹出框（杠）
							self:removeFlimLayer(PlaySceneDDH.FLIMTYPE.FLIMLAYER_BAR)
							-- 删除弹出框（补）
							self:removeFlimLayer(PlaySceneDDH.FLIMTYPE.FLIMLAYER_BU)
			...
			local cardList_bu = {}
			for idx, decisionData in ipairs(decisionTypes) do
				if decisionData.flag == 3 or decisionData.flag == 4 or decisionData.flag == 7 or decisionData.flag == 8 then
					-- 明暗杠
			...
			for idx, decisionData in ipairs(decisionTypes) do
				...
				elseif decisionData.flag == 3 or decisionData.flag == 4 or decisionData.flag == 7 or decisionData.flag == 8 then
					-- 明暗杠
					gt.log("decisionData.flag = "..decisionData.flag)
					...
					gt.log("btn_bar_name = "..btn_bar_name)
				-- elseif decisionData.flag == 7 or decisionData.flag == 8 then
				-- 	-- 明暗杠
				-- 	local btn_bar_name = "Btn_decisionBar"
				-- 	decisionBtn = gt.seekNodeByName(selfDrawnDcsNode, btn_bar_name)
				...
				if decisionData.flag == 3 or decisionData.flag == 4 or decisionData.flag == 7 or decisionData.flag == 8 then
					if #cardList_bar == 1 then
						...
						gt.addBtnPressedListener(decisionBtn, function(sender)
							...
							self:removeFlimLayer(PlaySceneDDH.FLIMTYPE.FLIMLAYER_BU)
							-- add new
							local flimLayer = self:createFlimLayer(PlaySceneDDH.FLIMTYPE.FLIMLAYER_BAR,cardList_bar)
							self:addChild(flimLayer,PlaySceneDDH.ZOrder.FLIMLAYER,PlaySceneDDH.TAG.FLIMLAYER_BAR)
				...
				-- 			self:removeFlimLayer(PlaySceneDDH.FLIMTYPE.FLIMLAYER_BU)
				-- 			-- add new
				-- 			local flimLayer = self:createFlimLayer(PlaySceneDDH.FLIMTYPE.FLIMLAYER_BU,cardList_bu)
				-- 			self:addChild(flimLayer,PlaySceneDDH.ZOrder.FLIMLAYER,PlaySceneDDH.TAG.FLIMLAYER_BU)
				-- 			flimLayer:ignoreAnchorPointForPosition(false)
				-- 			flimLayer:setAnchorPoint(0.5,0)
				-- 			local pos_x = 0
				-- 			if decisionBtn:getPositionX()+flimLayer:getContentSize().width/2 > gt.winSize.width then
...
-- @description 广播玩家出牌
-- end --
function PlaySceneDDH:onRcvSyncShowMjTile(msgTbl)
	gt.log("广播玩家出牌")
	dump(msgTbl)
	...
	if msgTbl.m_type == 2 then
		...
		self:showDecisionAnimation(seatIdx, PlaySceneDDH.DecisionType.SELF_DRAWN_WIN, msgTbl.m_hu)
	...
	elseif msgTbl.m_type == 3 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneDDH.DecisionType.DARK_BAR)
		end
	...
	elseif msgTbl.m_type == 4 then
		...
		if (next(msgTbl.m_think) ~= nil) then
			...
			self:showDecisionAnimation(seatIdx, PlaySceneDDH.DecisionType.BRIGHT_BAR)
		end
...
-- end --
function PlaySceneDDH:onRcvSyncStartDecision(msgTbl)
	local seatIdx = msgTbl.m_pos + 1
	if msgTbl.m_type == 1 then
		-- 缺一色
		self:showStartDecisionAnimation(seatIdx, PlaySceneDDH.StartDecisionType.TYPE_QUEYISE, msgTbl.m_card)
	elseif msgTbl.m_type == 2 then
		-- 板板胡
		self:showStartDecisionAnimation(seatIdx, PlaySceneDDH.StartDecisionType.TYPE_BANBANHU, msgTbl.m_card)
	elseif msgTbl.m_type == 3 then
		-- 大四喜
		self:showStartDecisionAnimation(seatIdx, PlaySceneDDH.StartDecisionType.TYPE_DASIXI, msgTbl.m_card)
	elseif msgTbl.m_type == 4 then
		-- 六六顺
		self:showStartDecisionAnimation(seatIdx, PlaySceneDDH.StartDecisionType.TYPE_LIULIUSHUN, msgTbl.m_card)
	end
end
...
-- end --
function PlaySceneDDH:onRcvSyncBarTwoCard(msgTbl)
	-- set test data
	-- auto yang
	-- msgTbl = {}
	-- msgTbl.m_msgId = 68
...
function  PlaySceneDDH:updatePlayerInfo()
	for i = 1, 4 do
...
-- @description 展示杠两张牌
-- end --
function PlaySceneDDH:showBarTwoCardAnimation(seatIdx,cardList)
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	self:addChild(image_bg,PlaySceneDDH.ZOrder.HAIDILAOYUE)
	image_bg:setScale(0)
	-- 设置坐标位置
	local  m_curPos_x = 1
	local  m_curPos_y = 1
...
function PlaySceneDDH:discardsOneCard(seatIdx,mjColor,mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	local mjTilePos = mjTilesReferPos.holdStart
...
-- end --
function PlaySceneDDH:showStartDecisionAnimation(seatIdx, decisionType, showCard)
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	...
	self.rootNode:addChild(decisionSignSpr, PlaySceneDDH.ZOrder.DECISION_SHOW)
	-- 标识显示动画
	...
	-- 展示起手胡牌型
	local copyNum = 1
	if decisionType == PlaySceneDDH.StartDecisionType.TYPE_QUEYISE then
		copyNum = 1
	elseif decisionType == PlaySceneDDH.StartDecisionType.TYPE_BANBANHU then
		copyNum = 1
	elseif decisionType == PlaySceneDDH.StartDecisionType.TYPE_DASIXI then
		copyNum = 4
	elseif decisionType == PlaySceneDDH.StartDecisionType.TYPE_LIULIUSHUN then
		copyNum = 3
	end
...
-- end --
function PlaySceneDDH:onRcvStartDecision(msgTbl)
...
-- 起手胡,过之外的按钮
function PlaySceneDDH:createStartHuDecisionMenu()
...
function PlaySceneDDH:createStartDecisionFlimLayer(flimLayerType,cardList, posx, posy)
...
function PlaySceneDDH:onSendMSg66( cartType, cardarray )
	self.isPlayerDecision = false
	-- 隐藏决策按键
	local decisionBtnNode = gt.seekNodeByName(self.rootNode, "Node_start_decisionBtn")
	decisionBtnNode:setVisible(false)
...
-- @description 通知玩家决策
-- end --
function PlaySceneDDH:onRcvMakeDecision(msgTbl)
...
--msgTbl.m_think = {{6, {{3,4},{3,5}}}, {6, {{3,7}, {3,8}}}, {6, {{3,5},{3,7}}}   }
	...
	if msgTbl.m_flag == 1 then
		...
		for i, v in ipairs(noSame) do
			...
			if self.playType == gt.RoomType.ROOM_CHANGSHA and (m_type == 1 or m_type == 6) then
				mjTileSpr:setVisible(false)
			end
...
-- @description 广播决策结果
-- end --
function PlaySceneDDH:onRcvSyncMakeDecision(msgTbl)
	if msgTbl.m_errorCode ~= 0 then
		return
	end
	...
	if msgTbl.m_think[1] == 2 or msgTbl.m_think[1] == 10 then
		-- 接炮胡m_hu
		self:showDecisionAnimation(seatIdx, PlaySceneDDH.DecisionType.TAKE_CANNON_WIN, msgTbl.m_hu)
		self:showAllMjTilesWhenWin(seatIdx, msgTbl.m_cardCount, msgTbl.m_cardValue, msgTbl.m_color, msgTbl.m_number)
		if msgTbl.m_rewardCardNum and tonumber(msgTbl.m_rewardCardNum) > 0 then
			self:showFangjianYuAnimation(msgTbl, seatIdx)
		end
	...
	elseif msgTbl.m_think[1] == 5 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneDDH.DecisionType.PUNG)
	...
	elseif msgTbl.m_think[1] == 6 then
		...
		-- 碰牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneDDH.DecisionType.EAT)
	...
	elseif msgTbl.m_think[1] == 7 or msgTbl.m_think[1] == 8 then
		...
		-- 杠牌动画
		self:showDecisionAnimation(seatIdx, PlaySceneDDH.DecisionType.BRIGHT_BU)
...
function PlaySceneDDH:onRcvChatMsg(msgTbl)
...
--单局结束
function PlaySceneDDH:onRcvRoundReport(msgTbl)
	self.roundReportMsg = nil
	gt.log("00000---")
	gt.dump(msgTbl)
...
function PlaySceneDDH:showRoundReport(msgTbl)
	...
	local callFunc = cc.CallFunc:create(function(sender)
		...
		if msgTbl.m_end == 0 then -- 不是最后一局
			...
			self:addChild(roundReport, PlaySceneDDH.ZOrder.ROUND_REPORT)
		else
			gt.isZan = true
...
function PlaySceneDDH:copyTab(st)
    local tab = {}
...
--八局结束总结
function PlaySceneDDH:onRcvFinalReport(msgTbl)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
function PlaySceneDDH:showFinalReport(msgTbl)
	self.lastRound = true
	...
	self:addChild(self.finalReport, PlaySceneDDH.ZOrder.REPORT)
...
-- 扎鸟
function PlaySceneDDH:onRcvZhaNiao(msgTbl)
	...
	local callFunc = cc.CallFunc:create(function(sender)
		table.foreach(msgTbl.m_bird_infos, function(i, bird)
			...
			local card = bird[2]
			if seatIdx > 0 then
				local roomPlayer = self.curRoomPlayers[seatIdx]
				...
				local sprite = self:addAlreadyOutMjTilesByCopy(
					msgTbl.m_end_pos + 1, card[1], card[2], self.curRoomPlayers[msgTbl.m_end_pos + 1])
...
function PlaySceneDDH:birdFly(layer, x, y, ex, ey)
	local x = x
	local y = y
	local ex = ex
	local ey = ey + 50
...
-- @description 更新当前时间
-- end --
function PlaySceneDDH:updateCurrentTime()
...
function PlaySceneDDH:checkPlayName( str )
	local retStr = ""
	local num = 0
	local lenInByte = #str
	local x = 1
...
-- end --
function PlaySceneDDH:roomAddPlayer(roomPlayer)
	local playerInfoNode = gt.seekNodeByName(self.rootNode, "Node_playerInfo_" .. roomPlayer.displaySeatIdx)
...
function PlaySceneDDH:showPaiType(roundReportMsg, seatIdx)
	if not self.paiTypeActivity then
		...
		self:addChild(self.paiTypeActivity, PlaySceneDDH.ZOrder.ROUND_REPORT_Activity)
	end
end


function PlaySceneDDH:showFangjianYuAnimation(msgTbl, seatIdx)
	self.curRoomPlayers = self:copyTab(self.roomPlayers)
...
-- @param msgTbl 消息体
-- end --
function PlaySceneDDH:playerEnterRoom(msgTbl)
	-- 房间中的玩家
	self.roomPlayers = {}
	-- 玩家自己放入到房间玩家中
	local roomPlayer = {}
	...
	for i=1,#msgTbl.m_playtype do
		if msgTbl.m_playtype[i] == 31 then
			tableStr = tableStr .. "1拖1 "
		elseif msgTbl.m_playtype[i] == 32 then
			tableStr = tableStr .. "1拖2 "
		elseif msgTbl.m_playtype[i] == 33 then
		...
		elseif msgTbl.m_playtype[i] == 2 then
			tableStr = tableStr .. "点炮胡(可抢杠) "
		elseif msgTbl.m_playtype[i] == 5 then
		...
		elseif msgTbl.m_playtype[i] == 38 then
...
-- end --
function PlaySceneDDH:readyBtnClickEvt()
	local readyBtn = gt.seekNodeByName(self.rootNode, "Btn_ready")
	readyBtn:setVisible(false)
...
-- @param seatIdx 座次
-- end --
function PlaySceneDDH:playerGetReady(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneDDH:hidePlayersReadySign()
	for i = 1, 4 do
		local readySignNode = gt.seekNodeByName(self.rootNode, "Node_readySign")
...
-- @param sender
-- end --
function PlaySceneDDH:showPlayerInfo(sender)
	...
	self:addChild(playerInfoTips, PlaySceneDDH.ZOrder.PLAYER_INFO_TIPS)
end
...
-- @return 玩家麻将基础参考位置
-- end --
function PlaySceneDDH:setPlayerMjTilesReferPos(displaySeatIdx)
...
-- @param seatIdx 座位编号
-- end --
function PlaySceneDDH:setTurnSeatSign(seatIdx)
	-- 显示轮到的玩家座位标识
	local turnPosBgSpr = gt.seekNodeByName(self.rootNode, "Spr_turnPosBg")
...
-- @return
-- end --
function PlaySceneDDH:playTimeCDStart(timeDuration)
...
-- @param delta 定时器周期
-- end --
function PlaySceneDDH:playTimeCDUpdate(delta)
...
-- @param mjNumber
-- end --
function PlaySceneDDH:addMjTileToPlayer(mjColor, mjNumber)
...
-- end --
function PlaySceneDDH:sortPlayerMjTiles()
...
-- @return 选中的麻将牌
-- end --
function PlaySceneDDH:touchPlayerMjTiles(touch)
	local roomPlayer = self.roomPlayers[self.playerSeatIdx]
...
-- @param mjNumber 麻将编号
-- end --
function PlaySceneDDH:addAlreadyOutMjTiles(seatIdx, mjColor, mjNumber, isHide)
	-- 添加到已出牌列表
	local roomPlayer = self.roomPlayers[seatIdx]
...
function PlaySceneDDH:addAlreadyOutMjTilesByCopy(seatIdx, mjColor, mjNumber, roomPlayer, isHide)
	-- 添加到已出牌列表
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(string.format(gt.MJSprFrameOut, roomPlayer.displaySeatIdx, mjColor, mjNumber))
	local mjTile = {}
	mjTile.mjTileSpr = mjTileSpr
...
function PlaySceneDDH:checkOutMjTile( color, number )
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
...
function PlaySceneDDH:checkMjTile()
...
function PlaySceneDDH:checkPengAndGang( isGang, color, number )
	local mjNumber = 0
	for seatIdx, roomPlayer in ipairs(self.roomPlayers) do
		for i,outMjTile in ipairs(roomPlayer.outMjTiles) do
			--检查打出去的牌里是否还有碰 杠的牌
...
-- end --
function PlaySceneDDH:removePreRoomPlayerOutMjTile()
	-- 移除上家打出的牌
...
-- @param seatIdx 座次
-- end --
function PlaySceneDDH:showOutMjtileSign(seatIdx)
	local roomPlayer = self.roomPlayers[seatIdx]
	local endIdx = #roomPlayer.outMjTiles
	local outMjTile = roomPlayer.outMjTiles[endIdx]
...
-- end --
function PlaySceneDDH:hideOtherPlayerMjTiles(seatIdx, isBar, isBrightBar)
	if seatIdx == self.playerSeatIdx then
		return
	end
...
-- end --
function PlaySceneDDH:addMjTilePung(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	local pungData = {}
...
-- end --
function PlaySceneDDH:addMjTileBar(seatIdx, mjColor, mjNumber, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- end --
function PlaySceneDDH:addMjTileBu(seatIdx, mjColor, mjNumber, isBrightBu)
	local roomPlayer = self.roomPlayers[seatIdx]
...
-- 胡牌之后,牌应该推到
-- end --
function PlaySceneDDH:showAllMjTilesWhenWin(seatIdx, m_cardCount, m_cardValue, m_color, m_number)
	dump(m_cardValue)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
...
-- @return
-- end --
function PlaySceneDDH:pungBarReorderMjTiles(seatIdx, mjColor, mjNumber, isBar, isBrightBar)
	local roomPlayer = self.roomPlayers[seatIdx]
	local mjTilesReferPos = roomPlayer.mjTilesReferPos
	-- 显示碰杠牌
	local groupMjTilesPos = mjTilesReferPos.groupMjTilesPos
...
-- @param mjNumber
-- end --
function PlaySceneDDH:changePungToBrightBar(seatIdx, mjColor, mjNumber)
	local roomPlayer = self.roomPlayers[seatIdx]
	if seatIdx == self.playerSeatIdx then
		for i, mjTile in ipairs(roomPlayer.holdMjTiles) do
...
-- @param decisionType 决策类型
-- end --
function PlaySceneDDH:showDecisionAnimation(seatIdx, decisionType, huCard)
	...
	-- 接炮胡，自摸胡，明杠，暗杠，碰文件后缀
	local decisionSuffixs = {1, 4, 2, 2, 3}
	local decisionSfx = {"hu", "zimo", "gang", "gang", "peng"}
	-- 显示决策标识
	local roomPlayer = self.roomPlayers[seatIdx]
	...
	local seqAction = cc.Sequence:create(easeBackAction, fadeOutAction, callFunc)
	if decisionType == 3 or decisionType == 4 then
		decisionSignSpr:setVisible(false)
	end
	decisionSignSpr:runAction(seqAction)

	-- 播放全屏动画
	if decisionType == PlaySceneDDH.DecisionType.BRIGHT_BAR then
		if not self.brightBarAnimateNode then
			...
			self.rootNode:addChild(brightBarAnimateNode, PlaySceneDDH.ZOrder.MJBAR_ANIMATION)
		end
		self.brightBarAnimate:play("run", false)
	elseif decisionType == PlaySceneDDH.DecisionType.DARK_BAR then
		if not self.darkBarAnimateNode then
			...
			self.rootNode:addChild(darkBarAnimateNode, PlaySceneDDH.ZOrder.MJBAR_ANIMATION)
		end
...
-- @param seatIdx 座次
-- end --
function PlaySceneDDH:showMjTileAnimation(seatIdx, startPos, mjColor, mjNumber, cbFunc)
...
function PlaySceneDDH:backMainSceneEvt(eventType, isRoomCreater, roomID)
	-- 事件回调
	gt.removeTargetAllEventListener(self)
	-- 消息回调
	self:unregisterAllMsgListener()
...
function PlaySceneDDH:createFlimLayer(flimLayerType,cardList)
	-- 一个麻将
	local mjTileName = string.format(gt.SelfMJSprFrameOut, 2, 2)
	local mjTileSpr = cc.Sprite:createWithSpriteFrameName(mjTileName)
	local width_oneMJ = mjTileSpr:getContentSize().width
...
function PlaySceneDDH:removeFlimLayer(flimLayerType)
	local child = self:getChildByTag(PlaySceneDDH.TAG.FLIMLAYER_BAR)

	if flimLayerType == PlaySceneDDH.FLIMTYPE.FLIMLAYER_BAR then
		child = self:getChildByTag(PlaySceneDDH.TAG.FLIMLAYER_BAR)
	elseif flimLayerType == PlaySceneDDH.FLIMTYPE.FLIMLAYER_BU then
		child = self:getChildByTag(PlaySceneDDH.TAG.FLIMLAYER_BU)
	else
...
-- @param isShow 显示标志
-- end --
function PlaySceneDDH:LaoYueNodeVisible(isShow)
	-- body
	local laoYueNode = gt.seekNodeByName(self.rootNode, "Node_Laoyue")
	if(isShow)then
		self.rootNode:reorderChild(laoYueNode, PlaySceneDDH.ZOrder.HAIDILAOYUE)
...
-- @param msgTbl
-- end --
function PlaySceneDDH:onHaidiRcvMakeDescision(msgTbl)
	-- body
	local seatIdx = msgTbl.m_pos + 1
	-- 当前出牌座位
	self:setTurnSeatSign(seatIdx)
...
function PlaySceneDDH:showHaidiInLayer(msgTbl)
	self.isPlayerDecision = true
	...
	spr:setSpriteFrame(string.format(gt.SelfMJSprFrameOut, msgTbl.m_color, msgTbl.m_number))
...
function PlaySceneDDH:startAudio()
...
function PlaySceneDDH:stopAudio()
	if gt.isUseNewMusic() == false then
		return
	end
	--停止录音
...
function PlaySceneDDH:cancelAudio()
	self:getLuaBridge()
	if gt.isIOSPlatform() then
...
function PlaySceneDDH:IsSameIp()
...
function PlaySceneDDH:showLastFourCard()
